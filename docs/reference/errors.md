...
data that looks right but isn't. The exceptions are two **warnings** that let the run
finish — `TDC136` (a single malformed `<map>` row is skipped and the valid rows still
apply) and `TDC200` (a memory estimate that's large but still fits). Both say as much in
their rows below.
...
| `TDC110` | *retired* — an unnamed `<gen>` beside a named one now [composes](../core-concepts/sequences.md#a-composed-sequence) rather than failing | — |
...
| `TDC136` | A `<map>` row isn't `KEY:VALUE` (a **warning** — the bad row is skipped and the rest of the map still applies) | One pair per entry, separated by a colon |
...
| `TDC171` | A data-pack file lands at no address (a **warning** — the file is skipped) | Add `address:` or `locale:` to its header, or move it under a locale folder |
| `TDC200` | The estimated memory use is a large share of this machine's RAM (a **warning** — the run continues) | For very large sets, use `mode="disk"`, which keeps memory flat |
