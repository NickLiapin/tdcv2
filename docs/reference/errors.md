...
| Code     | Fires when                                                        | Fix                                                                                                                                    |
| :------- | :---------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| `TDC001` | The file has no `<tdc>` root, **or** it could not be parsed at all — a syntax error, including a closing tag that names another element (`<sequence>…</gen>`) | Wrap everything in a single `<tdc>…</tdc>`, or fix the syntax the message points at. Nothing else is checked until the file parses      |
