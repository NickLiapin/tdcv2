...
The domain is signed 64-bit, the same as the [compute layer](compute.md#top) — the widest integer
all five implementations hold natively. Past it the answer is a refusal, in the same words
compute uses, rather than a quiet slide back into floating point:
