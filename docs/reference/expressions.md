...
**The rest of the maths library.** `asinh`, `acosh`, `atanh`, `log2`, `log1p`, `expm1`,
`hypot`, `sign`, `degrees` and `radians` are refused by name rather than guessed at:
...
Note what it does NOT say: "did you mean `atan`?" Edit distance would have offered exactly
that, and `atan` is the inverse of a different function. A name on this list is answered with
the reason instead.
