...
- **`--engine 1|2|3`** — force a specific engine; `--stream` is a legacy alias for
  `--engine 2`. Naming an engine means it **refuses** rather than quietly running another, so
  a measurement measures what it says it does. `--mode` describes a cost instead, and a run
  that describes a cost may still be handed to a different engine.
