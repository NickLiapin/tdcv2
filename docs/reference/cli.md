...
meant to — a birthday that tracks today is the point. But it makes the clock a fourth
input to a run, alongside the config, the seed and the engine, and it is the one you
cannot write down. The same file with the same seed gives you different rows tomorrow.
