...
All five are published: **0.3.0** on npm, PyPI, NuGet and crates.io, and
**0.3.0** on Maven Central — usually the same number, occasionally one
...
coincidence: where they match, `tdcv2 0.3.0` from any of them answers a
...
| Java           | `java -jar tdcv2-0.3.0-cli.jar tdcv2-examples/01-starter.tdc` — the `cli` classifier of the library's own coordinates            |
