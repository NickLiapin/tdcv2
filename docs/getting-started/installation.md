...
All five are published, at the same version: **0.2.0** on npm, PyPI, Maven
Central, NuGet and crates.io. Equal version numbers are not a coincidence — they
mean the same engine, so `tdcv2 0.2.0` from any one of them answers a config the
same way.
...
| Java           | `java -jar tdcv2-0.2.0-cli.jar tdcv2-examples/01-starter.tdc` — the `cli` classifier of the library's own coordinates            |
