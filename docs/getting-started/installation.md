...
All five are published: **0.3.1** on npm, PyPI, NuGet and crates.io, and
**0.3.1** on Maven Central — usually the same number, occasionally one
...
coincidence: where they match, `tdcv2 0.3.1` from any of them answers a
...
```kotlin
implementation("io.github.nickliapin:tdcv2:0.3.1")
```
...
java -jar tdcv2-0.3.1-cli.jar init
```
...
| Java           | `java -jar tdcv2-0.3.1-cli.jar tdcv2-examples/01-starter.tdc` — the `cli` classifier of the library's own coordinates            |
