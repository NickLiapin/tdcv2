...
- **Data and declarative generators only — no executable code.** A pack is either "take
  ...
  [Writing your own](writing-your-own.md#top)). Nothing arbitrary runs, which is what keeps
...
| `inject`      | A custom interpolation marker for a generator                  | [Writing your own](writing-your-own.md#top) |
...
locale segment. See the [`template`](../generators/template.md#top) generator for how one
...
You call an address the way you call any [`template`](../generators/template.md#top)
generator. Pull the parts you want into named
[sequences](../core-concepts/sequences.md#top) and assemble them in the `.tdc`:
...
[`<distinct>`](../constructs/unique-values.md#top) — the mechanics are covered in
[Writing your own](writing-your-own.md#top).
...
- a **weighted** list — each value's real-world frequency, laid out exactly, with the
  same Hamilton method as [`percent`](../generators/text.md#top);
- a small **generator** written in the DSL — a [`regex`](../generators/regex.md#top)
  pattern, a name assembled from neighboring lists, or a
  [`<mix>`](../reference/tags.md#top) that splits by exact percentage.
...
- **Your own folders** are added with the CLI flag `--data-path <folder>` (repeatable)
  or the library's `dataPaths`. See [Installing packs](installing-packs.md#top) for the
...
- **[Installing packs](installing-packs.md#top)** — `tdcv2 init` and `tdcv2 pack`.
- **[Writing your own](writing-your-own.md#top)** — headers, generators, weighted packs.
- **[The `template` generator](../generators/template.md#top)** — calling addresses and
  locale resolution.
