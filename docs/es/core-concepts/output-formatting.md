...
Una vez que los datos están declarados como [secuencias](sequences.md#top), el **bloque de
...
- **A** — se imprime una vez, al mero principio
- **B** — antes de cada tarjeta
- **C** — una línea de la tarjeta
- **D** — entre las líneas de una tarjeta — nunca después de la última línea
- **E** — después de cada tarjeta
- **F** — entre tarjetas — nunca después de la última tarjeta
- **G** — se imprime una vez, al mero final
- **H** — todo lo que está dentro del corchete se repite una vez por tarjeta
...
`<block>` es el único hijo **obligatorio** de [`<tdc>`](../reference/tags.md#top). No tiene
...
[`<gen>`](../generators/overview.md#top) mal puesto o una etiqueta con errata antes de que
corrompa la salida:
...
directamente en un `<line>`: declare una [`<sequence>`](sequences.md#top) nombrada (o un
[`<mix>`](../reference/tags.md#top)) en [`<env>`](configuration.md#top) y refiéralo con
`${{Name}}`. Así se mantiene separado _qué son los datos_ de _cómo se acomodan_.

Un límite relacionado: un generador
[`advanced_regex`](../generators/advanced-regex.md#top) usado desde la salida no puede
...
- **En el diseño, `<data>` vive dentro de un `<line>`** —las líneas del bloque o las de
  ...
  `<env>`: como hijo de una [`<sequence>`](sequences.md#top) es texto literal pegado a un
...
2. **Una secuencia declarada sin valor en esta fila** (filtrada por un
   [`parent`](../guides/hierarchical-dependencies.md#top) en esta iteración) → la **cadena
   vacía**.
...
[Nombres integrados](../reference/builtins.md#top).
...
escapes `csv`/`sql`— está en **[Máscaras y mayúsculas](../guides/masks-and-case.md#top)**.
...
Todas las fixtures viven dentro de [`<env>`](configuration.md#top) y, como el bloque,
...
- **La interpolación SÍ se ejecuta en las fixtures.** Un `${{Name}}` lee la fila junto a
  la que está la fixture: `<before>` ve la primera, `<after>` la última, y las de registro
  y de línea la fila que envuelven. Eso es lo que permite que un registro ponga sus propios
  campos alrededor de una lista anidada. (Es uno de los pocos comportamientos que hubo que
  corregir en la implementación de referencia: imprimía las llaves literalmente mientras
  los cuatro ports las expandían.)
...
[`pair`](../reference/attributes.md#top); tanto la etiqueta de apertura como la de cierre
deben llevar el mismo marcador. Aquí el `${{...}}` también debe aparecer literal, así que
el ejemplo además aparta el [marcador de interpolación](configuration.md#top) con
`inject`:
...
- **[Máscaras y mayúsculas](../guides/masks-and-case.md#top)** — el conjunto completo de
  filtros y máscaras.
- **[Determinismo y proporciones](determinism.md#top)** — `seed`, `count` y `percent`.
- **[Dependencias jerárquicas](../guides/hierarchical-dependencies.md#top)** — cómo `parent`
  filtra una secuencia, lo que da el caso de la cadena vacía de arriba.
- [Nombres integrados](../reference/builtins.md#top) — `_count`, `_first`, `_last`, `_total`.
- [Referencia de etiquetas](../reference/tags.md#top) — `<block>`, `<line>`, `<data>` y las
