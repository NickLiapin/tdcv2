...
así, y un pack simple o una lista de archivo ahora toman el mismo camino —, y el
reparto que divide una columna libre entre los bloques del `<switch>` entrega
sus unidades sobrantes por una lista global de reclamos, de modo que ningún
bloque se queda sin un valor. El resultado es un borde limpio: medido sobre una
forma —dos sujetos, tres nombres cada uno, cinco valores compartidos— **todo
count hasta el techo de 30 se genera, desde cualquier semilla, y todo count por
encima se rechaza**. La semilla decide qué disposición le toca, nunca si existe
una.
...
`<switch>`, sumados — y es exactamente el count más alto que sí se genera. Si un
count es rechazado, el arreglo es más valores en algún miembro: más nombres
distintos, rangos más anchos.
