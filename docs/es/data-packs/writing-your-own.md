...
Las salidas de ejemplo de abajo son **ilustrativas**: los valores exactos dependen del
seed y pueden cambiar entre versiones del núcleo. Lo que sí está garantizado —el
determinismo por seed y las proporciones exactas— se señala donde importa.
...
entrega un valor *calculado*. Está escrito en el DSL propio de TDC, así que no hay nada
...
A lo largo de 100 filas, exactamente 60 llevan dos apellidos y 40 llevan uno: el cuerpo
del generador corre sobre el mismo motor, que conoce `count` y reparte los porcentajes
con Hamilton, no al azar.
...
motor vuelve a extraer uno de los valores, y el determinismo por seed se conserva.
