...
[`<data>`](../core-concepts/output-formatting.md#top) para el texto literal entre ellas.

Un `<data>` ahí también puede **leer la fila en la que está**: `${{Name}}` dentro de un
case se resuelve igual que en una línea de salida, filtros incluidos.
...
Eso no es lo mismo que un `<gen type="template">` a su lado: el generador saca un valor
NUEVO, mientras que una referencia mantiene el registro coherente con lo que la fila ya
tiene. Un nombre que nadie declaró se rechaza (`TDC193`) en vez de imprimirse tal cual.
...
El autocompletado de direcciones en el editor se basa en esas mismas cabeceras
`description:`, y ya está disponible — véase [Compatibilidad con el
editor](../getting-started/editor-support.md#top). Escriba una descripción que valga la pena
leer: es lo que verá junto a la dirección quien la complete.

## Describir una carpeta — `_pack.json`

Una carpeta de paquetes es todo contenido y ninguna procedencia: las listas y los
generadores dicen qué producen y nada sobre de dónde salieron. Eso está bien
mientras los únicos paquetes son los incluidos, y deja de estarlo en el momento en
que alguien le pasa una carpeta y pregunta si los datos que usted construyó con
ella se pueden distribuir.
...
Todos los campos son opcionales, y **nada de esto llega a los datos generados**. Un
manifiesto no puede cambiar un solo valor: la misma semilla da los mismos bytes esté
o no esté.
...
En cada ruta de datos configurada, y **en cada carpeta un nivel adentro**. Eso cubre
las dos formas que toma un manifiesto: «toda esta carpeta es mi paquete» y «esta
configuración regional de adentro vino de otro lado».

Ahí se detiene, a propósito. Bajar más invitaría a un manifiesto por cada `.txt`, y
la pregunta que esto responde — quién escribió estos datos y bajo qué licencia — no
es una que una sola lista de nombres de ciudades tenga por su cuenta.

### Qué pasa cuando está mal

`tdcv2 pack info` **rechaza** un manifiesto que no se puede leer, o uno cuyo `name`,
`version`, `license`, `author`, `homepage` o `description` no sea texto. Leer ese
archivo es todo el trabajo de ese comando, y una licencia que su autor escribió y
nadie puede leer es peor que una que nadie escribió.

Una corrida **nunca lo menciona**. El manifiesto no alcanza ningún valor, así que
detener una generación por él sería bloquear el trabajo por un campo que el trabajo
nunca lee.

Las claves que TDC no conoce se conservan en silencio, así que una carpeta escrita
para una versión más nueva — o que lleve los campos de su propia herramienta junto a
estos — sigue funcionando aquí.
