...
- **A** — datos internacionales, iguales en cualquier locale
- **B** — el eje del idioma: cómo se escriben los nombres y las palabras
- **C** — el eje del país: lo que es específico de un solo país
- **D** — una dirección — cuál conjunto la responde depende de la ejecución, no de la dirección
...
- **Solo datos y generadores declarativos: nada de código ejecutable.** Un paquete es o
  ...
  TDC (vea [Cree su propio paquete](writing-your-own.md#top)). Nada de código arbitrario:
...
el segmento del locale. Vea el generador [`template`](../generators/template.md#top) para
...
se explica en [Cree su propio paquete](writing-your-own.md#top).

## Cómo usar una dirección

Llame a una dirección como a cualquier generador
[`template`](../generators/template.md#top). Traiga las partes que quiera a
[secuencias](../core-concepts/sequences.md#top) con nombre y ármelas en el `.tdc`:
...
envuélvalos en [`<distinct>`](../constructs/unique-values.md#top); la mecánica se explica en
[Cree su propio paquete](writing-your-own.md#top).
...
- una lista **ponderada** — la frecuencia real de un valor, repartida con exactitud por
  el mismo método de Hamilton que usa [`percent`](../generators/text.md#top);
- un pequeño **generador** en el DSL — una plantilla de
  [`regex`](../generators/regex.md#top), un nombre armado a partir de listas vecinas, o un
  [`<mix>`](../reference/tags.md#top) que reparte por porcentaje exacto.
...
- **Sus propias carpetas** se agregan con la bandera de CLI `--data-path <carpeta>`
  ...
  [Instalar paquetes de datos](installing-packs.md#top) para el flujo `tdcv2 init` /
  `tdcv2 pack`.
...
- **[Cree su propio paquete](writing-your-own.md#top)** — encabezados, generadores,
  paquetes ponderados.
- **[El generador `template`](../generators/template.md#top)** — llamar direcciones y
