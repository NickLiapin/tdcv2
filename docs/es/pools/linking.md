...
Declarar la referencia por la que filtras **encima** de la que filtra es la costumbre que
conviene mantener: se lee en el orden en que ocurre. No es, sin embargo, un requisito que
el motor imponga: intercambiar `Seen` y `Assisted` en este ejemplo produce una salida
idéntica y pasa `check`, porque las TABLAS de los pools se construyen antes de sacar
ninguna fila, así que el filtro encuentra lo que necesita esté en la línea que esté.
...
- [Resumen](overview.md#top) — qué es un pool, y el techo de tamaño
- [Acotar con `filter`](filter.md#top) — el lenguaje de expresiones en el que se escribe un
  enlace
- [Dependencias jerárquicas](../guides/hierarchical-dependencies.md#top) — `parent`, la otra
