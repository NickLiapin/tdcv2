...
A partir de aquí `base` se lee como `<var name="base"/>` en el resto de este `<compute>`,
...
Ocho dígitos y el noveno, unidos. `<result>` es donde termina un `<compute>`: hay
exactamente uno y su valor es lo que imprime `${{Routing}}`.
