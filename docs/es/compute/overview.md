...
Un `<compute>` produce exactamente un valor. `<result>` es la forma de decir de qué árbol
sale ese valor; todo lo demás en el bloque es una ligadura `<let>` o parte de ese árbol.
Cuando el bloque es una sola expresión puede omitir `<result>`: la expresión es el valor.
Lo que se rechaza son dos árboles lado a lado, porque solo uno de ellos puede ganar.
