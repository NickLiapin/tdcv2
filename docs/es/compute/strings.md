...
**Toma** un string → **da** un string. Normalmente del mismo largo, pero no siempre: `straße`
en mayúsculas es `STRASSE`, siete caracteres a partir de seis, y `ﬁ` se vuelve `FI`. No arme
una tubería de ancho fijo suponiendo que la cuenta sobrevive.
...
| `x[2]` `w[1]`       | un pedazo POR POSICIÓN, así que puede reordenarse o repetirse      |
| cualquier otra cosa | un literal: guion, punto, espacio, corchetes — se imprime tal cual |

Las formas con índice son las mismas que toma el filtro `mask:` — `x[0..2]` para un rango,
...
**Toma** un string más `from=` y un `to=` opcional → **da** un string. La cuenta empieza en 0, un `from` negativo cuenta desde el final, y un rango más allá del final da un string vacío en vez de un error. Si se omite `from=` vale 0, así que un `<slice>` con solo `to=` corta desde el principio.
...
| :------- | :---------- | :--------------------------------------------------------------------- |
| `from`   | no          | índice del primer carácter (empieza en cero), incluido; omitido → 0    |
...
| Atributo | Obligatorio | Define               |
| :------- | :---------- | :------------------- |
| `from`   | no          | qué buscar (literal); omitido → no se reemplaza nada |
| `to`     | no          | con qué reemplazarlo; omitido → cada coincidencia se elimina |
...
**Toma** un valor más `width=` y `fill=` → **da** un string. `width` es un mínimo: un valor que ya mide eso o más pasa intacto, nunca se recorta. Si omite `width=`, la etiqueta no hace absolutamente nada — el valor pasa tal cual, algo fácil de pasar por alto cuando una columna rellenada vuelve sin rellenar.
