...
datos que parecen correctos pero no lo son. Las excepciones son seis **advertencias** que
...
`<distinct>` con un solo miembro, que no restringe nada) y `TDC234` (un pool de más de
...
| `TDC234` | _(advertencia)_ Más de 100.000 miembros                                     | Un pool se mantiene en memoria toda la ejecución — unos 320 bytes por miembro con cuatro campos. Si querías decir el número de FILAS, eso es `count` en `<env>`                                              |
