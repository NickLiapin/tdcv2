...
| `TDC299` | _(aviso)_ `uniq="true"` sobre más de 100 000 filas                       | Cumplir una promesa sobre la columna terminada cuesta memoria que crece con `count`, en cualquier motor: el coste pertenece a la promesa, no a uno de ellos. Unos 250 bytes por valor, medido: 2 000 000 de filas cuestan unos 477 MB, y un `uniq` compuesto mide aún más. Una sola columna sacada paga dos veces, porque sacar sin reemplazo no se puede hacer fila a fila, así que además se ejecuta en memoria diga lo que diga `mode=`. Funciona; conviene hacerlo a conciencia |
