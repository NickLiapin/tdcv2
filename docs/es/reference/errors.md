...
datos que parecen correctos pero no lo son. Las excepciones son doce **advertencias** que
...
100.000 miembros), `TDC299` (una columna `uniq` más allá de 100.000 filas, cuya memoria
crece con el número de filas — en su segundo sentido, un pool declarado fuera de orden, es un error), `TDC251`
...
viaje él) y `TDC301` (una lista de porcentajes que deja un valor declarado en 0%, de modo que
ya nunca puede salir). Cada una lo indica en su fila
más abajo.
...
| `TDC254` | Un recorrido sin paso: un `repeat="2..5"` por rango junto a `order="sequential"`, o una `date` recorrida con cualquier `repeat=` | Un `repeat="4"` fijo sí funciona: la fila toma cuatro valores del recorrido. Uno por rango no tiene un número fijo de valores por fila, así que la fila 5 empezaría donde las filas 0 a 4 hubieran acabado. Y una fecha recorrida lleva un instante junto a su texto, y una fila con varias fechas no tiene uno solo que dar a `of=` y `plus=` |
...
| `TDC267` | _(segundo significado)_ `uniq="true"` en una `<sequence>` que envuelve un `<distinct>` | `<distinct>` repara un registro para que sus campos difieran; `uniq` luego reordena las columnas terminadas y no sabe qué combinaciones descartó la reparación, así que la deshace. Quédese con uno — o dé a cada campo su propia `<sequence>`, envuélvalas en `<uniq>` y ponga el `<distinct>` a nivel de env |
...
| `TDC268` | Un `<gen type="pool">` en cualquier sitio que no sea una `<sequence>` entera: bajo `if=`, como rama de respaldo, dentro de un `<case>`, como parte o como campo | Una referencia publica un REGISTRO entero como columnas `Ref.field`, y una rama o una parte no tiene columna propia bajo la que publicarlas — así que `${{Ref.name}}` llegaba a la salida como su propio texto literal, o la corrida se detenía tras un `check` limpio. Sortee el miembro en una secuencia propia y lea sus campos donde los necesite; para dejar filas sin miembro, use `parent=` |
...
| `TDC295` | Una columna calculada donde no puede significar nada: `running`, `stat` o una fórmula con `prev()` en cualquier sitio que no sea una `<sequence>` entera; cualquier `formula` o desplazamiento de fecha como parte o campo de una `<sequence>` compuesta | Las tres primeras leen las filas anteriores a esta, o todas, así que son columnas enteras, y un `<case>` o una rama `if=` recibe solo parte de las filas. Declárelas como secuencias propias y úselas por su nombre. Las partes de un registro se sortean y reordenan juntas, y un valor calculado que acaba en otra fila ya no la describe. Una `formula` simple o un desplazamiento de fecha leen solo su propia fila, así que pueden estar en un `<case>` o en una rama `if=` |
...
| `TDC299` | _(advertencia)_ `uniq="true"` sobre más de 100 000 filas                       | Cumplir una promesa sobre la columna terminada cuesta memoria que crece con `count`, en cualquier motor: el coste pertenece a la promesa, no a uno de ellos. Unos 250 bytes por valor, medido: 2 000 000 de filas cuestan unos 477 MB, y un `uniq` compuesto mide aún más. Una sola columna sacada paga dos veces, porque sacar sin reemplazo no se puede hacer fila a fila, así que además se ejecuta en memoria diga lo que diga `mode=`. Funciona; conviene hacerlo a conciencia |
...
| `TDC301` | _(advertencia)_ Una lista de proporciones que deja un valor declarado en 0% | Un `percent` más corto que `value` está bien: lo que sobra va a las posiciones que no escribió, así que `value="a,b,c" percent="30,40"` le da a `c` los 30 restantes. Cuando las proporciones escritas ya suman 100 no sobra nada, y ese valor queda declarado y no puede salir nunca: `percent="50,50"` sobre 300 filas dio 150 `a`, 150 `b`, ningún `c`. Un cero que usted mismo escribe — `percent="50,0,50"` — se toma al pie de la letra y no dice nada |
| `TDC302` | Un `<distinct>` o `<uniq>` sobre referencias `<gen type="pool">` en una forma que no puede significar nada: una referencia junto a una secuencia corriente, referencias a dos pools distintos, o más referencias que miembros tiene el pool | Un grupo sobre referencias compara QUÉ MIEMBRO tomó cada fila, así que debe haber algo comparable y miembros suficientes. Las tres formas se aceptaban y no hacían nada |
| `TDC303` | Un `missing_when=` que no decide nada: sin condición, sin la tasa `missing=` al lado, o sobre una celda con `repeat=` | La condición es la que nombra el mecanismo — `missing_when="Age < 30"` es MAR, `missing_when="_value > 150000"` es MNAR —, así que una escrita y vacía es una pregunta a medio hacer, y una sin tasa no tiene sorteo que condicionar. Una celda repetida guarda varios valores en una fila mientras la condición pregunta por uno; en vez de adivinar qué lectura quería, se rechaza. Las tres se aceptaban en silencio y no hacían nada |
| `TDC304` | Listas estacionales que no cuadran: `amplitude=` o `peak_at=` con distinta cantidad de entradas que `period=`, o un `0` entre varios períodos | `period="7,365" amplitude="120,400" peak_at="5,182"` describe las mismas dos ondas tres veces, posición por posición, así que una longitud que no cuadra no describe ninguna onda — la entrada sobrante se descartaba en silencio. Una sola `amplitude` para ondas de igual altura es la única abreviatura. `period="0"` solo significa «sin onda estacional»; entre varios es una onda sin largo junto a ondas que sí lo tienen |
| `TDC305` | Un `noise_correlation=` que no se puede honrar: fuera de −1..1, o sin `noise=` al lado | Es cuánto ruido de una fila pasa a la siguiente. En 1 la serie se iría y no volvería — una caminata aleatoria, no ruido — y por encima de 1 crece sin límite; los dos se rechazan en vez de recortarse. Sin `noise=` no hay nada que correlacionar, y el atributo no decide nada |
| `TDC306` | Un `<assert>` que lleva a la vez `that=` y `each=` | Responden a preguntas distintas y se comprueban un número distinto de veces: `that=` una vez, sobre valores de la ejecución entera, y `each=` en cada fila. Una sola etiqueta con ambos deja a quien lee sin saber a cuál de las dos se refiere la frase de `says=`. Escriba dos aserciones, cada una con su propio `says=`, para que un fallo diga cuál se rompió |
| `TDC307` | `distinct="true"` junto a `order="sequential"` | `distinct=` toma los valores de la fila sin reemplazo, y una fila que camina no toma nada: sus valores los decide su posición. Cumplir ambos es imposible, e ignorar uno en silencio es como una configuración acaba afirmando algo que nunca tuvo |
| `TDC308` | Una condición en un `<gen>` — `if=` o `missing_when=` — que nombra una columna declarada más abajo | La condición se resuelve mientras se construye esta columna, así que una columna posterior todavía no existe. Los dos motores no coincidían en qué hacer con eso, y ambas respuestas eran defendibles: el motor en memoria la leía vacía y la condición nunca se cumplía, mientras que el de flujo construía la columna bajo demanda y la condición se cumplía de verdad. Mueva la `<sequence>` que nombra por encima de esta. Una condición leída cuando la fila ya está terminada — `<data if=>`, `<line if=>`, `<assert that=>` — puede nombrar cualquier columna |
...
| `TDC153` | `local` nombra un locale sin datos de fecha | Vienen veinticuatro: `ar`, `cs`, `de`, `el`, `en`, `es`, `fi`, `fr`, `hi`, `hu`, `id`, `it`, `ja`, `ko`, `nl`, `pl`, `pt`, `ru`, `sv`, `th`, `tr`, `uk`, `vi`, `zh-cn`. Diecinueve responden también a un alias de tres letras (`ara`, `ces`, `deu`, `ell`, `eng`, `spa`, `fin`, `fra`, `hun`, `ind`, `ita`, `jpn`, `kor`, `nld`, `pol`, `por`, `tur`, `ukr`, `vie`); `hi`, `ru`, `sv`, `th`, `zh-cn` no tienen |
...
| `TDC193` | _(segundo significado)_ `anomaly_flag=` en un `<gen>` sin `anomaly=` | La columna de marca señala qué filas se volvieron atípicas, y no se volvió atípica ninguna. Añada `anomaly="p"` o quite la marca |
