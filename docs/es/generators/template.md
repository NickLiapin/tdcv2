...
| :------------------------ | :---------------------------------------------- | :----------------: |
| `person.male.firstName`   | Un nombre de pila masculino                     |         86         |
| `person.female.firstName` | Un nombre de pila femenino                      |         86         |
| `person.lastName`         | Un apellido (masculinos + comunes del locale)   |         86         |
| `person.male.diagnosis`   | Un diagnóstico masculino + los comunes          |         86         |
| `person.female.diagnosis` | Un diagnóstico femenino + los comunes           |         86         |
| `person.gender`           | Un género al azar; la etiqueta viene del locale |         86         |
| `person.b_day`            | Una fecha de nacimiento en el formato dado      |  solo el formato   |

Las seis vienen en cada uno de los **86 paquetes de idioma** del registro — la lista está en
el [catálogo](../data-packs/catalogue.md#top).
Cualquier otro locale recibe [TDC217](../reference/errors.md#top), que nombra los locales que
sí traen la ruta en lugar de dejar que la corrida adivine.
...
| Ruta               | Produce           | Depende del locale |
| :----------------- | :---------------- | :----------------: |
| `location.country` | Un nombre de país |  los 86 paquetes   |
...
bajo `en`, `Киргизия` bajo `ru`. Ojo: las listas no miden lo mismo — cada paquete nombra los
países para los que su idioma tiene nombre —, así que una misma semilla no cae en el mismo
país en los tres locales; lo que se conserva es la reproducibilidad dentro de cada uno.

> [!NOTE]
> **Cada locale que tiene un paquete**
>
> `location.country` viene en los **86** paquetes de idioma que trae el registro. Las listas no
> son todas del mismo largo: un paquete nombra los países para los que su idioma tiene nombre,
> así que van de 23 entradas a 247, con 206 en medio y 237 en inglés. Un locale sin paquete
> tampoco tiene lista: ahí la ruta da un error en vez de caer al inglés.
...
`./run csv.tdc — la fila 2 tiene ahora tres campos, no dos`
...
[`csv`](../core-concepts/output-formatting.md#top) entrecomilla el valor, y lo hace **siempre**
...
`./run csv.tdc — las mismas tres filas, y la 2 vuelve a ser un campo`
...
| `<país>.geo.<división>`       | paquete de país   | Una división, **con la palabra de ese país**    |
| `<país>.geo.<código postal>`  | paquete de país   | Un código postal, también con su propia palabra |
| `<país>.geo.streetName`       | paquete de país   | El nombre de una calle                          |

Las cinco filas del paquete de idioma existen hoy en **`en` y `ru`**; pida `geo.country`
con `local="es"` y la ejecución se detiene con TDC217, nombrando las locales que sí lo
traen. Las filas de país son donde la tabla tiene que quedarse vaga, y la razón es el
tema mismo: **un paquete de país nombra sus divisiones como las nombra ese país.**

| Hoja              | Paquetes de país que la traen |
| :---------------- | :---------------------------- |
| `geo.city`        | 147 de 152                    |
| `geo.postalCode`  | 76                            |
| `geo.region`      | 68                            |
| `geo.streetName`  | 45 (más 38 con `geo.streetNamed`) |
| `geo.province`    | 35                            |
| `geo.municipality`| 21                            |
| `geo.district`    | 21                            |
| `geo.zip`         | 16                            |

Alguna división existe en 137 de los 152 paquetes de país y un código postal en 92, bajo
...
`check` — el diagnóstico enumera lo que hay de verdad:
...
Lea [Paquetes de datos](../data-packs/overview.md#top) para saber qué trae un país concreto.
...
| `precision` | `millisecond` | `day`, `second` o `millisecond`                  |
...
> [!NOTE]
> **La misma fecha de nacimiento, dos valores por omisión**
>
> `person.b_day` y [`<gen type="date" value="birth">`](date.md#un-cumpleaños-con-valuebirth)
> calculan lo mismo a partir de la misma ventana de edad, y su valor por omisión difiere:
> la plantilla saca un **milisegundo**, el generador de fechas un **día**. Con un formato
> sin hora no se nota. Pida la hora y la diferencia es el valor entero:
>
> ```
...
> ```
>
> Escriba `precision="day"` en la plantilla cuando una fecha de nacimiento deba ser una
> fecha y nada más. No es solo cosmético: redondear al día puede mover el valor a la fecha
> vecina, así que las dos precisiones no siempre dan el mismo día.
...
| `precision` | `day`       | `day`, `second` o `millisecond`              |

`range` acepta esa escritura y ninguna otra: dos fechas con **puntos**, separadas por
un guion. La escritura `"2020-01-01..2020-12-31"` —la que usa `<gen type="date">`— se
rechaza con [TDC073](../reference/errors.md#top) en vez de leerse mal en silencio.
...
#### Una marca de tiempo en vez de una fecha

Por omisión el sorteo se ajusta al **día**, así que un formato que pide hora imprime
medianoche en cada fila. `precision="second"` sortea también la hora del día:
...
Es el espejo de la nota sobre `person.b_day` de arriba: el cumpleaños por omisión es
**milisegundo** y un rango es **día**, así que cada uno necesita la corrección
contraria cuando quiere el otro comportamiento.
