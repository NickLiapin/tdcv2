...
`\k<name>` repite el grupo por ese nombre, igual que en [`regex`](regex.md#top), y es el
mismo grupo que repite `\1`:

```xml
<gen type="advanced_regex" value="(?<c>(?%{50:AB;50:CD}))-\k<c>"/>
```

El nombre debe pertenecer a un grupo que ya **cerró**: es la regla que siguen `\1` y
`(?if{…})`, porque un grupo más adelante no ha producido nada que repetir.

Los nombres empiezan con una letra o `_` y llevan letras, dígitos y `_`. Dos grupos no
pueden compartir un nombre: `(?if{sex=…})` sería entonces un volado entre ellos. Eso
vale también cuando uno de los dos está dentro del otro, y no solo al lado.
...
`uniq="true"` da a cada fila una cadena distinta, y las cuotas exactas siguen siendo exactas:
...
El sesenta por ciento de las matrículas empieza por `77` y el cuarenta por `78`, igual que sin
`uniq`, y no hay dos matrículas iguales. Primero la columna se reparte cuota por cuota; una
matrícula que se repite se vuelve a sacar **por las mismas ramas**, así que una fila de `77` sigue
siendo de `77` y ninguna cuota se mueve ni una fila.

Por eso «¿hay sitio?» se pregunta a cada cuota, no solo al patrón. Cada cuota se cuenta por sus
propias ramas: `(?%{70:RU;30:US})-[0-9]{3}` produce 2 000 cadenas, y con 2 000 filas su cuota
RU necesita 1 400 de las 1 000 que empiezan por `RU-`. La corrida se rehúsa antes de volver a
sacar nada, nombrando la cuota por su porcentaje y ambos números. Una cuota no puede tomar
prestado de otra sin dejar de ser exacta.

Un condicional que lee un grupo con cuotas sigue de acuerdo con él: el grupo ya está repartido,
así que la rama que lo lee es la misma tras volver a sacar. Cuando una elección con cuotas está
dentro de una repetición, de un `|` o de un condicional, las ramas de una fila dependen también de
sus sorteos libres, y un nuevo intento que se desvía a otras ramas se descarta en lugar de mover
una cuota. Esa cuota no se puede contar por separado, así que la cuenta del patrón entero ocupa su
lugar, y una cuota que se agota se informa tras una larga racha de repeticiones, no por adelantado.
