...
ponderada** (en inglés, *weighted choice*), que acomoda las variantes hasta cantidades
**exactas**.
...
Las salidas de ejemplo de abajo son ilustrativas — las filas exactas dependen del seed
...
> proporciones exactas en un flujo, sin retener la columna completa. Y si *a mano* se
...
## Planeado, aún no implementado

Lo siguiente describe la dirección prevista, pero **no** es sintaxis válida hoy.
Capturas con nombre:
...
Condicionales, cuyas ramas están pensadas como expresiones `advanced_regex` completas,
de modo que elecciones ponderadas y más condicionales puedan anidarse dentro:

```text
(?if{sex=male:MR;sex=female:MS})
```

Hasta que lleguen, modele la lógica entre campos con un filtro
[`parent`](../core-concepts/sequences.md#secuencias-dependientes-parent) o con un
[`<mix>`](../reference/tags.md#distribuciones-y-selección).
