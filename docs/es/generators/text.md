...
Sin `percent`, las opciones se reparten **por partes iguales** — y por partes iguales
significa una cuota exacta sobre toda la ejecución, no un dado por fila. Tres valores en
300 filas salen 100 / 100 / 100 fila a fila, y en 301 filas uno se lleva la extra. El
mismo `seed` siempre reproduce la misma secuencia. Los valores pueden
...
Al ser una cuota, la columna se acomoda contra el **total**: se recalcula cuando `count`
cambia, y una corrida corta no es prefijo de una larga. Vea
...
los porcentajes. Lo aleatorio es solo el _orden_ — por eso `percent` y
`order="sequential"` se excluyen (`TDC271`): recorrer la lista en orden ya fija qué valor
recibe cada fila, y no queda proporción que repartir.
