...
| `local`               | `ar`, `de`, `el`, `en`, `es`, `fr`, `it`, `pl`, `pt`, `ru`, `zh-cn`. Nueve de ellos responden también a un alias de tres letras — `ara`, `deu`, `ell`, `eng`, `spa`, `fra`, `ita`, `pol`, `por` —, y `ru` y `zh-cn` no tienen ninguno. Se hereda de [`<env>`](../reference/tags.md#top) si se omite |
