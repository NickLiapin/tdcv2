...
| `local`               | Veinticinco: `ar`, `cs`, `de`, `el`, `en`, `es`, `fi`, `fr`, `hi`, `hu`, `id`, `it`, `ja`, `ko`, `nl`, `pl`, `pt`, `ru`, `sv`, `th`, `tr`, `uk`, `vi`, `zh-cn`, `zh-tw`. Diecinueve responden también a un alias de tres letras — `ara`, `ces`, `deu`, `ell`, `eng`, `spa`, `fin`, `fra`, `hun`, `ind`, `ita`, `jpn`, `kor`, `nld`, `pol`, `por`, `tur`, `ukr`, `vie` —, y `hi`, `ru`, `sv`, `th`, `zh-cn`, `zh-tw` no tienen ninguno (el `zh` a secas se acepta y lee la tabla tradicional). Se hereda de [`<env>`](../reference/tags.md#top) si se omite |
...
Un **paso de calendario** — `1mo`, `3mo`, `1y` — se rechaza por la razón opuesta: _no_
...
separados no pueden decirlo. Ponen el alta _antes_ del ingreso en buena parte de
las filas.
...
Cada alta es posterior a su propio ingreso, entre 2 y 14 días — y la _duración_
...
Una columna de fechas imprime una _representación_ de su fecha. `03/02/2026` es
...
columna cargada con [`file`](./file.md#top), por ejemplo. Allí solo existe el texto,
...
una que [`missing`](../guides/missing-data.md#top) vació — tampoco recibe fecha. No
...
Un desplazamiento lee solo su propia fila, así que también puede ser una de las ramas de una
elección por fila: un [`<case>`](../constructs/switch.md#case--una-rama-con-un-generador), a
cualquier profundidad, o una de las ramas `<gen if="…">` de una secuencia. Cada fila de la rama
se mide desde su propia fecha.

```xml
<tdc>
  <env count="6" seed="ship" local="es">
    <sequence name="Ordered"><gen type="date" from="2026-03-01" to="2026-03-31" format="D [de] MMMM"/></sequence>
    <sequence name="Method"><gen type="text" value="mensajería,recogida"/></sequence>
    <switch name="Ready" on="Method">
      <case is="mensajería"><gen type="date" of="Ordered" plus="2..5d" format="D [de] MMMM"/></case>
      <case is="recogida"><data>el mismo día, en el mostrador</data></case>
    </switch>
  </env>
...
```
23 de marzo | mensajería | 27 de marzo
6 de marzo | mensajería | 11 de marzo
13 de marzo | recogida | el mismo día, en el mostrador
21 de marzo | recogida | el mismo día, en el mostrador
7 de marzo | recogida | el mismo día, en el mostrador
17 de marzo | mensajería | 19 de marzo
```

Un `plus=` con rango dentro de una rama toma el paso de cada fila de la secuencia propia de esa
fila, así que una fila conserva su paso cuando cambia el reparto de la rama a su alrededor.
...
Un desplazamiento es una columna calculada a partir de otra, así que no es parte ni campo de
una `<sequence>` compuesta (`TDC295`): las partes de un registro se sortean juntas, y una fecha
calculada que acaba en otra fila ya no la describe. Declárelo como una secuencia propia.
...
> [!NOTE]
> **Son tokens, y un casi-token se rechaza**
>
...
De dónde salen las palabras: veinticuatro locales vienen integrados en el motor, y
cualquier otro cuyo pack de datos incluya un `DATE_LOCALE.json` — setenta lo hacen —
traduce sus fechas en cuanto el pack es visible. El georgiano, el letón, el kazajo y
los suyos imprimen ahora sus propios meses y días de la semana, incluida la forma del
mes dentro de una fecha (`15 იანვარს`); antes caían al inglés sin decir palabra. Un
archivo trae `L` y `LL`; `LLL` y `LLLL` se toman de él cuando los escribe y se derivan
si no — `LL` más la hora, el día de la semana delante. Un locale sin palabras de fecha
sigue imprimiendo inglés, y el validador lo dice en voz alta en lugar de dejarlo pasar
en silencio.
