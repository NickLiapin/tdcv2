...
[secuencias](../core-concepts/sequences.md#top) en una relación **padre → hijo**: los
...
- **drawn** — filas donde el padre eligió A — el hijo de abajo siempre es 1, 2 o 3
- **alt** — filas donde el padre eligió B — el hijo de abajo siempre es 7 u 8

## El problema: los campos independientes producen pares imposibles

Para ver por qué importa [`parent`](../reference/attributes.md#top), conviene mirar qué
pasa **sin** él. Dos secuencias [`text`](../generators/text.md#top), país y ciudad,
...
La columna `Country` es la misma (mismo [`seed`](../core-concepts/determinism.md#top)),
...
1. Se declara el **padre** con una distribución
   ([`text`](../generators/text.md#top) con [`percent`](../generators/text.md#top),
   [`template`](../generators/template.md#top) o cualquier otro tipo).
...
> en silencio: detiene la corrida con _filters on parent value "Kind.ALPHA", which the
> parent never produces_. Lo mismo vale para `mask=`.
>
...
- `Gender` con `count="8"` produce 5 `Hombre` + 3 `Mujer` (60/40 de ocho, redondeado con
  el método del resto mayor — vea [`percent`](../core-concepts/determinism.md#top)). Con
...
rusos se reduce a la mitad. Ponga el [`percent`](../generators/text.md#top) en el hijo y se
...
> [!CAUTION]
> **`missing=` en el padre no detiene al hijo**
>
>
> «Cualquier fila donde el padre tenga un valor» se lee como **cualquier fila que dejó
...
> [`missing=`](../guides/anomalies.md#top) sigue contando esas filas como suyas, y el hijo
> las llena:
>
> ```
> P=[]  C=[child]
...
Las [expresiones `if`](../reference/attributes.md#top) se evalúan contra los valores de la
...
- **[Secuencias](../core-concepts/sequences.md#top)** y
  [`parent`](../reference/attributes.md#top) — la mecánica detrás del filtro.
- **[Determinismo y proporciones](../core-concepts/determinism.md#top)** — cómo
  [`seed`](../core-concepts/determinism.md#top) y
  [`percent`](../core-concepts/determinism.md#top) se mantienen exactos.
- **[Datos coherentes y relacionales](coherent-data.md#top)** — padre → hijo mediante
  búsqueda por nombre.
