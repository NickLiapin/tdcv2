...
`uniq` cumple esa promesa reordenando los sorteos, y un reordenamiento necesita la columna
entera — no se puede hallar fila a fila. Por eso esta forma corre en el motor en memoria,
igual que un total acumulado; `<distinct>` se resuelve por fila y fluye sin problema.
