...
| :--- | :-------------------------- | :------------------------------ |
| `anomaly="p"` en un [`<gen>`](../generators/overview.md#top) que produce números | el valor base **× un factor** | `anomaly_flag="Name"` |
| `anomaly="true"` en un `<case>` dentro de un [`<mix>`](../reference/tags.md#top) | lo que sea que genere esa rama | `flag="Name"` |
...
números**; los valores no numéricos pasan intactos (vea [Detalles](#detalles)).
...
[`order="sequential"`](masks-and-case.md#top) (estrictamente en orden) y arme **dos**
...
secuencia, el mismo `seed`, el mismo `anomaly="0.25"`. Con el seed, el nombre y la tasa
...
[`<mix>`](../reference/tags.md#top), marque ese [`<case>`](../reference/tags.md#top) con
`anomaly="true"` y pídale al mix una columna de respuesta con
[`flag="Name"`](../reference/attributes.md#top). Aquí las temperaturas normalmente andan en
...
Tres de doce filas — exactamente el 25% que pidió [`percent`](../generators/text.md#top) —
...
- **Marcar un «evento raro».** La marca no tiene que significar *descompuesto*: puede
...
- **Solo números, y decide el valor — no el `type=`.** En los ejemplos de arriba la
  lista está escrita como `type="text"`, pero los valores *son* números, así que
...
- **[Generador number](../generators/number.md#top)** — rangos y distribuciones a los
  cuales enganchar `anomaly`.
- **[Panorama de generadores](../generators/overview.md#top)** — `missing=` y `anomaly=`
  lado a lado, ambos aplicados después de producir el valor.
- **[Referencia de etiquetas](../reference/tags.md#top)** — `<mix>` y `<case>`, la
  ramificación sobre la que se apoya la ruta del valor atípico a la medida.
- **[Salida y formato](../core-concepts/output-formatting.md#top)** — la expresión `if` que
