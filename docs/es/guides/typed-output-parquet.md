...
**Las columnas calculadas también reciben tipo**, por el mismo razonamiento: lo que pueden
producir se sabe antes de que exista una sola fila.

| generador | tipo de columna |
| :-------- | :-------------- |
| [`timeseries`](../generators/timeseries.md#top), [`pattern`](../generators/pattern.md#top) | entero, o decimal con `decimals` |
...
La decisión se toma sobre la página, no sobre el fragmento, así que un fragmento que se
comprime a casi nada puede terminar un par de bytes más grande de lo que entró — un caso
medido es 99 bytes de entrada y 101 de salida. Dos bytes en un archivo que de todos modos
es un tercio de su tamaño crudo: conviene saberlo, no conviene evitarlo.
