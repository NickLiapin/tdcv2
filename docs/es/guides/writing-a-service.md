...
decida los valores. Esta página es la otra mitad: **cómo escribir ese servicio**, en Node,
Python o Java, de modo que sea rápido, correcto y — la parte que todos fallan —
**reproducible**.
...
## El servicio, en cuatro lenguajes
...
Cualquiera de los cuatro, contra el mismo config:
...
```
77737493 -> 777374935   |  cuenta: 71102997
14850763 -> 148507635   |  cuenta: 54325378
87262332 -> 872623327   |  cuenta: 37547759
```

`Card` pasó por el manejador — la carga volvió con su dígito de control. `Acct` se inventó
solo a partir del seed. Cambie el puerto por 5702, 5703 o 5704 y la salida es carácter por
carácter la misma.
...
### La trampa: 32 bits en cuatro lenguajes

Para que los cuatro coincidan, tiene que coincidir la aritmética. Aquí es donde se rompe un
...
PRNG está escrito con `Math.imul` y operaciones de 32 bits justamente para que los
bindings de todos los lenguajes coincidan — así que la restricción no es un invento
de este ejemplo.

Las implementaciones de arriba se ejecutaron y se compararon:
...
Idénticas, no meramente parecidas. Si lo lleva a un quinto lenguaje, haga la misma
