...
- **A** — guardar cada fila en memoria: el costo crece con la corrida
- **B** — streaming: una ventana a la vez, así que el costo se mantiene plano por larga que sea la corrida
...
El modo disco tiene un tercer destino, y es el que conviene conocer: ocho formas de
...
configuración, y ocho formas rutean la corrida al **motor pequeño en memoria**, cuya
...
| [`weight=`](../generators/file.md#top) y [`row=`](../generators/file.md#top) en el mismo generador `file`                                                                                                | Ponderar un sorteo de fila enlazada hasta una cuota exacta necesita los totales del archivo por adelantado. **`weight=` por sí solo sí transmite** — lo que no puede es la pareja.                                                      |
...
| Un `percent=` dentro de una rama de [`<switch>`](../constructs/switch.md#una-proporción-dentro-de-una-rama) con varias claves — `is="US\|CA\|MX"` — o dentro de `<default>`                        | La proporción es una cuota sobre las filas propias de la rama, y esas filas son una unión de subconjuntos, o lo que dejaron las demás ramas. Ninguna de las dos se puede numerar fila a fila. |
| Una columna **derivada de otra columna** — un [total acumulado](../generators/running.md#top), un [estadístico](../generators/stat.md#top), una [fecha medida desde otra columna](../generators/date.md#top), una fórmula | Cada una lee una columna que tiene que existir antes, y el constructor de flujo la rechaza por nombre. |
| Un [`parent="Nombre"`](../guides/hierarchical-dependencies.md#top) pelado sin valor | Reduce la columna a las filas donde el padre produjo algo, y eso no se sabe sin la columna entera del padre. |
...
Una bajada de motor no es un error. Cada una de las ocho formas es una promesa sobre una
...
> ([`toArray`/`iterate`/`getAt`](../bindings/typescript.md#top)). Fuerce un motor específico con
> [`--engine 1|2|3`](../reference/cli.md#top); `--stream` es un alias heredado del motor rápido
> de streaming.
...
> ponderado — vea [Determinismo y proporciones](../core-concepts/determinism.md#top), la
...
**El motor rápido resuelve casi todo:** [`<sequence>`](../core-concepts/sequences.md#top)
...
Una secuencia hija con [`parent="Parent.Value"`](hierarchical-dependencies.md#top) está activa
...
Lo mismo vale para el [`<uniq>`](../constructs/unique-values.md#top) a nivel de env, donde la tupla única se
...
[`<mix>`](../reference/tags.md#top) elige un case para cada fila por **porcentaje exacto** (la
...
Ajústelo a mano con [`--jobs N`](../reference/cli.md#top) si quiere (`--jobs 1` fuerza un solo
...
[`--engine 1|2|3`](../reference/cli.md#top) (avanzado); `mode="memory"` es el motor pequeño en
...
[ocho formas de configuración que rutean una corrida a disco de vuelta a
...
- Antes de una corrida muy grande, revise la configuración contra [las ocho formas que la
...
- **[Valores únicos](../constructs/unique-values.md#top)** — `uniq`, `<uniq>` y `<distinct>` a fondo.
- **[Dependencias jerárquicas](hierarchical-dependencies.md#top)** — `parent` a fondo.
- **[Bindings de lenguajes](../bindings/typescript.md#top)** — la API de la biblioteca completa.
