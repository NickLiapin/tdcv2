...
> [!NOTE]
> **El envoltorio no lleva valores**
>
> Las fixtures son **texto literal**: un `${{Name}}` escrito en una no se expande, y un `<gen>` dentro
> de una se rechaza (`TDC131`). Así que los corchetes van en las fixtures y todo lo que varía va en el
> `<block>`. Un registro que necesita sus propios campos *junto a* una lista anidada es la forma a la
> que esto todavía no llega.
