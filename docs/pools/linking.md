...
- **The link is fixed per member, not per row.** Dr. Jones is at the East clinic on
  every row where he appears, because the clinic was decided when the _doctor_ was
  built.
...
Declaring the reference you filter on **above** the one that filters is the habit to keep
— it reads in the order it happens. It is not, however, a requirement the engine enforces:
swapping `Seen` and `Assisted` in this example produces identical output and passes
`check`, because the pool TABLES are built before any row is drawn, so the filter finds
what it needs whichever line it sits on.
