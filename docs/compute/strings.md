...
[`<compute>`](overview.md#top), next to [`<gen>`](../generators/overview.md#top) in a
[`<sequence>`](../core-concepts/sequences.md#top), and they read other sequences with
[`<field name="…"/>`](overview.md#top) — the same names you'd use in
[`${{…}}`](../core-concepts/output-formatting.md#top).

Most of them (`mask`, `case`, `slice`, `replace`, `trim`, `group`) are also available in
two other ways — as a [`<gen>`](../generators/overview.md#top) attribute
...
separator**. Unlike [`<add>`](arithmetic.md#top), this is a **string** operation:
...
[`<let>`](overview.md#top), then glue it on (the integer becomes text automatically).
...
`${{X | …}}` filters (see [Masks & case](../guides/masks-and-case.md#top)); the compute-tag
...
Formatting is available as a compute tag, a [`<gen>`](../generators/overview.md#top)
...
- **[Masks & case](../guides/masks-and-case.md#top)** — the same operations as `${{X|…}}`
  filters and `<gen>` attributes, with worked output.
- **[Compute overview](overview.md#top)** — how `<compute>`, `<let>`, and `<field>` fit
  together.
- **[Arithmetic](arithmetic.md#top)** — `<to_number>` / `<encode>` to go from text to
  numbers.
- **[Lists & iteration](lists.md#top)** — `<join>` to turn a list into a string before
  `<concat>`.
