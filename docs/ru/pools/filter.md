...
На ссылке на пул есть только `filter`. `if` [отвергается](../reference/errors.md#top),
...
ещё до прогона и без догадок, кодом [`TDC225`](../reference/errors.md#top). Граница проходит
...
- [Связывание пулов](linking.md#top) — `filter`, читающий поле _другой_ ссылки на пул: так
  строится цепочка
