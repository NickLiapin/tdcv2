...
Объявлять ссылку, по которой вы фильтруете, **выше** той, которая фильтрует, — хорошая
привычка: так читается в том порядке, в каком всё происходит. Но движок этого не требует:
если поменять `Seen` и `Assisted` местами, вывод будет тот же, а `check` промолчит —
ТАБЛИЦЫ пулов строятся до того, как вытянута первая строка, поэтому фильтр находит нужное
на любой строке.
