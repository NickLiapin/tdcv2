...
Дальше `base` читается как `<use name="base"/>` до конца этого `<compute>` — и только
...
Восемь цифр и девятая, склеенные. `<result>` — это конец `<compute>`, и его значение
печатается как `${{Routing}}`. Блок кончается один раз, поэтому рядом с `<result>`
не должно стоять ничего.
