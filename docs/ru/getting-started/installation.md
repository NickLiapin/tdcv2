...
>
> TDC ещё не выложен в npm. Как только это произойдёт, установка и запуск станут
> двумя строчками:
...
Пока пакет не опубликован, движок запускается напрямую из копии репозитория.
...
Запустите той командой, которую дала ваша установка. **В пакетные реестры пока
ничего не выложено**, поэтому сегодня любой путь — это клон репозитория; правая
колонка показывает, как будет, когда пакеты выйдут:

| Язык | Сегодня, из клона | Когда выйдут пакеты |
| :--- | :--- | :--- |
