...
| `filter=`               | из каких участников [пула](../pools/filter.md#top) эта строка может тянуть            |
| `expr=`                 | ЗНАЧЕНИЕ колонки [`formula`](../generators/formula.md#top)                            |
| параметр распределения  | значение `mean`, `sd`, `lambda` … [на этой строке](../guides/statistical-distributions.md#параметр-может-следовать-за-другой-колонкой) |

Первые два берут ответ как «да/нет» и выбрасывают значение; последние два его оставляют.
Те же операторы, те же функции, те же имена для тех же колонок — именно это не даёт
условию и вычисленной колонке начать понимать одни и те же слова по-разному.
...
которое все пять реализаций держат сами. РЕЗУЛЬТАТ арифметики за его пределом — отказ, теми
же словами, что у compute, а не тихое сползание обратно в плавающую точку:
...
Операторы — сравнения, логические связки и арифметика — точные: собраны из того, что IEEE-754 задаёт
однозначно, поэтому пять реализаций не могут о них разойтись. А ФУНКЦИИ из таблицы выше TDC
считает сам — потому у каждой и стоит граница точности, а не обещание точного ответа.
...
Пять функций из таблицы выше существуют ради одной задачи: построить величину, которая
МЕНЯЕТСЯ так, как меняются настоящие данные, ничего при этом не разыгрывая. Каждая — это
обычный расчёт над своими аргументами, поэтому одни и те же аргументы всегда дают один и
тот же ответ, в любой из пяти реализаций.
...
- **В узле решётки значение И ЕСТЬ `hash` в этой точке.** `noise(k * scale, scale, salt)`
  равен `hash(k, salt)` — теми же битами, а не «с точностью до ulp», — потому что
  сглаживание интерполирует как `a * (1 - u) + b * u`. Значит, граница ячейки непрерывна.
...
Это не плотность вероятности — она не нормирована к единичному интегралу. Это ФОРМА, на
которую умножают что-то другое.
...
>
> `order="sequential"` у `<gen>` идёт по значениям генератора в том порядке, в каком они
...
- Прогон держится в памяти (движок 1), поколоночные оптимизации выключены. Это честная
  цена колонки, читающей своё прошлое, и платит её только тот, кто попросил.
- `mode="sequential"` вместе с `engine="2"` или `engine="3"` — отказ, называющий **оба**
  атрибута. Те движки получают любую строку без предыдущей, это их устройство, и оба
  условия сразу выполнить нельзя.
- `prev()` внутри `if=` тоже отказ. `if=` — выбор, делаемый построчно, и движок,
  отвечающий на него, вправе брать строки в любом порядке.
