...
доверять каждому языку. Результат ложится примерно в 2 ulp от истинного значения — там же, где
...
**Остальная математическая библиотека.** `sinh`, `cosh`, `tanh`, `asin`, `acos`, `atan`,
`atan2` и `cbrt` отвергаются по имени, а не угадываются:
...
Каждую из них нужно построить и закрепить по битам в пяти языках, прежде чем предлагать, — и
это единственная причина, по которой их здесь пока нет.
