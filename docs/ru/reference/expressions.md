...
**Остальная математическая библиотека.** `asinh`, `acosh`, `atanh`, `log2`, `log1p`, `expm1`,
`hypot`, `sign`, `degrees` и `radians` отвергаются по имени, а не угадываются:
...
Обратите внимание, чего сообщение НЕ говорит: «может, вы имели в виду `atan`?» Расстояние
редактирования предложило бы именно это, а `atan` — обратная к совсем другой функции. Имя из
этого списка получает в ответ причину, а не догадку.

Каждую нужно построить и закрепить по битам в пяти языках, прежде чем предлагать, — и это
единственное, что держит её в списке.
