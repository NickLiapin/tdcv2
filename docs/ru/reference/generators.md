...
- `order=` / `cycle=` — порядок значений (по умолчанию случайный или `sequential`).
  Он что-то обходит, поэтому работает на [`text`](../generators/text.md#top),
  [`file`](../generators/file.md#top) и [`date`](../generators/date.md#top) — диапазон дат
  обходится по шагу. [`number`](../generators/number.md#top) берёт значение из диапазона,
  обходить ему нечего, и там атрибут отвергается ([TDC015](errors.md#top)).
