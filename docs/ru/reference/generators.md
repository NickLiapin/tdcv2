...
Работают почти на любом генераторе (см. [Маски и регистр](../guides/masks-and-case.md#top)):

- `case=` / `mask=` — регистр букв и маски отображения;
- `missing=` — оставить часть ячеек пустыми.

Четыре типа отвергают все три ([TDC015](errors.md#top)), и по одной причине:
[`formula`](../generators/formula.md#top), [`running`](../generators/running.md#top) и
[`stat`](../generators/stat.md#top) вычисляются в порядке объявления, до слоя форматирования,
а [`pool`](../pools/overview.md#top) отдаёт строке целую запись, а не собственное значение.
Отвергают, а не игнорируют, потому что ответ есть шагом позже и он лучше: фильтр
подстановки работает там, где значение ПЕЧАТАЕТСЯ, так что `${{Weight|mask:x}}` и
`${{Doctor.name|upper}}` работают уже сейчас.
