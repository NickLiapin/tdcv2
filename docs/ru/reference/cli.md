...
| Опция                   | Что делает                                                                                                                                                                                                                                                                                                                                                                            |
| :---------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `-o, --output <path>`   | Пишет результат в файл. Без этой опции выводит в stdout. Путь, заканчивающийся на `.parquet`, включает писателя [Parquet](../guides/typed-output-parquet.md#top) — другого способа получить его нет. Файл появляется только после окончания прогона: он пишется как `<path>.partial` и переименовывается, поэтому неудачный прогон оставляет прежний файл нетронутым и нового не создаёт |
...
Поэтому в файле нет ЧИСЛА фаз. «Фаза 2 из 4», объявленная в начале, была бы числом, до
которого этот прогон может и не дойти, а полоса, построенная на нём, прыгала бы — чего
полосе делать нельзя. Рисуйте фазу и её собственные числа: они верны всегда.
