...
команда говорит об этом, а не принимает флаг и молча его теряет. Счётчики приходят от того
процесса, который рисует строки, поэтому прогон, разбитый на несколько воркеров, сообщает
фазы и завершающее `done`, а не число строк.
