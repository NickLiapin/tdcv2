...
>
> `npm install -D tdcv2` даёт команду `tdcv2` через `npx`. Остальные четыре в свои реестры
> пока не выложены, поэтому их ставят из репозитория — команда своя для каждого языка, и
> все они собраны на странице [Установка](../getting-started/installation.md#top). Java —
> единственная, где голой команды `tdcv2` не будет никогда (в Maven нет аналога npm-ного
> `bin`); алиас делает все команды на этой странице одинаковыми. Всё, что ниже, работает
