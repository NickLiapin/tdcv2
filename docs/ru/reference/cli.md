...
- **`--engine 1|2|3`** — форсировать конкретный движок; `--stream` — легаси-алиас
  для `--engine 2`. Названный движок **отказывается**, а не подменяется тихо другим, — поэтому
  замер меряет то, что называет. `--mode` описывает стоимость, и такой прогон по-прежнему может
  уйти на другой движок.
