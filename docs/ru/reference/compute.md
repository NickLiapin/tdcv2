...
| [`<use>`](../compute/overview.md#let-даёт-значению-имя-один-раз-use-читает-его-обратно) | `name=` → `int\|str\|list` | Значение, связанное `<let>`                                    |
| [`<let>`](../compute/overview.md#let-даёт-значению-имя-один-раз-use-читает-его-обратно) | `name=` + 1 → `—`          | Назвать промежуточный результат для соседних тегов             |
