...
После того как данные объявлены в виде [последовательностей](sequences.md#top),
...
- **A** — печатается один раз, в самом начале
- **B** — перед каждой карточкой
- **C** — строка карточки
- **D** — между строками одной карточки — и никогда после последней
- **E** — после каждой карточки
- **F** — между карточками — и никогда после последней
- **G** — печатается один раз, в самом конце
- **H** — всё, что внутри скобки, повторяется на каждой карточке
...
`<block>` — единственный **обязательный** ребёнок [`<tdc>`](../reference/tags.md#top).
...
[`<gen>`](../generators/overview.md#top) или тег с опечаткой, прежде чем он испортит вывод:
...
Если в [`<env>`](configuration.md#top) объявлены
...
`<line>` — объявите именованную [`<sequence>`](sequences.md#top) (или
[`<mix>`](../reference/tags.md#top)) в [`<env>`](configuration.md#top) и сошлитесь на неё через
`${{Name}}`. Так _что за данные_ отделяется от _как они разложены_.

Связанное ограничение: генератор [`advanced_regex`](../generators/advanced-regex.md#top),
...
- **В разметке `<data>` стоит внутри `<line>`** — строк блока или строк фикстуры — и
  ...
  [`<sequence>`](sequences.md#top) он даёт литеральный текст, вклеенный в составное
...
только то, что стоит _между_ `<data>` и `</data>`. В схеме ниже два пробела перед `id`
...
2. **Объявленная последовательность без значения на этой строке** (отфильтрована
   [`parent`](../guides/hierarchical-dependencies.md#top) на этой итерации) → **пустая
   строка**.
...
[Встроенные имена](../reference/builtins.md#top).
...
Фикстуры — единственное место, где её обычно не ждут, а она туда доходит: см.
[Фикстуры](#фикстуры--текст-вокруг-карточек) ниже.
...
Все фикстуры живут внутри [`<env>`](configuration.md#top) и, как и блок, содержат `<line>` →
...
- **Интерполяция в фикстурах РАБОТАЕТ.** `${{Name}}` читает ту строку, рядом с которой
  стоит фикстура: `<before>` видит первую, `<after>` — последнюю, а покарточные и
  построчные — ту, которую обрамляют. Именно это позволяет записи обернуть вложенный
  список собственными полями. (Это одно из немногих мест, где пришлось исправлять
  эталонную реализацию: она печатала скобки буквально, а все четыре порта раскрывали.)
...
[`inject`](../reference/attributes.md#top) на [`<env>`](configuration.md#top) меняет маркеры
интерполяции. На место `%` встаёт имя, и вокруг него нужны открывающая и закрывающая
части — TDC режет шаблон по **самому правому** `%`, при котором обе стороны непусты,
что и позволяет самому `%` встречаться в обёртке:

```xml
<env inject="${{%}}">…</env>   <!-- по умолчанию -->
<env inject="[%]">…</env>       <!-- тогда в <data> пишем [Name] -->
<env inject="%{%}%">…</env>     <!-- режется по среднему %: пишем %{Name}% -->
```
...
документации — дайте тегу маркер [`pair`](../reference/attributes.md#top); один и тот же маркер
должен стоять и на открывающем, и на закрывающем теге. Здесь и `${{...}}` должен попасть в
вывод буквально, поэтому пример заодно уводит [маркер подстановки](configuration.md#top)
...
- **[Маски и регистр](../guides/masks-and-case.md#top)** — полный набор фильтров и масок.
- **[Детерминизм и пропорции](determinism.md#top)** — `seed`, `count` и `percent`.
- **[Иерархические зависимости](../guides/hierarchical-dependencies.md#top)** — как `parent`
  фильтрует последовательность, давая случай пустой строки выше.
- [Встроенные имена](../reference/builtins.md#top) — `_count`, `_first`, `_last`, `_total`.
- [Справочник тегов](../reference/tags.md#top) — `<block>`, `<line>`, `<data>` и фикстуры
  вкратце.
