...
| `seed`    | случайный    | Сид генератора случайных чисел — прогон без него печатает выбранный, в **stderr** |
...
> [!TIP]
> **Прогон без сида всё равно говорит, как его повторить**
>
> Не указали `seed` — TDC выберет сам и напишет его в **stderr** перед данными:
>
> ```
...
> ```
>
> Данные идут в stdout, поэтому `tdcv2 people.tdc > people.csv` оставит заметку на экране, а
> `2>/dev/null` выбросит единственную запись о том, как получить этот файл снова.
...
([`TDC021`](../reference/errors.md#top)): там подходят два, движок прочитал бы правый, а левый
...
[выводе и форматировании](output-formatting.md#фикстуры--текст-вокруг-карточек).
