...
ваши маркеры не сталкивались с маркерами цели. У `%` должен быть текст с обеих сторон, а если
их в строке несколько, местом подстановки становится **самый правый, у которого он есть**: в
`inject="%{%}%"` имя встаёт между `%{` и `}%`, а крайние два — литеральные.
