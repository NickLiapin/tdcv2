...
[English](../../core-concepts/determinism.md#top) · **Русский** · [Español](../../es/core-concepts/determinism.md#top)
...
[часы](#часы--пятый-вход). Эта страница
...
- **A** — первый прогон с одним сидом
- **B** — второй прогон с тем же сидом — совпадает значение в значение
- **C** — другой сид: данные той же природы, но ни одного совпадающего числа
...
> дисковый — для уникальности, маленький оперативный — под `mode="memory"` и на тех формах,
> которым нужна вся колонка сразу. Объектное API к таким формам не относится: `toArray`,
> `iterate` и `getAt` идут на том же движке, что выбрал маршрутизатор, как и текстовый вывод.
> **Все три дают одни и те же значения при одном сиде.** Значение строки выводится из
...
> всех трёх. Как выбирается движок, см. в [Больших выводах](../guides/large-outputs.md#top) —
> это про скорость и память, а не про то, какими будут ваши данные.

`seed` задаётся на [`<env>`](configuration.md#top). Значение — любая строка: хеш, слово,
...
И имя (из [`template`](../generators/template.md#top)), и код (из
[`number`](../generators/number.md#top)) выглядят случайно. Запустите документ
...
Имена совпали — имя приходит от одного лишь сида. Даты рождения сдвинулись, потому что
возрастное окно отсчитывается назад от сегодняшнего дня.
...
про флаг — на [`--now`](../reference/cli.md#--now--зафиксировать-часы).

## `count` — сколько записей

`count` — это сколько раз рендерится блок. Он задаётся на
[`<env>`](configuration.md#top), по умолчанию равен **10** и переопределяется опцией
...
генераторов — [`number`](../generators/number.md#top), невзвешенный
[`template`](../generators/template.md#top),
[`counter`](../generators/counters.md#top), [`regex`](../generators/regex.md#top) —
...
**пересчитываются** — их колонки уже не префикс. Так работают пять возможностей:
точные доли (`percent` на [`text`](../generators/text.md#top) и на `<mix>`, метод
Хэмилтона), **простой список `text` вообще без `percent`** — ровная квота всё равно
квота, поэтому `value="A,B,C"` на 4 и на 8 строках общего префикса не имеют, —
уникальность ([`uniq`](../constructs/unique-values.md#top)), **взвешенный**
пакет [`template`](../generators/template.md#top) и **длины списков** у
[`repeat="min..max"`](../constructs/multiple-values.md#top) — диапазон там задаёт квоту на
весь прогон, а не кубик на строку, поэтому 200 строк с `repeat="1..4"` выходят
50 / 50 / 50 / 50, а на 201 строке одна из четырёх длин получает лишнюю строку — какая
именно, решает seed: при `seed="s"` измерено 50 / 50 / 51 / 50, при `seed="beta"` —
51 / 50 / 50 / 50. Пакеты имён и мест несут частоты по
...
`repeat`. И простой список `text` тоже: то же `value="A,B,C"` даёт `C A B C` на четырёх
строках и `B B A B C C C A` на восьми, и ни одно не префикс другого.
...
конфиге есть хоть одна из пяти возможностей выше, не ждите, что третья строка
...
Добавьте `percent` к генератору [`text`](../generators/text.md#top) (или к `<mix>`) — и
...
[`value`](../generators/text.md#top) разворачивается: заполненные позиции фиксируют
...
Когда у последовательности есть [`parent`](sequences.md#top), проценты считаются
...
- **[Text](../generators/text.md#top)** — `percent` целиком, включая короткие маски.
- **[Иерархические зависимости](../guides/hierarchical-dependencies.md#top)** — пропорции внутри подмножества.
- **[Уникальные значения](../constructs/unique-values.md#top)** — другая раскладка «на весь набор», пересчитываемая с `count`.
- **[Большие объёмы вывода](../guides/large-outputs.md#top)** — как точные пропорции работают при потоковой генерации.
