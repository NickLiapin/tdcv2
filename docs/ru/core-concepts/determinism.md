...
же данные байт в байт, а доли ложатся в **точные** пропорции. Эта страница разбирает
сразу три атрибута — `seed`, `count` и `percent`, — потому что они работают вместе:
`count` решает, **сколько** записей вы получите, `seed` — **какие именно**, а
`percent` фиксирует их пропорции.

> [!NOTE]
> Примеры вывода ниже иллюстративны — конкретные имена и числа могут отличаться от
> версии к версии ядра, но их *свойства* (воспроизводимость, префиксы, точные
...
- **A** — первый прогон с одним seed
- **B** — второй прогон с тем же seed — совпадает значение в значение
- **C** — другой seed: данные той же природы, но ни одного совпадающего числа
...
> [!NOTE]
> **Тот же вывод — на том же движке**
>
> Гарантия держится в пределах одного движка. TDC выбирает движок по конфигу (по
> умолчанию быстрый потоковый; маленький оперативный — под `mode="memory"` или за
> объектным API `toArray`/`getAt`), и движки тянут значения в разном порядке — поэтому
> **числа и имена** для одного сида могут различаться, хотя **форма** — число строк,
> точные доли, уникальность — одинакова. Текстовый вывод (`toString`, CLI, `writeFile`) —
> это один движок; объектные методы — другой, и их случайные значения совпадать не
> обязаны. Как выбирается движок, см. в [Больших выводах](../guides/large-outputs.md#top).
> Что не меняется никогда — данный движок при данном сиде.
...
Хэмилтона), уникальность ([`uniq`](../constructs/unique-values.md#top)), **взвешенный**
