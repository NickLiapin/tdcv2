...
Одно имя папки — исключение: `countries/` сегментом никогда не становится. Файл
`countries/usa/docs/ssn.txt` адресуется как `usa.docs.ssn`: страновые пакеты адресуются
по стране, а папка существует только чтобы в хранилище был порядок.
...
должен быть кодом локали, названием страны, `common` или `user`. Используйте это, когда адрес и раскладка на
...
Свободный текст с описанием пакета. На вывод не влияет — он для людей и для
автодополнения в редакторе, которое его читает (см.
[Поддержка редактора](../getting-started/editor-support.md#top)). Держите его коротким:
«города России», «HTTP-коды статусов».
...
#### Региональный вариант наследует базовый язык

`en-gb`, `pt-br` и `de-at` — такие же локали, как остальные, и почти все они не везут
ничего своего. Прогон, который называет такую локаль, получает данные базового языка:
`local="en-gb"` берёт из `en`, `local="pt-br"` — из `pt`. Даты идут тем же шагом, поэтому
`de-at` печатает `März`, а не `March`.

Именно поэтому пакет-вариант дёшево писать — он везёт только то, что действительно
отличается. Положите один `en-gb/geo/city.txt`, и британские города победят, а все
остальные адреса по-прежнему придут из `en`:

```text
packs/
  en-gb/
    geo/
      city.txt      ← отличается только это
```

Шаг делается **один раз** и никогда не доходит до английского. Именно из-за этого
одного шага традиционный китайский лежит в `zh`, а не в `zh-tw`: Тайвань, Гонконг и
Макао пишут одним и тем же письмом, все три имени — `zh-tw`, `zh-hk` и `zh-mo` — за один
шаг приходят в `zh`, и ни одно из них не пришло бы в пакет, названный именем другого.
У `zh-cn` есть собственный полный пакет, и он никуда не откатывается, так что `zh` —
ровно та ячейка, которая общая у традиционных локалей. Таблицы дат делятся так же:
`zh-cn` пишет короткий день недели как `周日`, традиционные — как `週日`.
...
Автодополнение адресов в редакторе работает по этим же шапкам `description:` и уже
поставляется — см. [Поддержку в редакторе](../getting-started/editor-support.md#top).

## Как описать папку

Папка пакетов может сказать, кто её написал, под какой лицензией и какой версии —
для этого в ней лежит `_pack.json`, см.
[Свой пакет](writing-your-own.md#как-описать-папку--_packjson). Ничто из него не
попадает в сгенерированные данные; читает его `tdcv2 pack info`.
