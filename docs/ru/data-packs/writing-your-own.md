...
На 100 строк ровно 60 строк несут две фамилии и 40 — одну: тело генератора исполняется
тем же движком, а он знает `count` и раскладывает проценты методом Гамильтона, а не
случайно.
...
> [!NOTE]
> **Что разрешено внутри генератора (сейчас)**
>
> Примитивы — [`regex`](../generators/regex.md#top), [`number`](../generators/number.md#top),
> [`symbol`](../generators/symbol.md#top), [`date`](../generators/date.md#top),
> [`text`](../generators/text.md#top) — распределение
> [`<mix>`](../reference/tags.md#распределения-и-выбор) / `percent` и ссылки на
> **списки-данные** и на **другие генераторы** по адресу. Сложные корреляции между полями —
> это дело конфига, а не генератора в пакете.
