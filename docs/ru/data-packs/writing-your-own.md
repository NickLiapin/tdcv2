...
[`<data>`](../core-concepts/output-formatting.md#top) для текста между ними.

А ещё `<data>` внутри кейса может **прочитать строку, на которой стоит**: `${{Name}}`
разрешается там так же, как в выходной строке, вместе с фильтрами.
...
Это не то же самое, что `<gen type="template">` рядом: генератор тянет НОВОЕ значение, а
ссылка сохраняет запись согласованной с тем, что в строке уже есть. Имя, которого никто не
объявлял, отклоняется (`TDC193`), а не печатается буквально.
...
Автодополнение адресов в редакторе работает по тем же заголовкам `description:` — оно
отгружено, см. [Поддержка редактора](../getting-started/editor-support.md#top). Напишите
описание, которое стоит читать: именно его увидит рядом с адресом тот, кто его дополняет.

## Как описать папку — `_pack.json`

В папке пакета есть содержимое и нет происхождения: списки и генераторы говорят,
что они выдают, и ничего — откуда они взялись. Пока пакеты только встроенные, это
неважно, и становится важно ровно в тот момент, когда вам передали папку и
спрашивают, можно ли отгружать данные, собранные из неё.

Положите `_pack.json` в корень папки — рядом с `_locale.json`, если он там есть:
...
Все поля необязательные, и **ничто отсюда не попадает в сгенерированные данные**.
Манифест не может изменить ни одного значения: тот же сид даёт те же байты,
независимо от того, есть манифест или нет.

Прочитать обратно — `tdcv2 pack info`:
...
В каждой настроенной папке данных и **в каждой папке на уровень внутри неё**. Этим
покрываются обе формы, в которых манифест бывает нужен: «вся эта папка — мой
пакет» и «вот эта локаль внутри пришла откуда-то отдельно».

Глубже — намеренно нет. Иначе напрашивался бы манифест на каждый `.txt`, а вопрос,
на который это отвечает, — кто написал эти данные и под какой лицензией — у
отдельного списка названий городов своего ответа не имеет.

### Что будет, если он сломан

`tdcv2 pack info` **отклонит** манифест, который не разбирается, и такой, у
которого `name`, `version`, `license`, `author`, `homepage` или `description` — не
текст. Читать этот файл — вся работа этой команды, а лицензия, которую автор
написал, а прочитать нельзя, хуже, чем ненаписанная.

**Прогон о нём вообще не упоминает.** Манифест не дотягивается ни до одного
значения, поэтому останавливать генерацию из-за него значило бы блокировать работу
из-за поля, которое эта работа никогда не читает.

Незнакомые ключи молча сохраняются — папка, написанная для более новой версии или
несущая поля вашего собственного инструмента, здесь по-прежнему работает.
