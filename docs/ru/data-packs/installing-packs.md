...
> | Java     | скачать `tdcv2-0.3.2-cli.jar` с Maven Central     | `java -jar tdcv2-0.3.2-cli.jar pack add ru` |
>
...
> ```
>
> Стоит завести алиас — `alias tdcv2='java -jar /путь/к/tdcv2-0.3.2-cli.jar'` — после этого
...
Пишет конфиг в пользовательскую папку конфигурации, а не в текущую:

| Платформа                    | Куда попадёт                                            |
| :--------------------------- | :------------------------------------------------------ |
| Windows                      | `%APPDATA%\tdcv2\config.json` (или `~/AppData/Roaming/…`, если `%APPDATA%` не задан) |
| POSIX, задан `XDG_CONFIG_HOME` | `$XDG_CONFIG_HOME/tdcv2/config.json`                   |
| POSIX, по умолчанию          | `~/.config/tdcv2/config.json`                           |
 Пригодится, когда
...
Прогон ищет свой конфиг обходом **вверх от папки самого файла `.tdc`** (так же, как
инструменты находят `tsconfig.json`) — поэтому `tdcv2 sub/users.tdc` подхватит проектный
конфиг из `sub/`, а не из папки, где стоит оболочка. У `tdcv2 pack` и `tdcv2 init` файла
`.tdc` нет, поэтому эти две команды идут вверх от текущей папки. Когда одно и то же определено в нескольких источниках, приоритет идёт от
низшего к высшему:
...
каталог. 294 наборов на экран не помещаются, поэтому их листают так, как они устроены:
...
В каталоге сегодня **294 наборов**: `common`, 95 языков и 198 стран. Языка или страны,
...
Ещё две переменные влияют на команду `pack`, а не на поиск:

- **`TDCV2_NO_PICKER`** — если задана хоть как-нибудь, `tdcv2 pack` без подкоманды печатает
  список вместо полноэкранного выбиральщика. Это единственный способ получить печатный
  список в терминале; в скрипте или в конвейере он и так печатается.
- **`TDCV2_ASCII`** — если задана хоть как-нибудь, выбиральщик рисует ASCII-глифами и по
  одной строке карты на строку вывода. Это ручное переопределение, когда автоопределение
  Unicode ошиблось; `NO_COLOR` — отдельная переменная и выключает только цвет.
