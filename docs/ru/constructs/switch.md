...
Ветка может и вычислять из своей строки: [`formula`](../generators/formula.md#в-ветке) или
[смещение даты](../generators/date.md#в-ветке) читают колонки этой строки и ничего больше.
Конструкции, которые читают весь прогон, — [`running`](../generators/running.md#top),
[`stat`](../generators/stat.md#top), формула с `prev()` — и
[ссылка на пул](../pools/overview.md#top) должны быть отдельными последовательностями
(`TDC295`, `TDC268`).
...
> [!CAUTION]
> **Знаменатель мал, поэтому малая доля может округлиться в ничто**
>
> В примере выше стоит `20` не случайно. Мужских записей всего пять, поэтому `20 %`
> просит ровно одну запись. Напишите вместо этого `10 %` — и ветка попросит
> **половину** записи, которую выдать нельзя. Тогда `prostatitis` появится один раз
> либо не появится вовсе, решит это seed, а прогон будет выглядеть так, будто
> конфиг вообще не сработал.
>
> Умножайте долю на число строк, которые совпадёт эта ветка. Меньше 1 — и колонка
> становится броском монеты — и это умножение `check` делает за вас, предупреждая
> кодом [`TDC251`](../reference/errors.md#top) ещё до прогона. Разобранный пример:
> [Доля меньше одной записи](./mix.md#доля-меньше-одной-записи).
...
записи. Доля внутри ветки стоит вам потокового движка в двух случаях, см.
...
Потокового движка лишают две формы, и обе — про **долю** внутри ветки:

- ветка совпадает более чем с одним ключом (`is="US|CA|MX"`) либо это `<default>`;
- либо **предмет** `<switch>` — не простой список `value=`, а пакет, файл, колонка
  с `order="sequential"` или с `repeat=`. Тогда даже ветка на один ключ теряет поток:
  движок не может перечислить значения предмета заранее, до строк.

В обоих случаях это подмножество, которое потоковый движок не может пронумеровать по
одной строке за раз, а точной доле нужна именно нумерация. TDC сам направляет такой
конфиг на движок в памяти: вы получаете точную долю и плату за то, что прогон держится
в памяти. Ветка на **один** ключ над простым списком `value=` работает потоково как
обычно.
