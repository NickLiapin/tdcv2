...
Всё, кроме `count`, — арифметика, поэтому ей нужны числа. Колонка, в которой стоит слово,
отвергается с указанием колонки и первой ячейки, которая не число, — вместо `NaN` в каждой
строке. Число здесь — то, что читает как число нарастающий итог: цифры, необязательный знак и
необязательная дробная часть. `count` считает любые ячейки.
...
Статистика — **целая колонка**: она читает все строки прогона, — поэтому она должна быть
отдельной `<sequence>`. Внутри `<case>`, как одна из веток `<gen if="…">`, как часть или поле
составной последовательности она отвергается кодом `TDC295`. Посчитайте статистику отдельной
последовательностью выше и читайте её по имени там, где она нужна.
...
>
> Статистика нужна, когда строке надо знать про КАЖДУЮ строку. Когда ей достаточно знать
> о себе самой, [`<gen type="formula">`](formula.md#top) делает это и **стримится**, так что
> описанная выше граница по целому прогону к нему не относится.
