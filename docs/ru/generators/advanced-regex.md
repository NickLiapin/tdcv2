...
`\k<name>` повторяет группу по этому имени — так же, как в [`regex`](regex.md#top), — и
...
`uniq="true"` даёт каждой строке своё значение, а точные доли остаются точными:
...
Шестьдесят процентов номеров начинаются с `77` и сорок — с `78`, ровно как без `uniq`, и двух
одинаковых номеров нет. Сначала колонка раскладывается по долям; номер, который повторился, тянется
заново **по тем же веткам**, так что строка с `77` остаётся строкой с `77`, и ни одна доля не
сдвигается ни на строку.

Поэтому вопрос «хватит ли места» задаётся каждой доле, а не только образцу. Каждая доля
считается по своим веткам: `(?%{70:RU;30:US})-[0-9]{3}` даёт 2 000 строк, и на 2 000 строк доле
RU нужно 1 400 из той тысячи, что начинается с `RU-`. Прогон откажет раньше, чем что-то вытянет
заново, и назовёт долю по её проценту и оба числа. Доля не может занять у другой и остаться
точной.

Условие, которое читает группу с долями, по-прежнему с ней согласно: группа разложена заранее, и
ветка, которая её читает, после перетяжки та же. Если выбор с долями стоит внутри повтора, `|` или
условия, ветки строки зависят и от её свободных выборов, и перетяжка, которая забрела на другие
ветки, отбрасывается, а не сдвигает долю. Такую долю не сосчитать отдельно, поэтому за неё отвечает
общий счёт образца, и нехватка в ней выясняется после долгой череды повторов, а не заранее.
