...
- **Движок.** Конфиг с формулой идёт на [движок в памяти](../guides/large-outputs.md#top):
  потоковый сборщик пока не умеет читать соседнюю колонку — то же ограничение, что и у
  [смещения даты](date.md#top). Маршрутизатор делает это сам, настраивать нечего.
