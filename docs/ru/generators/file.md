...
[`weight=`](#weighted-rows--row--weight) с его точной квотой. Квантильное чтение —
...
| :-- | :-- | :-- |
| считаемое (город, статус, число заказов) | `value,count` | [`weight="count"`](#weighted-rows--row--weight) — точная квота |
