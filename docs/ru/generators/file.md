...
(принимаются также `"1"` и `"0"`), чтобы пропустить строку-заголовок:
...
выбирается одна строка, а разные значения `column` читают из неё разные ячейки. Ключ над
двумя разными файлами `check` отклоняет: связка — это строка ОДНОГО файла, поэтому строки,
принадлежащей обоим, не существует.
...
выборка в копейках — копейки. [`decimals`](../reference/attributes.md#top) это
переопределяет.
...
сида, которой пользуется [`uniq`](../constructs/unique-values.md#top), — и прогон
...
получить [`TDC297`](../reference/errors.md#top).
