...
| `<страна>.geo.province`       | страновой пакет  | Провинция, штат, регион или департамент        |
| `<страна>.geo.postalCode`     | страновой пакет  | Почтовый индекс в формате этой страны          |
| `<страна>.geo.streetName`     | страновой пакет  | Название улицы                                 |

`geo.city` есть в **92 страновых пакетах из 111**. Набор географических имён у стран разный,
потому что разные сами страны: у Польши есть `voivodeship`, у Италии — `comune`, у США —
`state` и `zip`. Что несёт конкретная страна, смотрите в
