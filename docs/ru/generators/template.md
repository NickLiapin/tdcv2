...
Какая-нибудь административная единица есть в 95 пакетах из 111, какой-нибудь почтовый
индекс — в 48, под именами вроде `department`, `canton`, `governorate`, `voivodeship`,
`prefecture`, `eircode`, `cep`, `cap`. Поэтому `usa.geo.province` — не адрес: у США есть
