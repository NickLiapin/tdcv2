...
- **`Content-Type: text/plain`**, ровно так, без параметра charset. Все пять реализаций шлют
  одну и ту же строку, поэтому сервис может сверять её буквально.
...
| `secret="k7Fm2p…"`                  | само значение — работает и предупреждает ([`TDC284`](../reference/errors.md#top)) |
...
- **Из библиотеки — только асинхронный путь.** Сетевой вызов не может выйти из синхронной
  функции, поэтому `toString()` и `writeFile()` на `http`-конфиге **бросают исключение**.
  Берите [`toStringAsync()` / `writeFileAsync()`](../bindings/typescript.md#терминальные-методы)
  — CLI так и делает. Если вы запускаете из командной строки, делать ничего не нужно.
