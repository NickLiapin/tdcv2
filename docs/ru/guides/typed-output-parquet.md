...
| :-------- | :---------- |
| [`timeseries`](../generators/timeseries.md#top), [`pattern`](../generators/pattern.md#top) | целое, с `decimals` — дробное |
| [`running`](../generators/running.md#top) | тип той колонки, которую называет `of=` |
...
[Квантильное чтение](../generators/file.md#top) — единственная строка таблицы, где генератор
...
Решение принимается по странице, а не по куску, поэтому кусок, сжимающийся почти в ничто,
может выйти на пару байт больше, чем вошёл, — намеренный случай: 99 байт на входе, 101 на
выходе. Два байта на файле, который и так втрое меньше исходного: знать полезно, избегать
незачем.
...
Читатель получает настоящий словарь, а не строку, которую надо разбирать самому:
...
- **Ключ — всё до первого `:`, значение — остальное.** Значит, в значении двоеточия
  допустимы (`start:12:30:00` — одна пара), а в ключе нет.
...
- **Три вещи отклоняются**, а не угадываются, потому что каждая оставила бы вас со
  словарём, где тихо не хватает записи: кусок вообще без `:`, пустой ключ и ключ,
  который повторяется внутри одной строки (читатели расходятся в том, какой из двух
  побеждает, а некоторые выбрасывают словарь всей строки).
...
- **Списки списков** — `[]int64` и `{}int64` работают, а `[][]int64` нет: понадобился бы
  второй разделитель, и ни одна его форма пока не заслужила своего места.
...
- **[Языковые привязки](../bindings/python.md#top)** — чтение и запись из всех пяти:
