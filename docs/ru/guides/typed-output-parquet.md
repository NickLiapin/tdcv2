...
| [`file` с `read="quantile"`](../generators/file.md#top) | **текст** — напишите `type="float"`, если нужна числовая колонка |
...
[Квантильное чтение](../generators/file.md#top) — единственная строка этой таблицы, где
значение заведомо ЧИСЛО (файл обязан быть числовым, иначе прогон откажется), и колонка всё
равно пишется текстом. Это вывод типа, идущий по своему правилу, а не по значению: движок
знает, что генератор `file` выдаёт то, что лежит в файле, — и это факт о генераторе, а не
об этом его применении. Напишите `type="float"` (или `type="int64"` для целочисленной
выборки), и колонка станет типизированной:
