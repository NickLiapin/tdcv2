...
Ограничена только составная форма, а таблица обещала это всем трём. Измерено на счётчике,
в один поток, пиковая резидентная память:

```
200 тыс → 168 МБ      4 млн → 822 МБ      20 млн → 3,0 ГБ
  1 млн → 308 МБ     10 млн → 1,9 ГБ
```

По этой строке люди планируют железо, поэтому стоит быть точными: ~1 ГБ — это порция
внешней сортировки, и в неё укладывается только составная форма.
