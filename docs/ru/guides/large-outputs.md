...
- **Точный движок на диске** — для обещания про **готовый** столбец, а не про текущую
  ...
  последовательность. Результат он гарантирует точно, и платит за это проверкой данных
  через внешнюю сортировку и проход починки — проверкой, которая **резко замедляется с
  ростом числа строк** (см. предупреждение ниже).

  Ограниченная память — обещание более узкое. Замерено, в один поток, на 3,2 млн строк:
  составной `uniq` держит ~1 ГБ и перестаёт расти — это размер куска внешней сортировки.
  Три другие формы откатываются на движок в памяти, когда потоковый сборщик их не берёт,
  и память там растёт вместе с числом строк, как у движка 1: env-уровневая группа
  `<uniq>` дошла до 1,9 ГБ на 800 тыс. строк, и ничто об этом не предупредило.
...
| `uniq="true"` на [счётчике](../generators/counters.md#top)                             | точный на диске | ограничена ~1 ГБ        |
| `uniq="true"` на составной последовательности (именованные поля `<gen>`)            | точный на диске | ограничена ~1 ГБ        |
| env-уровневая группа [`<uniq>`](../constructs/unique-values.md#top)                    | точный на диске | ограничена ~1 ГБ        |
