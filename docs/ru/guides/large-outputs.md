...
прогон поместится. А сколько повторов даст конкретная конфигурация, по формуле «парадокса
дней рождения» не угадать: на прогоне в 6 000 000 строк сканирование отпечатков передало
проверке 99 852 группы-кандидата там, где формула обещает около 11 000. Запустите и смотрите
`--progress`.
