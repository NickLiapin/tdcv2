...
- **Точный движок на диске** — для обещания про **готовый** столбец, а не про текущую
  ...
  счётчике и [`parent`](hierarchical-dependencies.md#top), у которого родитель — не текстовая
  последовательность. Результат он гарантирует точно, и память остаётся ограниченной — но
  платит за это проверкой данных через внешнюю сортировку и проход починки, и **эта
  проверка резко замедляется с ростом числа строк** (см. предупреждение ниже).
...
У дискового режима есть и третье назначение, и как раз о нём стоит знать: шесть форм
...
конфиг](#какой-движок-запустит-ваш-конфиг) перечисляет все пять.
...
конфиг, и шесть форм уводят прогон на **маленький in-memory движок**, память которого
...
| [`weight=`](../generators/file.md#top) и [`row=`](../generators/file.md#top) на одном генераторе `file`                                                            | Чтобы взвесить связанный выбор строки до точной квоты, нужны итоги по всему файлу заранее.                                                     |
...
| `uniq="true"` на [счётчике](../generators/counters.md#top)                             | точный на диске | ограниченная            |
| `uniq="true"` на составной последовательности (именованные поля `<gen>`)            | точный на диске | ограниченная            |
| env-уровневая группа [`<uniq>`](../constructs/unique-values.md#top)                    | точный на диске | ограниченная            |
...
Такой откат — не баг. Каждая из шести форм — это обещание про целый столбец, а движок,
...
Есть ещё один маршрут — для того, чего список не может увидеть заранее. Если потоковый
движок всё-таки отказывается от конфига — [нарастающий
итог](../generators/running.md#на-каком-движке-это-считается), голый `parent="Имя"` без
значения, ссылка на [пул](../pools/overview.md#top) — то дисковый прогон, выбранный
...
временем, а не ОЗУ — см. предупреждение выше. **Ограничения:** поля составного `uniq`
должны быть списками [`text`](../generators/text.md#top).
...
> [!CAUTION]
> **У самой проверки есть потолок**
>
> Примерно за сотней миллионов строк проверка ёмкости перестаёт успевать первой: прогон
> тянется за памятью раньше, чем досчитана арифметика, и вместо сообщения выше приходит
> падение. Держите `count=` в пределах, которые вы действительно стали бы генерировать, —
> тогда проверка делает свою работу.
...
(`toArray`/`iterate`/`getAt`), явный `mode="memory"` и любая из [шести форм конфига,
...
- Перед очень большим прогоном сверьте конфиг с [шестью формами, которые возвращают его
