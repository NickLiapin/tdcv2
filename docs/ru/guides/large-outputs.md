...
- **Быстрый потоковый движок** — берётся почти для всего. Ленивый,
  ...
  Точные проценты, уникальность, [`parent`](hierarchical-dependencies.md#top)-зависимости,
  [`<mix>`](../reference/tags.md#top), [`<distinct>`](../constructs/unique-values.md#top) — всё «на лету».
- **Точный движок на диске** — включается сам для тех случаев уникальности, которые
  потоковый движок не может решить на лету: `percent` **и** `uniq` на одних столбцах,
  либо `uniq` по нетекстовым полям (числа, даты, шаблоны). Результат он гарантирует
  точно, и память остаётся ограниченной — но платит за это проверкой данных через
  внешнюю сортировку и проход починки, и **эта проверка резко замедляется с ростом числа
  строк** (см. предупреждение ниже).
...
[`uniq="true"`](../constructs/unique-values.md#top) по конечному текстовому списку и
env-уровневый [`<uniq>`](../constructs/unique-values.md#top),
[`<distinct>`](../constructs/unique-values.md#top) и
[`<mix>`](../reference/tags.md#top) — всё «на лету», точно и параллельно. Точному движку он
...
[`text`](../generators/text.md#top), равномерно (без `percent` на колонках) и не
