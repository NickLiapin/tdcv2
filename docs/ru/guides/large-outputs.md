...
У дискового режима есть и третье назначение, и как раз о нём стоит знать: пять форм
...
конфиг, и пять форм уводят прогон на **маленький in-memory движок**, память которого
...
Такой откат — не баг. Каждая из пяти форм — это обещание про целый столбец, а движок,
...
(`toArray`/`iterate`/`getAt`), явный `mode="memory"` и любая из [пяти форм конфига,
