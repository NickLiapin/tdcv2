...
итог](../generators/running.md#на-каком-движке-это-считается),
[статистика](../generators/stat.md#top), [дата, отмеренная от другой
колонки](../generators/date.md#top), голый `parent="Имя"` без значения, ссылка на
[пул](../pools/overview.md#top) — то дисковый прогон, выбранный автоматически,
откатывается на in-memory движок, а не падает. Принудительный `--engine 2` всё равно
упадёт — ради этого его и форсируют:
