...
| `uniq="true"` на [счётчике](../generators/counters.md#top)                             | в памяти        | растёт с `count`        |
| `uniq="true"` на составной последовательности (именованные поля `<gen>`)            | точный на диске | ограничена ~1 ГБ        |
| env-уровневая группа [`<uniq>`](../constructs/unique-values.md#top)                    | в памяти        | растёт с `count`        |
