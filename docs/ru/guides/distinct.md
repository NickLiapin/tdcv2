...
скажем, одно слово на два поля, — ограничение выполнить невозможно. TDC сразу
выдаёт понятную ошибку, а не крутится в цикле вечно:
...
**Почему:** невыполнимый запрос должен падать сразу, а не зависать. Это та же
проверка выполнимости, что делает [`uniq`](../constructs/unique-values.md#top) перед генерацией.
...
значением** — простой [`<gen>`](../generators/overview.md#top) или `<mix>`. У составной
(многополевой) последовательности нет единственного значения для сравнения, поэтому
положить её в группу нельзя — ошибка `TDC129`:
