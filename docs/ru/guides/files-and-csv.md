...
## Файл измерений — `read="quantile"`

Всё выше читает файл как **мешок значений**: выбрать одно, положить в ячейку. Для
считаемого это верно — город, статус, число заказов, — и `weight=` даже соблюдает
настоящие доли.

Для **измерения** это неверно. Тысяча записанных сумм, прочитанная как мешок, даёт
тысячу разных сумм при любом числе строк: миллион строк по-прежнему держит эту тысячу
значений и ничего между ними. Настоящие деньги устроены не так, и модель, обученная на
этом, выучит структуру, которой в данных не было.

`read="quantile"` читает тот же файл иначе — один раз отсортировав и обращаясь с ним как
с линейкой, на которую строка может попасть в любое место:

```xml
<sequence name="Amount"><gen type="file" src="amounts.txt" read="quantile"/></sequence>
```

Форма выборки возвращается, значения между наблюдениями появляются сами, а ответ
пишется с тем же числом знаков после запятой, что и файл. Добавьте `sample="exact"`,
чтобы не разыгрывать распределение, а вымести его ровно, — тогда прогон воспроизводит
выборку вообще без выборочного шума. И то и другое полностью разобрано на странице
[генератора file](../generators/file.md#readquantile--измеренная-выборка-как-распределение).
