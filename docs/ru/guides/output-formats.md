...
[Блок вывода](../core-concepts/output-formatting.md#top) собирает **текст**:
[`<before>`](../core-concepts/output-formatting.md#top) / [`<after>`](../core-concepts/output-formatting.md#top)
обрамляют всю сборку, [`<block>`](../core-concepts/output-formatting.md#top) — одна запись, а
[`<data>`](../core-concepts/output-formatting.md#top) — сырой текст. Поэтому один и тот же движок
...
Заголовку место в [`<before>`](../core-concepts/output-formatting.md#top): он печатается ровно
...
**Инструмент.** Фильтр [`csv`](masks-and-case.md#top) берёт поле в кавычки и удваивает
...
[файла данных](files-and-csv.md#top).
...
(Атрибут [`delimiter`](files-and-csv.md#top) — это про другое: он говорит генератору
[`file`](../generators/file.md#top), как **читать** входной CSV. На выходе вы просто пишете нужный
символ.)
...
Первое — условный кусок. Второй [`<data if="!_last">`](../core-concepts/output-formatting.md#top)
...
Второе — отдельная фикстура. [`<delimiter_block>`](../core-concepts/output-formatting.md#top)
...
Несколько значений в одном поле даёт [`repeat`](../reference/attributes.md#top), а
[`separator`](../reference/attributes.md#top) — то, что встанет между ними. JSON хочет `", "` —
...
в JSON фильтром [`replace`](masks-and-case.md#top), где кавычки писать **можно**:
...
напрямую. И учтите: [`repeat`](../reference/attributes.md#top) берёт каждое значение независимо,
так что один тег может выпасть в строке дважды; если так не нужно, задайте фиксированный набор
через [`<mix>`](../reference/tags.md#top).

### `null` — это слово, а не пустота

**Задача.** Для необязательного поля напрашивается [`missing`](../reference/attributes.md#top),
...
**Инструмент.** Напишите слово `null` явно, как одну из веток [`<mix>`](../reference/tags.md#top),
...
[файла](files-and-csv.md#top). Товар `Кофе "Арабика" 250 г` попадает в строку как есть, и вторая
...
[`replace`](masks-and-case.md#top) это и делает — это единственное место, где слэш пишете вы сами:
...
в памяти, — см. [Большие выгрузки и потоковая запись](large-outputs.md#top).
...
[`<delimiter_line>`](../core-concepts/output-formatting.md#top). Построчные фикстуры видят те строки,
...
**Инструмент.** Фильтр [`sql`](masks-and-case.md#top) удваивает апостроф — так SQL экранирует его
внутри литерала:
...
[`<before>`](../core-concepts/output-formatting.md#top), который печатается впереди всех строк:
...
[`each`](../reference/attributes.md#top); полный приём — в
[Связные и реляционные данные](coherent-data.md#top).
...
- **[Вывод и форматирование](../core-concepts/output-formatting.md#top)** — `<block>`, `<line>`,
  `<data>`, фикстуры и условие `if`.
- **[Маски и регистр](masks-and-case.md#top)** — фильтры экранирования `csv` / `sql` / `replace`
  целиком.
- **[Чтение файлов и CSV](files-and-csv.md#top)** — тяните свои значения из файла генератором
  [`file`](../generators/file.md#top).
- **[Связные и реляционные данные](coherent-data.md#top)** — связанные таблицы, внешние ключи и
  атрибуты `each` / `weight` / `row`.
- **[Большие выгрузки и потоковая запись](large-outputs.md#top)** — миллионы строк и NDJSON на
  диске.
