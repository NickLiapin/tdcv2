...
Вот что выдаёт обычный генератор [`text`](../generators/text.md#top) для пачки
...
> [!WARNING]
> **Два генератора через этот слой не проходят**
>
>
...
- **A** — сгенерированное значение до маски
- **B** — маска: x это гнездо, всё остальное — литерал, который остаётся как есть
- **C** — результат — линии показывают, какой символ занял какое гнездо
...
| Атрибут на [`<gen>`](../generators/overview.md#top) | `<gen … mask="…" case="…"/>` | оформить **весь** генератор разом       |
| Тег в [`<compute>`](../compute/overview.md#top)     | `<mask pattern="…">…</mask>` | когда форматирование — звено вычисления |

Большинство примеров ниже — на фильтрах; они живут в
[интерполяции](../core-concepts/output-formatting.md#фильтры). У каждого
compute-тега тоже есть свой дом в
[Строках и форматировании](../compute/strings.md#top).
...
случаями — на [`<mask>`](../compute/strings.md#перестройка).
...
- **A** — исходное значение, каждая позиция пронумерована
- **B** — что получилось на выходе
- **drawn** — позиция, которую назвал индекс, и куда она попала
- **made** — что взяли голые слоты — в том порядке, в каком стояли
...
- **A** — исходное значение, до индексированной выборки
- **B** — пул, из которого тянут голые слоты, — позиции, забранной индексом, в нём больше нет
- **drawn** — позиция, которую назвал x[4]
...
- **A** — исходный код, каждая позиция пронумерована
- **B** — результат — десять символов из шести
- **drawn** — два символа, названные дважды: печатаются с обоих концов
- **made** — остальное, взятое `*` в исходном порядке
...
[`TDC199`](../reference/errors.md#top), до того как сгенерирована хоть одна строка:
...
[`<gen>`](../generators/overview.md#top). Слева — тот же генератор без атрибута
...
[`template`](../generators/template.md#технические-идентификаторы). Если заданы оба
...
Здесь работают **два** способа из трёх — фильтр и тег в
[`<compute>`](../compute/strings.md#top). Атрибутами `<gen>` из этого семейства существуют
только `mask` и `case`; `slice=` или `trim=` на `<gen>` — это `TDC015`, поэтому остальное
форматируйте там, где значение печатается. Ниже — по формуле «сырьё → инструмент →
...
[`<slice>`](../compute/strings.md#перестройка).
...
- **Пустой `from` отвергается.** У `replace:,+` и у голого `replace` нечего искать, поэтому
  [TDC275](../reference/errors.md#top) останавливает запуск, а не даёт фильтру тихо ничего не
  сделать. Чтобы УДАЛИТЬ, задавайте вторую часть: `replace:-,`.

Там, где это важно, берите вместо фильтра тег
[`<replace>`](../compute/strings.md#перестройка) внутри `<compute>`: он принимает
...
[`<trim>`](../compute/strings.md#перестройка).
...
[`<group>`](../compute/strings.md#перестройка).
...
| :-------- | :------------------------- | :---------------------------------------------------------------- |
| `slice`   | `slice:from[,to]`          | [`<slice from="0" to="4">`](../compute/strings.md#перестройка)   |
| `replace` | `replace:from,to`          | [`<replace from="-" to="/">`](../compute/strings.md#перестройка) |
| `trim`    | `trim`                     | [`<trim>`](../compute/strings.md#перестройка)                    |
| `group`   | `group:size[,sep]`         | [`<group size="3" sep=" ">`](../compute/strings.md#перестройка)  |
...
**Проблема.** По умолчанию [`text`](../generators/text.md#top) и
[`file`](../generators/file.md#top) берут значения **случайно**. Иногда у данных
...
[Форматы вывода → CSV](../guides/output-formats.md#csv).
...
фильтра, который сознательно игнорируется, а не [отвергается](../reference/errors.md#top).
...
[JSON](../guides/output-formats.md#json) отдельного фильтра нет: там кавычка
...
- **[Строки и форматирование](../compute/strings.md#top)** — те же операции как compute-теги.
- **[Вывод и форматирование](../core-concepts/output-formatting.md#top)** — где живут интерполяция и фильтры.
- **[Форматы вывода](../guides/output-formats.md#top)** — CSV, JSON и SQL от начала до конца.
