...
Обычный [`<gen type="number" value="150..200"/>`](../generators/number.md#top) выдаёт
...
может быть [выражением](../reference/expressions.md#top) над другими колонками строки, а
...
прогона, а [`_count`](../reference/builtins.md#top) — номер строки:
...
> [!NOTE]
> **Почему здесь можно, а построчный `repeat=` нельзя**
>
...
> строку считаемой в одиночку — а на этом построены [потоковые движки](large-outputs.md#top).
> Построчное число в [`repeat=`](../constructs/multiple-values.md#top) изменило бы количество
...
![Ровная гистограмма рядом с колоколом на том же диапазоне](../../img/distributions/uniform-vs-normal.svg)

*Один и тот же диапазон 150–200, сгенерированный по 20 000 раз каждым способом и посчитанный.*

- **faint** — обычный диапазон: любой рост примерно так же вероятен, как любой другой
- **made** — тот же диапазон с нормальным распределением: холм вокруг центра
...
![Гистограммы девяти поддерживаемых распределений](../../img/distributions/nine-shapes.svg)

*Девять прогонов по 8000 значений, посчитанных по корзинам. Вот как эти названия выглядят на самом деле.*

- **made** — сгенерированные значения, разложенные по корзинам
...
![Колокол и он же, обрезанный по границам](../../img/distributions/min-max.svg)
...
- **made** — слева без границ, справа тот же прогон с границами 160 и 180
...
[Большие объёмы вывода](../guides/large-outputs.md#top)). Миллиард логнормальных
«зарплат» — не проблема.
...
- **[Number](../generators/number.md#top)** — режимы диапазона и строки-из-цифр,
  которые заменяет этот атрибут.
- **[Большие объёмы вывода](../guides/large-outputs.md#top)** — почему распределения
  остаются дешёвыми даже на миллиарде строк.
- **[Связанные данные](../guides/coherent-data.md#top)** — как удерживать
