<a name="top"></a>

# Constructs

**English** · [Русский](../ru/constructs/README.md#top) · [Español](../es/constructs/README.md#top)

📖 **[Read this on the documentation site →](https://nickliapin.github.io/tdcv2/docs/intro)**

**[Contents](../README.md#top)**

---

- [Overview](./overview.md#top)
- [Choosing between values (mix)](./mix.md#top)
- [Lookup tables (switch)](./switch.md#top)
- [Conditional output (if)](./conditional-output.md#top)
- [Multiple values in a cell (repeat)](./multiple-values.md#top)
- [One row per element (each)](./relational-tables.md#top)
- [Uniqueness (uniq, distinct)](./unique-values.md#top)

---

**[Contents](../README.md#top)**
