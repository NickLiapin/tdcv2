...
have a `Country` column and you need a `Currency` beside it that is *always* consistent:
...
> can differ between core versions. Treat them as examples of *shape*, not guarantees.
...
Point `on` at a different sequence and the same table starts reading *its* values
...
- Key and value are split by the **first colon** `:` — so colons *inside* a value are
...
Here a customer tier drives a discount *range*, something a flat table can't express:
