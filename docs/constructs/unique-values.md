...
plain pack or file list now takes the same road — so an EVEN `count` reaches the
group's true ceiling from any seed. The top edge that remains belongs to the odd
counts: an odd run splits the blocks 13/12, a value's fair share of 2.6 and 2.4
rows has to round somewhere, and near the ceiling those roundings can leave one
block a single row short. Measured on one shape — two subjects, three names
each, five shared values, at `seed="blocks"`:

| count | 2–24 | 25  | 26  | 27  | 28–30 | 31+ |
| ----- | ---- | --- | --- | --- | ----- | --- |
|       | ✅   | ❌  | ✅  | ❌  | ✅    | ❌  |

Every count up to 24 renders, and so does the ceiling itself; the two odd counts
just under it fall one row short on this seed and collect on others. If a count
near the limit is refused, the honest fix is more values in a member, which
moves the whole range rather than one point of it.
...
into, added up — so it sits within one row of the largest count that renders.
