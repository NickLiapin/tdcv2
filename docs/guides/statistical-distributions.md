...
## A parameter can follow another column

Every parameter — `mean`, `sd`, `lambda`, `rate`, `alpha`, `xmin`, `shape`, `scale` —
may be an [expression](../reference/expressions.md#top) over the row's other columns
instead of a fixed number. A plain number is the ordinary case and costs nothing.

That is how an intensity that depends on something else is written. Orders arriving
at a rate that changes with the hour:
...
The same seam gives a spread that drifts. An ageing sensor gets noisier as the run
goes on, and [`_count`](../reference/builtins.md#top) is the row number:
...
> [!NOTE]
> **Why this is allowed, when a per-row `repeat=` is not**
>
> How many random draws a row consumes depends on WHICH distribution and never on its
> parameters, so a parameter that changes per row leaves the row computable on its own —
> the property the [streaming engines](large-outputs.md#top) are built on. A per-row
> [`repeat=`](../constructs/multiple-values.md#top) count would change the number of draws,
> which is why the shape of a fan-out is declared as a whole-run quota instead.
