<a name="top"></a>

# Guides

**English** · [Русский](../ru/guides/README.md#top) · [Español](../es/guides/README.md#top)

📖 **[Read this on the documentation site →](https://nickliapin.github.io/tdcv2/docs/intro)**

**[Contents](../README.md#top)**

---

- [Hierarchical dependencies](./hierarchical-dependencies.md#top)
- [Coherent & relational data](./coherent-data.md#top)
- [No repeats within a row](./distinct.md#top)
- [Reading files & CSV](./files-and-csv.md#top)
- [Output formats (CSV, JSON, SQL…)](./output-formats.md#top)
- [Masks & case](./masks-and-case.md#top)
- [Statistical distributions](./statistical-distributions.md#top)
- [Anomalies & outliers](./anomalies.md#top)
- [Missing data](./missing-data.md#top)
- [Typed output & Parquet](./typed-output-parquet.md#top)
- [Large outputs & streaming](./large-outputs.md#top)
- [Writing a service generator](./writing-a-service.md#top)
- [Performance](./performance.md#top)

---

**[Contents](../README.md#top)**
