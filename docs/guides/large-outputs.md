...
| `uniq="true"` on a [counter](../generators/counters.md#top)                 | exact on-disk | bounded at ~1 GB   |
| `uniq="true"` on a compound sequence (named `<gen>` fields)              | exact on-disk | bounded at ~1 GB   |
| An env-level [`<uniq>`](../constructs/unique-values.md#top) group           | exact on-disk | bounded at ~1 GB   |
