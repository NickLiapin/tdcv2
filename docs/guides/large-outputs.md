...
Only the compound form is bounded, and this table used to promise all three were. Measured
on a counter, single-threaded, peak resident:

```
200 k rows  168 MB      4 M rows   822 MB      20 M rows  3.0 GB
  1 M rows  308 MB     10 M rows   1.9 GB
```

It is the row people plan hardware on, so it is worth being exact: the ~1 GB is the
external sort's chunk, and only the compound form stays inside it.
