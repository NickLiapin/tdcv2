...
| [`file` with `read="quantile"`](../generators/file.md#top) | **text** — declare `type="float"` when you want a numeric column |
...
A [quantile read](../generators/file.md#top) is the one row on that table where the value
IS provably a number — the file has to be numeric or the run refuses — and the column is
still written as text. That is inference following its own rule rather than the value:
what the engine knows is that a `file` generator produces whatever the file holds, and
that is a fact about the generator, not about this use of it. Write `type="float"` (or
`type="int64"` for a whole-number sample) and the column is typed:
