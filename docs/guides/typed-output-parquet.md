...
A column can hold a **map** — `type="{}int64"` is a map from text to `int64`. The
cell reads `key:value` pairs, separated the same way a list's elements are:
...
The reader gets a real map, not a string it has to pull apart — `gauges['cpu']` is
a query, and `missing=` on the generator behind a value comes through as a genuine
`null` inside it.

**The key is always text.** Parquet does not allow a null key, a cell arrives as
text anyway, and a second type parameter would double the syntax to buy a
conversion nobody has asked for. So `{}T` reads "a map from text to T" and there
is nothing else to decide. `|null` binds to the VALUE, exactly as it binds to a
list's element.

### Writing the cell

- **The key is everything before the first `:`; the value is the rest.** So a
  value may hold colons — `start:12:30:00` is one pair — and a key may not.
- **An empty cell is an empty map**, the same rule a list follows.
- **Three things are refused** rather than guessed at, because each would leave
  you with a map quietly missing an entry: a piece with no `:` at all, an empty
  key, and a key that repeats inside one row (readers disagree about which of the
  two wins, and some drop the row's map entirely).
...
- **Lists of lists** — `[]int64` and `{}int64` both work, but `[][]int64` does not:
  it would need a second separator, and no shape for one has earned its place yet.
