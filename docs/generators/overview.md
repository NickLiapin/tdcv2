...
Add a fixed [`repeat`](../constructs/multiple-values.md#top) and the row takes **N values
off the walk** instead of one. The walk carries on across rows, so element _k_ of row _r_
is source value _r_×N+_k_:

```xml
<sequence name="Step"><gen type="text" value="created,paid,shipped,delivered" repeat="4" order="sequential"/></sequence>
```

`./run steps.tdc (3 rows)`
...
record's [whole lifecycle](../constructs/relational-tables.md#top) in one sequence. Over a
...
[length quota](../constructs/multiple-values.md#top) has no such number. **A walked `date`**
