...
A big atom is unaffected — 40% of the sample being exactly zero comes back as
39.995% — because its plateau dwarfs the ramps at its edges. A value that appears
twice in a thousand keeps roughly half its share.

So: if the exact share of each listed value is what matters, that is
