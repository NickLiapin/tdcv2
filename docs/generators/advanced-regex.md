...
cannot share one name: `(?if{sex=…})` would then be a coin toss between them.
...
rebuilding the column as a [`<switch>`](../reference/tags.md#top).
...
- **Read anything but a named group** — not another column, not `${{…}}`. For
  cross-column logic use [`<switch>`](../reference/tags.md#top) or a
