...
A formula reads only its own row, so it can also be one branch of a per-row choice: a
[`<case>`](../constructs/switch.md#case--a-branch-with-a-generator) of a `<switch>` or a
`<mix>`, at any depth, or one of a sequence's `<gen if="…">` branches. Each row the branch
holds is computed from that row.

```xml
<tdc>
  <env count="6" seed="shop" local="en">
    ...
    <switch name="Each" on="Qty">
      <case is="0"><data>nothing bought</data></case>
...
`Total / Qty` is a division by zero on the rows where `Qty` is 0, and those rows belong to the
other case — so the formula is never asked about them. The same column written with `if=`
works the same way:

```xml
<sequence name="Each">
  <gen if="Qty > 0" type="formula" expr="Total / Qty" decimals="2"/>
  <gen type="text" value="nothing bought"/>
</sequence>
```

`_count` inside a branch is the row's own number, as it is everywhere else. Two things stay
out of a branch, and `check` says so with [`TDC295`](../reference/errors.md#top): a formula that
reads `prev()`, because it needs the rows before it and is built in row order over the whole
run, and any formula used as a part or a field of a composed `<sequence>`. Compute it as a
sequence of its own and put it into the record where the record is printed.
...
- **a name that is not a column declared above** — [`TDC240`](../reference/errors.md#top),
  ...
  arithmetic.
- **a formula where it cannot mean anything** — [`TDC295`](../reference/errors.md#top): one that
  reads `prev()` inside a `<case>` or an `if=` branch, and any formula as a part or a field of
  a composed `<sequence>`. See [In a branch](#in-a-branch).
