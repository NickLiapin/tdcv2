...
| :------------------------ | :----------------------------------------------- | :----------: |
| `person.male.firstName`   | A male first name                                |      86      |
| `person.female.firstName` | A female first name                              |      86      |
| `person.lastName`         | A last name (male + locale-common surnames)      |      86      |
| `person.male.diagnosis`   | A male diagnosis + common ones                   |      86      |
| `person.female.diagnosis` | A female diagnosis + common ones                 |      86      |
| `person.gender`           | A random gender; the label comes from the locale |      86      |
| `person.b_day`            | A birthday in the format you specify             | format only  |

All six ship in every one of the **86 language packs** the registry carries — see the
[catalogue](../data-packs/catalogue.md#top) for the list. Any other locale gets
[TDC217](../reference/errors.md#top), which names the locales that do ship
...
| Path               | Produces       | Locale-aware |
| :----------------- | :------------- | :----------: |
| `location.country` | A country name | all 86 packs |
...
>
> `location.country` ships in all **86** language packs the registry carries. The lists are not
> all the same length — a pack names the countries its own language has names for, so they run
> from 23 entries to 247, with 206 in the middle and 237 in English. A locale with no pack has
...
| `geo.postalCode`  | 76                         |
| `geo.region`      | 68                         |
| `geo.streetName`  | 45 (plus 38 with `geo.streetNamed`) |
| `geo.province`    | 35                         |
| `geo.municipality`| 21                         |
| `geo.district`    | 21                         |
| `geo.zip`         | 16                         |

Some subdivision exists in 137 of the 152 country packs and a postal code in 92 — under
