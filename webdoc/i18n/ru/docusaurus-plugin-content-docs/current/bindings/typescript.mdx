...
| `mode`          | `"memory"` или `"disk"` — тот же выбор, что и [`--mode`](../reference/cli.mdx)         |
...
[`<gen type="http">`](../generators/http.mdx) **обязан** пользоваться ею. Сетевой вызов
...
:::note[Те же значения, по одной строке]
...
[По одному значению](../getting-started/quick-api.mdx).

## Одни и те же имена во всех языках

Объект готового прогона отзывается на одни и те же имена во всех пяти пакетах,
записанные по правилам каждого языка. [Таблица здесь](same-names.mdx), и наборы
