...
- [`<env>`](../core-concepts/configuration.mdx) с `count="3"` — сгенерировать **3**
  строки; `seed` фиксирует случайность, чтобы результат
  [повторялся](../core-concepts/determinism.mdx) от запуска к запуску.
- [`<sequence>`](../core-concepts/sequences.mdx) — **столбец** данных; внутренний
  [`<gen>`](../generators/overview.mdx) говорит, откуда брать значения (здесь —
  случайное имя из списка).
- [`type="text"`](../generators/text.mdx) — выбрать одно значение из списка через
  запятую.
- [`type="number"`](../generators/number.mdx) с `value="18..65"` — случайное число
  из диапазона.
- [`<block>`](../core-concepts/output-formatting.mdx) / `<line>` / `<data>` —
...
Всё это описано в разделе [Установка](installation.mdx).
...
[справочнике CLI](../reference/cli.mdx).
...
- [`percent="50,50"`](../generators/text.mdx) заставляет генератор
  [`text`](../generators/text.mdx) делить значения примерно поровну между `Мужчина` и
  `Женщина`, а не выбирать равновероятно.
- [`type="template"`](../generators/template.mdx) с `value="person.male.firstName"`
...
разделе [Иерархические зависимости](../guides/hierarchical-dependencies.mdx).
...
- **[Структура конфигурации](../core-concepts/configuration.mdx)** — `<tdc>`, `<env>` и как устроен конфиг.
- **[Иерархические зависимости](../guides/hierarchical-dependencies.mdx)** — главная возможность.
- **[Значения-шаблоны](../generators/template.mdx)** — `person.*`, `date.*`, `location.*` и остальные встроенные данные.
- **[Справочник CLI](../reference/cli.mdx)** — полная командная строка, теги, атрибуты и генераторы.
