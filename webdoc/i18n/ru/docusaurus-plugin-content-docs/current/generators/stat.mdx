...
`accumulate=` считает итог по списку внутри одной записи. [`running`](running.mdx) считает
...
- **`stddev` — это стандартное отклонение по ГЕНЕРАЛЬНОЙ совокупности**, деление на _n_, а
  ...
  в [выражениях](../reference/expressions.mdx).
...
[`parent`](../guides/hierarchical-dependencies.mdx), не считаются и не участвуют ни в одной
...
правило, что у [`parent`](../guides/hierarchical-dependencies.mdx) и у `running`, и по той
...
[Потоковый движок](../guides/large-outputs.mdx) отказывает статистике по имени, и роутер
...
Цена — та же граница, что у [`running`](running.mdx#на-каком-движке-это-считается), только
...
- [`running`](running.mdx) — та же колонка, но накопленная на ходу, а не подытоженная в
  конце
- [`accumulate=` на repeat-списке](../constructs/multiple-values.mdx#accumulate--нарастающий-итог-по-списку) —
  та же идея внутри одной записи, бесплатно на любом движке
- [Выражения](../reference/expressions.mdx) — `sum`, `mean`, `median` и `stddev` есть и как
