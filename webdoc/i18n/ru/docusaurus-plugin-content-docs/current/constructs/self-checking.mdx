---
title: Самопроверяющиеся конфиги (assert)
sidebar_position: 8
---
...
Живёт в `<env>`, рядом с [`<uniq>` и `<distinct>`](unique-values.mdx): как и они, это
...
| :------ | :--------- | :------------------------------------------------------------------ |
| `that`  | да         | Условие, на том же языке, что и [`if=`](../reference/expressions.mdx) |
...
[`<gen type="stat">`](../generators/stat.mdx) — одно число на весь запуск.
...
- **Проверки контрольной цифры.** Вот это ловушка: её посчитал
  [`<compute>`](../compute/overview.mdx), и пересчёт утверждает лишь то, что один и тот же
...
[большие выгрузки](../guides/large-outputs.mdx).

## См. также

- [`stat`](../generators/stat.mdx) — откуда берутся числа
- [Выражения](../reference/expressions.mdx) — язык, на котором пишется `that=`
- [Уникальность](unique-values.mdx) — другие высказывания про весь запуск
