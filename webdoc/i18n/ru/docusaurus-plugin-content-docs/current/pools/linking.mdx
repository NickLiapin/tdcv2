...
- **Веса у членов.** Собственного веса у члена пула нет. Используйте
  [`<mix>`](../constructs/mix.mdx) внутри пула — это то же самое, сказанное языком, на
...
- [Обзор](overview.mdx) — что такое пул и потолок размера
- [Отбор через `filter`](filter.mdx) — язык выражений, на котором записывается связь
- [Иерархические зависимости](../guides/hierarchical-dependencies.mdx) — `parent`, другой
