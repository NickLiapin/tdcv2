---
title: Determinismo y proporciones
sidebar_position: 4
---
...
<Figure
  ...
  alt=""
  caption="Tres ejecuciones de la misma configuración, de 60 filas cada una."
/>

<Legend items={[["A", "primera ejecución, una semilla"], ["B", "segunda ejecución, la misma semilla — idéntica valor por valor"], ["C", "una semilla distinta: datos de la misma forma, pero ningún número en común"]]} />
...
:::note[La misma salida, sea cual sea el motor]
...
los tres. Cómo se elige el motor está en [Salidas grandes](../guides/large-outputs.mdx):
es cuestión de velocidad y memoria, no de qué datos obtiene.
:::

`seed` se define en [`<env>`](configuration.mdx). Su valor es cualquier string (una cadena
...
Tanto el nombre (de [`template`](../generators/template.mdx)) como el código (de
[`number`](../generators/number.mdx)) se ven aleatorios. Ejecute la configuración **dos
...
<Terminal title="./run people.tdc  —  la misma semilla, con un año de diferencia">
...
[`<env>`](configuration.mdx), vale **10** por omisión y lo sobrescriben la opción
...
mayoría de los generadores —[`number`](../generators/number.mdx), un
[`template`](../generators/template.mdx) sin ponderar,
[`counter`](../generators/counters.mdx), [`regex`](../generators/regex.mdx)— calculan el
...
trabajan así: las proporciones exactas (`percent` en [`text`](../generators/text.mdx) y en
...
([`uniq`](../constructs/unique-values.mdx)), un pack
[`template`](../generators/template.mdx) **ponderado**, y las **longitudes de lista** de
[`repeat="min..max"`](../constructs/multiple-values.mdx) — allí un rango es una cuota
...
Agregue `percent` a un generador [`text`](../generators/text.mdx) (o a un `<mix>`) y las
...
más corta que la lista [`value`](../generators/text.mdx) se expande: las posiciones llenas
...
Cuando la secuencia tiene un [`parent`](sequences.mdx), los porcentajes se miden **dentro
...
- **[Text](../generators/text.mdx)** — `percent` a fondo, incluidas las
  [máscaras cortas](../generators/text.mdx#máscaras-percent-cortas).
- **[Dependencias jerárquicas](../guides/hierarchical-dependencies.mdx)** — proporciones dentro de un subconjunto.
- **[Valores únicos](../constructs/unique-values.mdx)** — el otro diseño de ejecución completa que se recalcula con `count`.
- **[Salidas grandes](../guides/large-outputs.mdx)** — cómo funcionan las proporciones exactas durante el streaming.
