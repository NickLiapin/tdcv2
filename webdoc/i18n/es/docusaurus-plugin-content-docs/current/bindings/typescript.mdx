...
| `mode`          | `"memory"` o `"disk"` — la misma elección que [`--mode`](../reference/cli.mdx)         |
...
[`<gen type="http">`](../generators/http.mdx) **tiene que** usarlo. Una llamada de red no
...
superficie está en [Un valor a la vez](../getting-started/quick-api.mdx).
