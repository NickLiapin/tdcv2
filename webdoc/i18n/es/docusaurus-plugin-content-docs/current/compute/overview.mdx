...
azar (de eso se encarga [`<gen>`](../generators/overview.mdx)) — **procesa valores que
usted ya tiene**. Vive dentro del propio config, adentro de una
[`<sequence>`](../core-concepts/sequences.mdx), y así es como se construyen las sumas de
...
- [`<gen>`](../generators/overview.mdx) **produce** un valor (aleatorio, a partir de un
...
en [`${{…}}`](../core-concepts/output-formatting.mdx), con una restricción que la
...
:::caution[Un procesador, no un generador]
...
([`TDC218`](../reference/errors.mdx)). Un procesador no puede prometer unicidad: no tiene
...
lee, o envuélvalas en [`<uniq>`](../constructs/unique-values.mdx).
...
a un `<gen>` es [`TDC219`](../reference/errors.mdx). Mueva el `<compute>` a su propia
...
<Figure
  ...
  alt=""
  caption="Un número de tarjeta real de una corrida: quince dígitos generados y el dígito que compute derivó de ellos."
/>

<Legend items={[["A", "los dígitos generados"], ["B", "el dígito verificador derivado"], ["C", "la aritmética: cada segundo dígito se duplica, se suma todo, y se agrega el dígito que lleva el total a un múltiplo de diez"]]} />
...
<Figure
  ...
  alt=""
  caption="Un login hecho con dos columnas: la primera letra de una, pegada a la otra, en minúsculas."
/>

<Legend items={[["A", "las columnas que un <compute> lee con <field> — se sortearon en otro lado"], ["B", "las operaciones, cada una alimenta a la siguiente; la más interna corre primero"], ["C", "el valor terminado, que es lo que sostiene <result>"]]} />
...
<Figure
  ...
  frame
  zoom
  caption="La misma búsqueda en el canvas de Studio — la lista y la posición cuelgan de puertos con nombre, no de un primer y un segundo hijo. Haga clic para abrirla a tamaño completo."
/>
...
Repasando las piezas: [`<reduce>`](lists.mdx#reduce--plegar-a-un-solo-valor) pliega los
15 dígitos en una sola suma corrida, `sum`. Cada segundo dígito (un
[`<current_index/>`](lists.mdx) par) se «duplica», pero Luhn toma la _suma de los
dígitos_ del valor duplicado, así que en lugar de `<multiply>` lo buscamos en una tabla
pequeña con [`<at>`](lists.mdx#at--indexar-una-lista). Un
[`<choose>`](conditionals.mdx) elige el valor duplicado o el dígito tal cual. El segundo
paso, `check`, convierte esa suma en el dígito final, y
[`<concat>`](strings.mdx) lo pega a la base.
...
nombre, con [`TDC288`](../reference/errors.mdx).
...
rechaza esa pareja con [`TDC286`](../reference/errors.mdx) en vez de dejárselo a la corrida:
...
completo está en la [referencia de funciones de compute](../reference/compute.mdx).
...
[atributos de `<gen>` y filtros `${{X|…}}`](../guides/masks-and-case.mdx).

### [Condicionales](conditionals.mdx)
...
junto a [`<gen>`](../generators/overview.mdx). Dos mecanismos ayudan a quien escribe un
pack.
...
- **[Condicionales](conditionals.mdx)** — `<choose>`, los predicados y `<valid>` a fondo.
- **[Máscaras y mayúsculas](../guides/masks-and-case.mdx)** — el mismo formato mediante
