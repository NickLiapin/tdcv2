...
Dos campos [`<gen>`](../generators/overview.mdx) que sacan de la **misma** lista corren
...
**1. Dentro de una [`<sequence>`](../core-concepts/sequences.mdx)** — envuelve los
...
<Terminal title="./run person.tdc (6 rows)">
{`Gustavo Mateo
Esteban Máximo
Daniel Aurelio
Edgar Eleuterio
...
**2. Dentro de [`<env>`](../core-concepts/configuration.mdx)** — envuelve bloques
...
- **En el nivel `<env>` el grupo acepta solo secuencias de un valor** — un `<gen>`
  simple, un [`<mix>`](mix.mdx) o un [`<switch>`](switch.mdx). Una secuencia compuesta
  (de varios campos) se rechaza ahí con el error `TDC129`.

## `uniq` — la combinación nunca se repite

`uniq="true"` en una [`<sequence>`](../core-concepts/sequences.mdx) **compuesta**
...
patrones [`regex`](../generators/regex.mdx) y [`advanced_regex`](../generators/advanced-regex.mdx),
...
<Terminal title="./run p.tdc (4 rows, counted)">
{`Ana Ruiz    2
Beto León   2`}
</Terminal>
...
hay de cada uno. Por eso una máscara [`percent`](../generators/text.mdx) sigue siendo
...
([`text`](../generators/text.mdx) sin `percent`), una muestra sesgada puede reducir el
...
En el grupo solo pueden ir secuencias de un valor (un [`<gen>`](../generators/overview.mdx)
...
[Qué motor corre su configuración](../guides/large-outputs.mdx#qué-motor-corre-su-configuración)
tiene el enrutamiento completo, incluidas las cuatro formas sin `uniq` que también acaban
en memoria.

:::danger[`uniq` exacto sobre una salida enorme es LENTO — `uniq` + `percent` más que nada]
...
las formas de `uniq` — exacto, pero acotado por la RAM. Vea **[Salidas grandes](../guides/large-outputs.mdx)**.
...
- **[Determinismo y proporciones](../core-concepts/determinism.mdx)** — por qué `uniq`
