---
title: Elegir entre valores (mix)
sidebar_position: 2
---
...
exactos. Piénselo como una [`<sequence>`](../core-concepts/sequences.mdx) cuyos
...
determinista para una [`seed`](../core-concepts/determinism.mdx) dada.
...
<Figure
  ...
  alt=""
  caption="Lo declarado frente a lo producido, sobre 1000 filas. No de manera aproximada: los conteos caen exactamente en las proporciones declaradas."
/>

<Legend items={[["band", "los porcentajes escritos en la configuración"], ["made", "la proporción que realmente se produjo"]]} />

## Una distribución con nombre

`<mix>` va **directamente en [`<env>`](../core-concepts/configuration.mdx)**, justo
...
<Terminal title="./run code.tdc (primeras 6 filas)">
{`A
B
A
B
...
<Terminal title="./run code.tdc (count=100, con conteo)">
{`A   25
B   70
C    5`}
</Terminal>
...
[`percent`](../generators/text.mdx) en el generador `text`.
...
[`text`](../generators/text.mdx) ya reparte proporciones exactas con `percent`.
...
<Terminal title="./run gender.tdc (count=20, con conteo)">
{`Hombre   10
Mujer    10`}
</Terminal>
...
<Terminal title="./run bucket.tdc (count=99, con conteo)">
{`low    33
mid    33
high   33`}
</Terminal>
...
[`percent` en `text`](../generators/text.mdx): un número fija la proporción de esa
...
<Terminal title="./run temp.tdc — las primeras diez filas, la misma semilla">
...
fila — [`anomaly`](../generators/number.mdx) en el generador decide fila por fila, así que
...
[`<sequence>`](../core-concepts/sequences.mdx) dependiente. Aquí las cuentas de paga
...
<Terminal title="./run tier.tdc (solo las filas Paid, con conteo)">
{`Silver     18
Gold        9
Platinum    3`}
</Terminal>
...
<Terminal title="./run status.tdc (count=100, con conteo)">
{`ok            34
warn          33
error/minor   23
error/major    7
...
[`<switch>`](./switch.mdx#una-proporción-dentro-de-una-rama) toma su cuota sobre las
...
:::danger[Una proporción menor que una fila no es una probabilidad baja, es un volado invisible]
...
el [determinismo](../core-concepts/determinism.mdx) funcionando tal como se
...
<Terminal title="./run diagnosis.tdc (count=20, seed=demo, solo filas masculinas)">
{`Male,Osteoarthritis
Male,Hypothyroidism
Male,Obesity
...
[`<data>`](../core-concepts/output-formatting.mdx), uno o más
[generadores](../generators/overview.mdx) y `<mix>` anidados. En este contexto
...
<Terminal title="./run charge.tdc (primeras 8 filas)">
{`36
reembolso: 4
contracargo: 18
66 (marcado)
...
[`number`](../generators/number.mdx) en `1..10`; `66 (marcado)` es un número en
...
<Terminal title="./run charge.tdc (count=100, con conteo por rama)">
{`reembolso     10
contracargo   12
simple        34
marcado       44`}
...
[bloque de salida](../core-concepts/output-formatting.mdx) es solo para la
...
- **`<switch>`** elige por una **clave** — una tabla de consulta, como
  `país → moneda`.
- Una [`<sequence>`](../core-concepts/sequences.mdx) con ramas condicionales
  [`<gen if="…">`](../core-concepts/sequences.mdx) elige por una **condición
...
- **[Generador text](../generators/text.mdx)** — proporciones `percent` exactas para
  una lista simple de opciones, cuando las ramas son palabras sueltas.
- **[Secuencias](../core-concepts/sequences.mdx)** — la fuente con nombre junto a la
  cual vive `<mix>`, y el modelo `parent` que comparten.
- **[Dependencias jerárquicas](../guides/hierarchical-dependencies.mdx)** — la
