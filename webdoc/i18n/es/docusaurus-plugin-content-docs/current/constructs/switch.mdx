---
title: Tablas de consulta (switch)
sidebar_position: 3
---
...
[secuencia](../core-concepts/sequences.mdx) sujeto (indicada por
[`on`](#el-sujeto--on)) y sustituye el valor hallado bajo esa clave. A diferencia de
[`<mix>`](../reference/tags.mdx), que reparte un campo **al azar por porcentaje**,
...
[`<mix>`](../reference/tags.mdx) (aleatorio) como las cadenas de `if` (verbosas)
hacen de manera torpe.

<Figure
  src="/img/guides/switch.svg"
  alt=""
  caption="Una tabla de consulta de tres filas y 24 filas generadas a través de ella."
/>

<Legend items={[["A", "la tabla: cada clave lleva un valor"], ["B", "las filas generadas — la misma clave siempre trae el mismo número, todas las veces"]]} />

## De un vistazo

`<switch>` vive **directamente en `<env>`**, junto a
[`<sequence>`](../core-concepts/sequences.mdx) y
[`<mix>`](../reference/tags.mdx). Toma dos atributos y contiene una o más ramas.
...
[declarada antes](../core-concepts/sequences.mdx) en el mismo `<env>` (una secuencia
...
[`<data>`](../reference/tags.mdx)): no es marcado, sino registros `CLAVE:VALOR`.
...
[`<data>`](../reference/tags.mdx) y generadores
[`<gen>`](../generators/overview.mdx), exactamente igual que una rama de
[secuencia](../core-concepts/sequences.mdx).
...
[`formula`](../generators/formula.mdx#en-una-rama) o un
[desplazamiento de fecha](../generators/date.mdx#en-una-rama) leen las columnas de esa fila y
nada más. Las construcciones que leen toda la corrida — [`running`](../generators/running.mdx),
[`stat`](../generators/stat.mdx), una fórmula con `prev()` — y una
[referencia a un pool](../pools/overview.mdx) tienen que ser secuencias propias (`TDC295`,
`TDC268`).
...
:::note[Dentro de `<mix>` no hay `is`]
La misma etiqueta [`<case>`](../reference/tags.mdx) se usa también dentro de
[`<mix>`](../reference/tags.mdx), pero ahí las ramas se eligen **al azar por
...
[secuencia](../core-concepts/sequences.mdx) con ramas `<gen if="…">`.
:::
...
Un `<case>` puede contener un [`<mix>`](./mix.mdx), y sus porcentajes son una cuota
...
<Terminal title="./run clinic.tdc (count=100, recuento)">
{`female -> influenza    50
male -> influenza      40
male -> prostatitis    10`}
</Terminal>
...
:::danger[El denominador es pequeño, así que una proporción chica puede redondearse a nada]
...
usted, avisando con [`TDC251`](../reference/errors.mdx) antes de la corrida. El
ejemplo resuelto está en
[Una proporción menor que un registro](./mix.mdx#una-proporción-menor-que-un-registro).
:::
...
<Terminal title="./run shipping.tdc (count=120, recuento)">
{`express          27
standard         63
international    30`}
</Terminal>
...
El mismo `<case>` está dentro de [`<mix>`](./mix.mdx), así que un `<switch>` anidado también
funciona allí: el mix decide si un registro es una factura, el switch decide qué impuesto
nombra.

:::note[Una proporción dentro de una rama anidada no fluye]
...
<Terminal title="./run currency.tdc (sujeto en US,FR,CA,MX,TR,BR,GB)">
{`GB -> XXX
CA -> USD
GB -> XXX
TR -> REG-473
...
<Terminal title="./run shipping.tdc (engine=&quot;2&quot; forzado)">
...
Vea [Salidas grandes](../guides/large-outputs.mdx) para el camino de streaming, y
[Determinismo](../core-concepts/determinism.mdx) para la garantía.

## `<switch>` es generación, no formato

Igual que [`<mix>`](../reference/tags.mdx), `<switch>` **produce un valor** y vive
...
- [`<mix>`](../reference/tags.mdx) elige **al azar, por porcentaje** — para
...
- **[Datos coherentes y relacionales](../guides/coherent-data.mdx)** — la otra manera de mantener
  dos campos consistentes: una secuencia hija tomada de un archivo por padre.
- **[Secuencias](../core-concepts/sequences.mdx)** — cómo declarar el sujeto y los
  demás campos.
- **[Text](../generators/text.mdx)** y **[Number](../generators/number.mdx)** — los
  generadores que se usan dentro de `<case>` y `<default>`.
- **[Referencia de etiquetas](../reference/tags.mdx)** — `<mix>`, `<case>` y la lista
