---
title: Una fila por elemento (each)
sidebar_position: 6
---
...
El atributo [`each`](../reference/attributes.mdx) repite una
[`<line>`](../core-concepts/output-formatting.mdx) una vez por cada elemento de una
...
<Figure
  ...
  alt=""
  caption="Una corrida, cuatro padres y sus hijos, exactamente como salen las filas."
/>

<Legend items={[["A", "las filas padre, cada una con su propia llave"], ["B", "las filas hijas — todas y cada una cargan la llave de su padre, así que nada queda huérfano"]]} />

## De un vistazo

| Dónde        | Qué                                                                                         |
| :----------- | :------------------------------------------------------------------------------------------ |
| Se aplica en | [`<line>`](../core-concepts/output-formatting.mdx)                                          |
| Valor        | El **nombre** de una secuencia cuyo generador tiene [`repeat`](../reference/attributes.mdx) |
| Efecto       | La línea se emite una vez por cada elemento de esa lista                                    |

La secuencia destino **debe** llevar [`repeat`](../reference/attributes.mdx): eso es lo
...
Déle a los pedidos su propia lista con [`repeat`](../reference/attributes.mdx) y luego
...
[`parent`](../guides/hierarchical-dependencies.mdx) ya decidió cuál lista está activa.
...
| todo lo demás    | como siempre: `${{Id}}`, [`${{_count}}`](../reference/builtins.mdx), cualquier otra secuencia    |
...
paralelismo de [`--jobs`](../guides/large-outputs.mdx) idéntico byte por byte a una corrida de un
...
Lo hacen dos piezas. Un [`<mix>`](mix.mdx) elige qué camino toma este pedido, e
[`if`](../core-concepts/output-formatting.mdx) en cada `<line>` decide si ese paso
...
:::note[La forma corta, y cuándo es la correcta]
...
más [`each`](../reference/attributes.mdx) para desplegarla. Un `repeat` fijo sobre una
...
<Terminal title="./run steps.tdc (3 pedidos)">
{`1000,created
1000,paid
1000,shipped
1000,delivered
...
rechaza están en [`order=` / `cycle=`](../generators/overview.mdx).
:::
...
| Qué                                                                        | Por qué                                                                                                             | Error    |
| :------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------ | :------- |
| `each` sobre una secuencia **sin** [`repeat`](../reference/attributes.mdx) | no hay nada que recorrer                                                                                            | `TDC207` |
...
<Terminal title="./run broken.tdc  (each sobre algo que no es una lista)">
...
[`repeat`](../reference/attributes.mdx) se queda como una lista real dentro de la
...
- **[Dependencias jerárquicas](../guides/hierarchical-dependencies.mdx)** — [`parent`](../reference/attributes.mdx),
  que decide _cuál_ lista está activa en cada fila.
- **[Datos coherentes y relacionales](../guides/coherent-data.mdx)** — la otra manera de relacionar
  tablas, por búsqueda contra un padre compartido.
- **[`repeat` / `separator`](../reference/attributes.mdx)** — cómo una secuencia se
  convierte en lista, para empezar.
- **[Integrados](../reference/builtins.mdx)** — `_item`, `_item_id`, `_count` y
  compañía.
