...
[`order="sequential"`](../core-concepts/output-formatting.mdx) para que los valores
...
<Terminal title="./run age.tdc (6 filas)">
{`1. edad 15
2. edad 17
3. edad 18
4. edad 25
...
[`<line>`](../reference/tags.mdx). Cuando la condición es falsa, TDC descarta la
línea entera:
...
[`_count`](../reference/builtins.mdx) es el lugar de la tarjeta dentro del conjunto
completo, decidido antes de renderizar, así que suprimir una línea no renumera al
resto.

:::warning[Escriba `<`, `>` y `&` de manera literal]
...
| :--------------------------------------------------------------- | :------------------------------------------------------------------------------------------------- |
| [`<line>`](../reference/tags.mdx)                                | La línea entera se suprime — incluido el separador entre filas.                                    |
| [`<data>`](../reference/tags.mdx)                                | Solo se suprime ese trozo de texto; los demás `<data>` de la misma línea siguen.                   |
| [`<gen>`](../generators/overview.mdx) dentro de una `<sequence>` | Crea una **secuencia condicional** (abajo). Los generadores no se permiten en el bloque de salida. |
...
<Terminal title="./run gendered-name.tdc (6 filas, local=es)">
{`Mujer:  Violeta
Hombre: Emilio
Mujer:  Lorenza
Mujer:  Susana
...
[Datos coherentes y relacionales](../guides/coherent-data.mdx) y
...
- **El atajo `X.Value`** — si `X` es una secuencia y `X.Value` no es en sí mismo un
  ...
  punto que se usa en [`parent="X.Value"`](../core-concepts/sequences.mdx).
...
[Comparación y verdad](../reference/comparison.mdx).
...
<Terminal title="./run age-bands.tdc (6 filas)">
{`edad 15: menor18
edad 17: menor18
edad 18: adulto
edad 25: adulto
...
<Terminal title="./run roles.tdc (6 filas)">
{`1. guest: [normal]
2. user: [normal]
3. admin: [admin]
4. user: [normal]
...
[Comparación y verdad](../reference/comparison.mdx) recorre los rincones: qué cuenta como
...
<Terminal title="./run logical.tdc (6 filas)">
{`1. guest/15: marcado
2. user/17: marcado
...
<Terminal title="./run arithmetic.tdc (6 filas)">
{`1/6 edad 15:
2/6 edad 17:
3/6 edad 18:
...
:::caution[Operadores no soportados]
...
<Terminal title="./run dotted.tdc (6 filas)">
{`1. guest: [!Role.admin]
2. user: [!Role.admin]
3. admin: [Role.admin]
4. user: [!Role.admin]
...
[secuencias integradas](../reference/builtins.mdx) `_first` / `_last`, que se guardan
como las cadenas literales `"true"` / `"false"`, se comporten de forma intuitiva
dentro de `if`.

:::warning[Una columna que vale `0` es VERDADERA]
...
que usted quería hacer. Las [reglas de comparación](../reference/comparison.mdx) explican
...
<Terminal title="./run json-list.tdc (4 filas)">
{`--- HEAD ---
{"id": 1},
{"id": 2},
{"id": 3},
...
Vea [Secuencias integradas](../reference/builtins.mdx) para la lista completa.
...
<Terminal title="./run combined.tdc (6 filas)">
...
:::note[Se compila una vez, se reutiliza barato]
...
- [Secuencias integradas](../reference/builtins.mdx) — `_count`, `_first`, `_last`,
  `_total`.
- [`if` en la referencia de atributos](../reference/attributes.mdx).
- [Datos coherentes y relacionales](../guides/coherent-data.mdx) y
  [Dependencias jerárquicas](../guides/hierarchical-dependencies.mdx) — donde las secuencias
