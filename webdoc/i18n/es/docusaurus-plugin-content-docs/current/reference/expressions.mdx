...
| `filter=`                  | de qué miembros de un [pool](../pools/filter.mdx) puede sortear esta fila         |
| `expr=`                    | el VALOR de una columna [`formula`](../generators/formula.mdx)                    |
| un parámetro de distribución | el valor de `mean`, `sd`, `lambda` … [en esta fila](../guides/statistical-distributions.mdx#un-parámetro-puede-seguir-a-otra-columna) |
...
| `Person.Email`    | un campo de una [secuencia compuesta](../core-concepts/sequences.mdx)       |
...
| `_count`, `_last` | un [integrado](builtins.mdx) — el número de fila, la marca de última fila    |
...
por eso un nombre desconocido a la derecha de un punto lanza [TDC193](errors.mdx) en vez de
pasar.
...
<Terminal title="lo que un double dice de dos números distintos">
...
El dominio son 64 bits con signo, el mismo que la [capa compute](compute.mdx). Un RESULTADO
...
vea [Comparación y verdad](comparison.mdx), que además fija qué cuenta como verdadero para un
`if="Flag"` a secas.

:::caution[`%` es euclidiano, y eso no es lo que hace su lenguaje]
...
La razón no es de gusto. La [capa compute](compute.mdx) ya tenía `<mod>` y ya respondía 1, así
...
[TDC259](errors.mdx). La comparación de dentro es tan laxa como la de `==`, así que una
...
:::note[Dos reglas que conviene saber antes de apoyarse en ellas]
...
:::caution[`mode="sequential"` y `order="sequential"` no son lo mismo]
...
:::note[`at` cuenta desde cero y rechaza un índice que no lo es]
...
<Terminal title="cerca de cero, las definiciones no devuelven nada">
...
fila, [`preflight()`](../guides/large-outputs.mdx) estima la memoria antes de la corrida, y
...
Una [secuencia `<compute>`](compute.mdx) tiene división entera, restos, cirugía de cadenas,
