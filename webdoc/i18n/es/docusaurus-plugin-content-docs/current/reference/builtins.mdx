...
[interpolación](../core-concepts/output-formatting.mdx) (`${{_count}}`) y en las
...
Una línea [`<line each="List">`](../reference/attributes.mdx) se repite una vez por cada
elemento de una lista, y allí hay dos valores integrados más — **solo allí**; el patrón
completo está en [Tablas relacionales](../constructs/relational-tables.mdx):
...
[TDC193](errors.mdx) rechaza la configuración en vez de imprimir `${{_item}}` literalmente.
...
- **[Salida y formato](../core-concepts/output-formatting.mdx)** — interpolación e `if`.
