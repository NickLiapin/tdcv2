...
| Java        | descargar `tdcv2-%%TDC_VERSION%%-cli.jar` de Maven Central   | `java -jar tdcv2-%%TDC_VERSION%%-cli.jar pack add ru` |
...
Vale la pena un alias —`alias tdcv2='java -jar /ruta/a/tdcv2-%%TDC_VERSION%%-cli.jar'`— y a partir de
...
- **`dataPaths`** — las carpetas que el motor sí escanea en busca de paquetes. `pack add`
  ...
  apuntar el motor a [paquetes que escribió usted mismo](writing-your-own.mdx).
...
catálogo encima. %%TDC_PACK_SETS%% conjuntos no caben en una pantalla, así que se recorren con la forma
que tienen:
...
El catálogo tiene hoy **%%TDC_PACK_SETS%% conjuntos**: `common`, %%TDC_PACK_LANGUAGES%% idiomas y %%TDC_PACK_COUNTRIES%% países. Un idioma o
...
<Terminal title="tdcv2 pack list (almacén en la disposición antigua)">
