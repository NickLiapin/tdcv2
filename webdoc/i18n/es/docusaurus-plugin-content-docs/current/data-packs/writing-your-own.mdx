...
(vea [Descripción general](overview.mdx#cómo-se-forma-la-dirección)). A partir de ahí, un
...
| `generator`   | `tdc` — el cuerpo es un [`<gen>`](../generators/overview.mdx), no una lista           |
...
[`percent`](../generators/text.mdx#proporciones-exactas-con-percent). Hay dos maneras de
...
<Terminal title="./run surnames.tdc (100 000 filas)">
{`Smith      2022
Johnson    1599
Williams   1345`}
</Terminal>
...
<Terminal title="./run notices.tdc (30 filas)">
...
[`<gen>`](../generators/overview.mdx). Aquí, la placa de vehículo mexicana — el formato
...
[`<data>`](../core-concepts/output-formatting.mdx) que dice qué devolver.
...
compone. La etiqueta [`<distinct>`](../constructs/unique-values.mdx) dice que las dos
...
Un [`<mix>`](../reference/tags.mdx#distribuciones-y-selección) con `percent` funciona
...
<Terminal title="./run es-surname.tdc (100 filas)">
{`García Fernández
López
Martín Romero
Ruiz`}
...
configuración](../guides/large-outputs.mdx#qué-motor-corre-su-configuración).

Dentro de un [`<case>`](../reference/tags.mdx#distribuciones-y-selección), arme el valor
con las etiquetas mismas: [`<gen>`](../generators/overview.mdx), y
[`<data>`](../core-concepts/output-formatting.mdx) para el texto literal entre ellas.
...
:::note[Qué se permite dentro de un generador]
Ocho tipos de generador producen un valor por sí solos y se permiten en cualquier parte
del cuerpo de un paquete: [`text`](../generators/text.mdx),
[`number`](../generators/number.mdx), [`regex`](../generators/regex.mdx),
[`advanced_regex`](../generators/advanced-regex.mdx),
[`symbol`](../generators/symbol.mdx), [`date`](../generators/date.mdx),
[`increment`](../generators/counters.mdx) y [`decrement`](../generators/counters.mdx).
Dentro de un `<sequence>` también puede usar [`template`](../generators/template.mdx) para
traer una lista de datos u otro generador por dirección, junto con la distribución
[`<mix>`](../reference/tags.mdx#distribuciones-y-selección) / `percent`.
...
<Terminal title="./run pair.tdc (local=es)">
{`Saúl y Esteban
Jonás y Eloy
Josué y Ladislao`}
</Terminal>
...
Funciona tanto dentro de una [`<sequence>`](../core-concepts/sequences.mdx) (envolviendo
`<gen>`) como dentro de `<env>` (envolviendo secuencias enteras).

No lo confunda con [`uniq`](../constructs/unique-values.mdx): `uniq` impide que una fila
...
- **Sus propias carpetas** se agregan con la bandera de CLI `--data-path <carpeta>`
  ...
  [Instalar paquetes de datos](installing-packs.mdx).
...
editor](../getting-started/editor-support.mdx). Escriba una descripción que valga la pena
...
<Terminal title="tdcv2 pack info">
...
- **[Descripción general](overview.mdx)** — direcciones y uso de los paquetes.
- **[Instalar paquetes de datos](installing-packs.mdx)** — `tdcv2 init` y `tdcv2 pack`.
- **[Datos coherentes y relacionales](../guides/coherent-data.mdx)** — padre → hijo por
  nombre.
- **[Valores únicos](../constructs/unique-values.mdx)** — `<distinct>` y `uniq` a fondo.
