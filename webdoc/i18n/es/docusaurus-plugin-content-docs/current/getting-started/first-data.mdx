---
title: Su primer conjunto de datos
sidebar_position: 2
---
...
- [`<env>`](../core-concepts/configuration.mdx) con `count="3"` — generar **3**
  filas; `seed` fija la aleatoriedad para que el resultado
  [se repita](../core-concepts/determinism.mdx) de una ejecución a otra.
- [`<sequence>`](../core-concepts/sequences.mdx) — una **columna** de datos; el
  [`<gen>`](../generators/overview.mdx) que lleva adentro dice de dónde salen sus
  valores (aquí, un nombre al azar de una lista).
- [`type="text"`](../generators/text.mdx) — elegir un valor de una lista separada
  por comas.
- [`type="number"`](../generators/number.mdx) con `value="18..65"` — un número al
  azar dentro de un rango.
- [`<block>`](../core-concepts/output-formatting.mdx) / `<line>` / `<data>` — la
...
compílelo una vez. Todo esto se explica en [Instalación](installation.mdx).
...
lista completa de banderas está en la [referencia del CLI](../reference/cli.mdx).
...
- [`percent="50,50"`](../generators/text.mdx) hace que el generador
  [`text`](../generators/text.mdx) reparta más o menos mitad y mitad entre `Hombre`
  y `Mujer`, en lugar de elegir de manera uniforme.
- [`type="template"`](../generators/template.mdx) con `value="person.male.firstName"`
...
en [Dependencias jerárquicas](../guides/hierarchical-dependencies.mdx).
...
- **[Estructura de la configuración](../core-concepts/configuration.mdx)** — `<tdc>`, `<env>` y cómo se organiza una configuración.
- **[Dependencias jerárquicas](../guides/hierarchical-dependencies.mdx)** — la función estrella.
- **[Valores de plantilla](../generators/template.mdx)** — `person.*`, `date.*`, `location.*` y el resto de los datos integrados.
- **[Referencia del CLI](../reference/cli.mdx)** — la línea de comandos completa, las etiquetas, los atributos y los generadores.
