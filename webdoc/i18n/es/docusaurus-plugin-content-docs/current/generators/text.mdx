...
`case=` / `mask=` para el [formato de salida](../guides/masks-and-case.mdx) y
`order=` / `cycle=` para el [orden de los valores](overview.mdx).
...
:::caution[La coma es el separador, y no hay forma de escaparla]
...
- **Los espacios alrededor de una opción se recortan.** `value=" x , y "` da `x` e `y`,
  ...
  tiene que venir de un [archivo](file.mdx).
...
- **Una coma suelta crea una opción VACÍA.** `value="a,,b"` son tres opciones, una de
  ...
  en blanco a propósito está [`missing=`](../guides/missing-data.mdx), que lo dice.
:::
...
cada fila sea distinta, vea [Valores únicos](../constructs/unique-values.mdx).
...
`order="sequential"` — vea [Resumen de generadores](overview.mdx).

:::note[`value` se lee distinto en cada generador]
...
[`number`](number.mdx) es un rango numérico como `1..100`; en [`date`](date.mdx) es
un rango de fechas o una palabra de modo como `today`; en [`regex`](regex.mdx) es un
...
<Terminal title="./run gender.tdc (first 6 rows)">
{`Mujer
Hombre
Mujer
Hombre
...
<Terminal title="./run gender.tdc (count=100, tallied)">
{`Hombre   60
Mujer    40`}
</Terminal>
...
<Terminal title="./run grade.tdc (count=100, tallied)">
{`A   34
B   33
C   33`}
</Terminal>
...
<Terminal title="./run tier.tdc (count=100, tallied)">
{`gold     50
silver   30
bronze   20`}
</Terminal>
...
<Terminal title="./run coin.tdc (value=Heads,Tails percent=50,50 count=3)">
{`Heads
Tails
Heads`}
</Terminal>
...
[Una proporción menor que un registro](../constructs/mix.mdx#una-proporción-menor-que-un-registro).
...
Cuando la secuencia tiene un [`parent`](../core-concepts/sequences.mdx), los
...
secuencial. Vea **[Máscaras y mayúsculas](../guides/masks-and-case.mdx)**.

## Siguiente

- **[Number](number.mdx)** — enteros, rangos y cadenas de dígitos de ancho fijo.
- **[File](file.mdx)** — la misma idea de «elegir de una lista», pero la lista vive en un archivo o en una columna CSV.
