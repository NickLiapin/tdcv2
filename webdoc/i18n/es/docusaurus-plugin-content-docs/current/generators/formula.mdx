...
Una celda que [`parent=`](../core-concepts/sequences.mdx) o
[`missing=`](../guides/missing-data.mdx) dejó vacía no es un cero, y una fórmula
...
siguen [acumulado](running.mdx) y [estadística](stat.mdx) al saltarse una celda
...
elección por fila: un [`<case>`](../constructs/switch.mdx#case--una-rama-con-un-generador) de
...
cosas quedan fuera de una rama, y `check` lo dice con [`TDC295`](../reference/errors.mdx): una
...
`anomaly_factor=` se rechazan con [`TDC015`](../reference/errors.mdx) en lugar de
...
- **una fórmula donde no puede significar nada** — [`TDC295`](../reference/errors.mdx): una
...
- **Hace streaming.** Leer solo su propia fila es exactamente aquello sobre lo que está
  construido el [motor de streaming](../guides/large-outputs.mdx), así que una fórmula corre
  ...
  Esto es lo que separa una fórmula de [acumulado](running.mdx) y [estadística](stat.mdx):
