...
lista, y demasiada estructura interna para [`number`](number.mdx): letras, dígitos y
...
:::note[Un subconjunto finito y portable]
...
:::note[Aleatorio, no exacto]
`cat` y `dog` salen de forma despareja — es una elección **aleatoria**, no una proporción
exacta. Para exactamente 70/30, use [`advanced_regex`](advanced-regex.mdx).
:::
...
[Symbol](symbol.mdx#alfabetos-con-nombre-mediante-alphabet).
...
[`regex_max_length`](../reference/attributes.mdx) (por omisión **32**). Un patrón que
...
- [`<mix percent="…">`](../reference/tags.mdx) para elegir entre fragmentos del DSL;
- [`<gen type="text" percent="…">`](text.mdx) para un conjunto de valores;
- [`advanced_regex`](advanced-regex.mdx) para elección ponderada dentro del propio patrón.

## Vea también

- **[Advanced Regex](advanced-regex.mdx)** — el mismo motor más elección ponderada.
- **[Symbol](symbol.mdx)** — cuando solo se necesita un conjunto de caracteres, no
  estructura.
- [`regex_max_length`](../reference/attributes.mdx) y [`alphabet`](../reference/attributes.mdx).
