...
:::note[Un carácter, no una palabra]
`symbol` elige un solo **carácter** a la vez. Para elegir una **palabra** completa de una
lista, use [`text`](text.mdx): `<gen type="text" value="red,green,blue"/>`.
:::
...
[`number`](number.mdx#largo-variable) acepta rangos de longitud y grupos; `symbol` no.
...
El registro de alfabetos también está disponible dentro de [`regex`](regex.mdx) y
[`advanced_regex`](advanced-regex.mdx) mediante el escape `\a{name}`, así que se puede
...
- [`alphabet`](../reference/attributes.mdx) y [`length`](../reference/attributes.mdx)
  en la referencia de atributos.
- **[Regex](regex.mdx)** — cuando el string tiene estructura, no solo un conjunto de
  caracteres.
- **[Máscaras y mayúsculas](../guides/masks-and-case.mdx)** — cómo remodelar un string
  ya generado.
