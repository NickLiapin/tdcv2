...
| `secret="k7Fm2p…"`                  | el valor mismo — funciona, y advierte ([`TDC284`](../reference/errors.mdx)) |
...
- **Desde la biblioteca, solo el camino asíncrono.** Una llamada de red no puede salir de
  ...
  [`toStringAsync()` / `writeFileAsync()`](../bindings/typescript.mdx#métodos-terminales)
...
- [Códigos de error](../reference/errors.mdx) — `TDC065`–`TDC069`, `TDC284` y los fallos en corrida
