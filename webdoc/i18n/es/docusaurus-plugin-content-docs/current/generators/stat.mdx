...
`accumulate=` suma una lista dentro de un registro. [`running`](running.mdx) suma una
...
- **`stddev` es la desviación típica POBLACIONAL** — dividida por _n_, no por _n_−1. Una
  ...
  [expresiones](../reference/expressions.mdx).
...
[`parent`](../guides/hierarchical-dependencies.mdx) vació no se cuentan, y tampoco
...
la misma regla que siguen [`parent`](../guides/hierarchical-dependencies.mdx) y `running`,
...
El [motor de streaming](../guides/large-outputs.mdx) rechaza una estadística, por su
...
Lo que cuesta es el mismo límite que tiene [`running`](running.mdx#qué-motor-lo-ejecuta),
...
:::tip[Si basta con la propia fila, usa una formula]
Un estadístico es la herramienta correcta cuando una fila tiene que saber de TODAS las
filas. Cuando solo tiene que saber de sí misma, [`<gen type="formula">`](formula.mdx)
...
- [`running`](running.mdx) — la misma columna, acumulada sobre la marcha en lugar de
  resumida al final
...
- [Expresiones](../reference/expressions.mdx) — `sum`, `mean`, `median` y `stddev` también
