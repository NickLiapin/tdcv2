...
[`src`](../reference/attributes.mdx) le dice al generador dónde leerlos, y un atributo
más, [`column`](../reference/attributes.mdx), decide si ese archivo es una lista simple
o una tabla.
...
| :----------------------------------------- | :---------- | :------------------------------------------------------------------------------ |
| [`src`](../reference/attributes.mdx)       | sí          | Dónde está el archivo — ruta relativa, `@data` o absoluta                       |
| [`column`](../reference/attributes.mdx)    | no          | Lee una columna del CSV, por nombre o por número desde 1 (activa CSV)           |
| [`delimiter`](../reference/attributes.mdx) | no          | Separador de celdas en modo CSV — coma por omisión                              |
| [`header`](../reference/attributes.mdx)    | no          | Omite la primera línea cuando la columna se elige **por número**                |
| [`row`](../reference/attributes.mdx)       | no          | Liga varios campos a la **misma** línea del CSV (mantiene el registro completo) |
...
presencia de [`column`](../reference/attributes.mdx):
...
en el orden en que aparecen en el archivo (vea [`order=` / `cycle=`](overview.mdx)).

:::warning[Al pasar del final del archivo, vuelve a empezar]
...
**Herramienta.** El mismo `src`, más [`column`](../reference/attributes.mdx) — su sola
...
columna que reconocer, así que agregue [`header="true"`](../reference/attributes.mdx)
...
- Si el separador del archivo no es una coma, configure
  [`delimiter`](../reference/attributes.mdx) — de lo contrario toda la línea cae en una
...
[`weight="column"`](../reference/attributes.mdx) a uno de los campos del grupo y la fila
...
configuración](../guides/large-outputs.mdx#qué-motor-corre-su-configuración). Los grupos
ligados en sí se cubren en **[Datos coherentes y
relacionales](../guides/coherent-data.mdx)**.
...
- `row` requiere [`column`](../reference/attributes.mdx) — es una función de CSV, no de
...
[`decimals`](../reference/attributes.mdx) lo sobrescribe.
...
sembrada que usa [`uniq`](../constructs/unique-values.mdx) — y la ejecución sigue
...
- [`src`](../reference/attributes.mdx), [`column`](../reference/attributes.mdx),
  ...
  [`row`](../reference/attributes.mdx) y [`weight`](../reference/attributes.mdx) en la
  referencia de atributos.
- **[Archivos y CSV](../guides/files-and-csv.mdx)** — la guía completa para cargar datos
  externos.
- **[Datos coherentes y relacionales](../guides/coherent-data.mdx)** — cómo ligar
