...
| `local`               | Veinticuatro: `ar`, `cs`, `de`, `el`, `en`, `es`, `fi`, `fr`, `hi`, `hu`, `id`, `it`, `ja`, `ko`, `nl`, `pl`, `pt`, `ru`, `sv`, `th`, `tr`, `uk`, `vi`, `zh-cn`. Diecinueve responden también a un alias de tres letras — `ara`, `ces`, `deu`, `ell`, `eng`, `spa`, `fin`, `fra`, `hun`, `ind`, `ita`, `jpn`, `kor`, `nld`, `pol`, `por`, `tur`, `ukr`, `vie` —, y `hi`, `ru`, `sv`, `th`, `zh-cn` no tienen ninguno. Se hereda de [`<env>`](../reference/tags.mdx) si se omite |
