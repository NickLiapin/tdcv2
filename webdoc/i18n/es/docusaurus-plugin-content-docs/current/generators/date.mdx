...
| `local`               | Veinticinco: `ar`, `cs`, `de`, `el`, `en`, `es`, `fi`, `fr`, `hi`, `hu`, `id`, `it`, `ja`, `ko`, `nl`, `pl`, `pt`, `ru`, `sv`, `th`, `tr`, `uk`, `vi`, `zh-cn`, `zh-tw`. Diecinueve responden también a un alias de tres letras — `ara`, `ces`, `deu`, `ell`, `eng`, `spa`, `fin`, `fra`, `hun`, `ind`, `ita`, `jpn`, `kor`, `nld`, `pol`, `por`, `tur`, `ukr`, `vie` —, y `hi`, `ru`, `sv`, `th`, `zh-cn`, `zh-tw` no tienen ninguno (el `zh` a secas se acepta y lee la tabla tradicional). Se hereda de [`<env>`](../reference/tags.mdx) si se omite |
...
elección por fila: un [`<case>`](../constructs/switch.mdx#case--una-rama-con-un-generador), a
