---
title: Contadores (increment / decrement)
sidebar_position: 10
---
...
Los dos viven dentro de una [`<sequence>`](../core-concepts/sequences.mdx): la
secuencia se calcula una sola vez como arreglo, y cada fila de salida toma el
siguiente valor mediante la
[interpolación](../core-concepts/output-formatting.mdx) `${{Nombre}}`.
...
[`seed`](../core-concepts/determinism.mdx) (a diferencia de los generadores
aleatorios), así que la secuencia es idéntica en cada ejecución — las salidas de esta
página son exactas, no solo ilustrativas.

:::note[Bajo `parent=` los números siguen siendo 1..N, en un orden que elige la semilla]
Un contador en una [secuencia hija](../guides/hierarchical-dependencies.mdx) numera solo las
...
[`step`](../reference/attributes.mdx) define cuánto se mueve el contador entre filas.
...
la misma corrida sin importar el [`seed`](../core-concepts/determinism.mdx). Dos
...
existe el valor integrado [`${{_count}}`](../reference/builtins.mdx) — no hace falta
...
[máscara](../guides/masks-and-case.mdx), empezar en algo distinto de 1 o avanzar con
un paso distinto de 1.

## Vea también

- [Secuencias](../core-concepts/sequences.mdx) — el contenedor donde viven ambos contadores.
- [Valores integrados](../reference/builtins.mdx) — `_count`, `_first`, `_last`, `_total`.
- [Determinismo](../core-concepts/determinism.mdx) — por qué la misma semilla reproduce
