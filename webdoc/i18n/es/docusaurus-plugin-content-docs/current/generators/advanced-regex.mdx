...
**Se usa cuando** hace falta todo lo que hace [`regex`](regex.mdx), pero además la
...
El [`regex`](regex.mdx) normal elige una alternativa `(RU|US|DE)` **al azar**, así que
...
[`type="regex"`](regex.mdx) se mantiene estable y neutral respecto a TDC, mientras que
...
[`<mix>`](../reference/tags.mdx#distribuciones-y-selección) para expresarse. Con
...
<Terminal title="./run countries.tdc (100 filas, por prefijo)">
{`RU   70
US   20
DE   10`}
</Terminal>

:::note[El costo de la exactitud es memoria]
...
salida), [`<mix percent>`](../reference/tags.mdx#distribuciones-y-selección) y
[`<gen type="text" percent="…">`](text.mdx#proporciones-exactas-con-percent) dan
...
<Terminal title="./run weights.tdc (1000 filas, por prefijo)">
{`pesos            RU     US     DE
...
La diferencia se ve mejor lado a lado. El [`regex`](regex.mdx) simple elige cada
...
<Terminal title="./run coded.tdc (primeras 8 de 100 filas)">
{`RU-441627
RU-476822
RU-948319
US-450875
...
Toda construcción finita de [`regex`](regex.mdx) funciona también aquí: literales,
...
y el tope [`regex_max_length`](../reference/attributes.mdx). Un reparto ponderado
...
[alfabetos con nombre](symbol.mdx#alfabetos-con-nombre-mediante-alphabet), se pueden mantener
...
<Terminal title="./run nested.tdc (100 filas)">
{`AX   40
AY   10
B    50`}
</Terminal>
...
[jerarquía de secuencias](../core-concepts/sequences.mdx) de TDC.
...
<Terminal title="./run two.tdc (100 filas)">
{`M-99   44
F-99   31
M-00   16
F-00    9`}
...
[`parent`](../core-concepts/sequences.mdx#secuencias-dependientes-parent), los porcentajes
...
<Terminal title="./run parent.tdc (100 filas)">
{`F      50    (MaleCode vacío)
M-A    20
M-B    30`}
</Terminal>
...
que en el [`regex`](regex.mdx) normal: los primeros tres dígitos se repiten al final:
...
<Terminal title="./run branch-capture.tdc (40 filas)">
{`AB-AB   10
CD-CD   30`}
</Terminal>
...
- dentro de una [`<sequence>`](../core-concepts/sequences.mdx) — por `count`, o por un
  subconjunto `parent`;
- dentro de un `<case>` de un
  [`<mix>`](../reference/tags.mdx#distribuciones-y-selección) — por el tamaño de ese
  caso.
...
[`<line>`](../core-concepts/output-formatting.mdx) solo da formato — así que la
...
<Terminal title="./run code.tdc (primeras filas de 100)">
{`code=A-8870
code=B-2495
code=B-1961
code=A-8865
...
<Terminal title="./run segment.tdc (1000 filas, por prefijo)">
{`REG    800
VIP    150
TEST    50`}
</Terminal>
...
<Terminal title="./run doc.tdc (primeras filas de 100)">
{`924-B-924
419-B-419
788-A-788
692-B-692`}
...
<Terminal title="./run codes.tdc (100 filas, por longitud)">
{`longitud  4    85    (AB42)
...
Use [`<mix>`](../reference/tags.mdx#distribuciones-y-selección) cuando las ramas tienen
...
`\k<name>` repite el grupo por ese nombre, igual que en [`regex`](regex.mdx), y es el
...
`advanced_regex` y rehacer la columna como un [`<switch>`](../reference/tags.mdx).

```xml
<gen type="advanced_regex" value="(?<sex>(?%{50:male;50:female}))/(?if{sex=male:Mr;sex=female:Ms})"/>
```

<Terminal title="./run titles.tdc (count=8, seed=titles)">
{`female/Ms
male/Mr
male/Mr
female/Ms
...
<Terminal title="./run plates.tdc (count=10, seed=plates)">
{`RU-683
US-NGS
DE-ZQ5
US-VNS
...
- **No lee nada más que un grupo con nombre** — ni otra columna, ni `${{…}}`. Para
  lógica entre columnas están [`<switch>`](../reference/tags.mdx) y el filtro
  [`parent`](../core-concepts/sequences.mdx#secuencias-dependientes-parent); esto es
...
- **[Regex](regex.mdx)** — las construcciones finitas que hereda esta página.
- **[Symbol](symbol.mdx#alfabetos-con-nombre-mediante-alphabet)** — los alfabetos con nombre
  (`\a{name}`).
- [`regex_max_length`](../reference/attributes.mdx) en la referencia de atributos.
- [`<mix>`](../reference/tags.mdx#distribuciones-y-selección) — porcentajes exactos
