...
[`<data>`](../core-concepts/output-formatting.mdx), déle a la salida un nombre `.parquet`
...
- **Sin NULL.** Un espacio vacío entre dos comas: ¿es «texto vacío» o «no había valor»?
  [`missing`](../reference/attributes.mdx) emite un string vacío, y la distinción se
  pierde.
...
<Figure
  ...
  alt=""
  caption="Cómo queda acomodado en disco un archivo tipado. Esquemático."
/>

<Legend items={[["A", "un row group: una rebanada de filas que se arma, se escribe y se libera"], ["B", "la primera columna de esa rebanada, guardada por separado"], ["C", "la segunda columna"], ["D", "la tercera — un lector que necesita una sola columna toca únicamente sus trozos"]]} />

## Cómo activarlo

Son dos cosas: marcar cuáles etiquetas
[`<data>`](../core-concepts/output-formatting.mdx) son columnas, y nombrar el archivo
`.parquet`.
...
<Terminal title="./run data.tdc -o data.parquet   (esquema + primeras filas)">
{`id          INT64       REQUIRED
...
[`missing`](../reference/attributes.mdx) por fin llegando como debe ser. Y `is_outlier`
es un `BOOLEAN` real: una columna marcadora de
[`anomaly_flag`](../reference/attributes.mdx), es decir, un dataset etiquetado listo para
...
:::note[`decimal` nunca redondea en silencio]
`decimal(18,2)` con el valor `123.456` es un **error**, no un centavo perdido.
:::

:::note[Los flotantes angostos pierden precisión a propósito]
...
<Terminal title="./run inferred.tdc -o inferred.parquet   (esquema inferido)">
{`id     INT64                REQUIRED
...
Las reglas son simples: un [`number`](../generators/number.mdx) sin `decimals` → entero;
con `decimals` → flotante; un contador [`increment`](../generators/counters.mdx) →
entero; `common.id.uuid` → UUID; una columna marcadora de
[`anomaly_flag`](../reference/attributes.mdx) → booleano, y también una columna
[`<mix flag=>`](../constructs/mix.mdx). Y muy convenientemente,
[`missing`](../reference/attributes.mdx) **hace la columna nullable por sí solo** (`qty`
quedó como `OPTIONAL`).
...
| :-------- | :-------------- |
| [`timeseries`](../generators/timeseries.mdx), [`pattern`](../generators/pattern.mdx) | entero, o decimal con `decimals` |
| [`running`](../generators/running.mdx) | el tipo de la columna que nombra su `of=` |
| [`stat`](../generators/stat.mdx) | `count` → entero; `mean`, `median`, `stddev` → decimal; `sum`, `min`, `max` → el tipo del origen |
| [`formula`](../generators/formula.mdx) | entero o decimal **cuando se da `decimals=`**, texto en caso contrario |
| [`file` con `read="quantile"`](../generators/file.mdx) | decimal, o entero con `decimals="0"` |
| [`increment` / `decrement`](../generators/counters.mdx) | entero, o decimal cuando `value=` o `step=` es fraccionario |
| [`<mix>`](../constructs/mix.mdx) | el tipo compartido cuando **cada** rama es un solo `<gen>` que deriva a él; `string` ante cualquier desacuerdo, o cuando una rama tiene texto literal. Una columna `<mix flag=>` es booleana |
...
Una [lectura por cuantiles](../generators/file.mdx) es la única fila de esa tabla donde el
...
- **Un [`date`](../generators/date.mdx) sin `format="YYYY-MM-DD"`.** Por omisión una
  fecha se imprime como `05/25/1996`, que no es ISO — declararla como `date` sería
  deshonesto.
- **Una [`mask`](masks-and-case.mdx) o un [`case`](masks-and-case.mdx) en el generador.**
...
[salidas grandes](large-outputs.mdx).
...
Una condición: hace falta un archivo real ([`-o`](../reference/cli.mdx)) — es la extensión
...
aterrizó cada grupo. Fije el número de hilos con [`--jobs`](../reference/cli.mdx) si
quiere; los bytes son los mismos de cualquier forma.

## Listas

Una columna puede guardar una **lista de valores** en vez de uno solo:
`type="[]int64"`, o simplemente [`repeat`](../reference/attributes.mdx) en el generador —
...
<Terminal title="ls -lh   (sin compresión vs. ahora)">
...
[Bindings de lenguajes](../bindings/python.mdx).
...
- **`MAP` y estructuras anidadas** dentro de una columna — las listas ya existen (vea
  [`repeat`](../reference/attributes.mdx)), pero `MAP` y las listas de listas no están
  soportadas.
...
- **[Formatos de salida (CSV, JSON, SQL…)](output-formats.mdx)** — el lado de texto del
  mismo bloque de salida, y dónde muerde la sintaxis de cada formato.
- **[Salida y formato](../core-concepts/output-formatting.mdx)** — `<block>`, `<line>` y
  `<data>` a detalle.
- **[Salidas grandes y streaming](large-outputs.mdx)** — row groups, `--jobs` y memoria
  plana a cualquier tamaño.
- **[CLI](../reference/cli.mdx)** — `-o`, `--jobs`, `--engine`.
- **[Máscaras y mayúsculas](masks-and-case.mdx)** — `mask` / `case`, que apagan la
  inferencia de tipos.
- **[Bindings de lenguajes](../bindings/python.mdx)** — leer y escribir desde las cinco:
