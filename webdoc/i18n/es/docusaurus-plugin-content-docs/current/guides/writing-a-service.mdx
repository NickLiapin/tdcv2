---
title: Escribir un generador de servicio
sidebar_position: 12
---
...
El [generador `http`](../generators/http.mdx) deja que un servicio escrito por usted
...
El contrato completo está en la [página del generador](../generators/http.mdx#el-contrato-que-implementa-su-servicio).
...
El generador `http` es el único lugar donde TDC [cede su garantía](../generators/http.mdx#lo-que-no-promete):
...
<Terminal title="./run demo.tdc — dos veces">
...
La configuración añade [`secret=`](../generators/http.mdx#probar-que-la-petición-viene-de-tdc);
...
- [El generador `http`](../generators/http.mdx) — los atributos, el contrato, los modos de fallo
- [Códigos de error](../reference/errors.mdx) — `TDC065`–`TDC069` y `TDC284`
