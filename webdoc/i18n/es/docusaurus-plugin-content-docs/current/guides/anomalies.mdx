...
[`missing`](../reference/attributes.mdx) dejó en blanco ya no tiene pico que describir, así
...
La bandera necesita una secuencia cuyo valor sea ese único `<gen>`. Añada al cuerpo una segunda parte — otro `<gen>`, un literal `<data>` o un `name=` que convierta el generador en un campo — y no queda dónde poner una columna por fila, así que la configuración se rechaza ([`TDC283`](../reference/errors.mdx)) en lugar de ejecutarse sin ella. Mueva el generador a su propia `<sequence>`; así también obtiene el valor como columna propia.
