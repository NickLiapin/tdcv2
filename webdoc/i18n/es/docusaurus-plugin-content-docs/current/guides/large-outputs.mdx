...
| `uniq="true"` sobre un [contador](../generators/counters.mdx)                        | en memoria      | crece con `count` |
| `uniq="true"` sobre una secuencia compuesta (campos `<gen>` con nombre)              | exacto en disco | acotada en ~1 GB  |
| Un grupo [`<uniq>`](../constructs/unique-values.mdx) a nivel de env                  | en memoria      | crece con `count` |
