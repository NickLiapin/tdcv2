...
:::note[TDC y `tdcv2` — dos nombres, dos cosas]
...
Cubre TDC **%%TDC_VERSION%%**, última actualización **%%TDC_UPDATED%%** — la fecha del
...
:::note[Por qué los ejemplos dicen `xml`]
...
- **[Un reparto que existe antes que las filas](pools/overview.mdx).** Un `<pool>` construye
...
- **[Una columna que sigue a otra](generators/formula.mdx).** Un peso que sigue a una
  ...
  distribución](guides/statistical-distributions.mdx#un-parámetro-puede-seguir-a-otra-columna)
  también puede ser una expresión. De columnas independientes un modelo no puede aprender
  nada; estas se mueven juntas.

- **[La forma de una muestra real](generators/file.mdx#readquantile--una-muestra-medida-como-distribución).**
...
- **[Un valor sin configuración](getting-started/quick-api.mdx).** `tdc.person.lastName()`:
...
- **Los valores sueltos son todo lo que va a necesitar.** TDC también los responde
  ...
  [el API de un valor](getting-started/quick-api.mdx)—, pero un faker dedicado trae un
