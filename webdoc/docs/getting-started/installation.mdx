...
All five are published, at the same version: **%%TDC_VERSION%%** on npm, PyPI, Maven
Central, NuGet and crates.io. Equal version numbers are not a coincidence — they
mean the same engine, so `tdcv2 %%TDC_VERSION%%` from any one of them answers a config the
same way.
...
java -jar tdcv2-%%TDC_VERSION%%-cli.jar init
```
...
| Java           | `java -jar tdcv2-%%TDC_VERSION%%-cli.jar tdcv2-examples/01-starter.tdc` — the `cli` classifier of the library's own coordinates            |
