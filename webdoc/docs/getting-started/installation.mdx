...
All five are published. npm, PyPI, NuGet and crates.io carry **%%TDC_VERSION%%**;
Maven Central carries **%%TDC_JAVA_VERSION%%**, because it caps how many releases it
accepts from a project each month and the jar waits for the next allowance — see
[the Java page](../bindings/java.mdx). Equal version numbers are not a coincidence:
where they match, `tdcv2 %%TDC_VERSION%%` from any of them answers a config the same
