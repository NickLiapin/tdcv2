...
:::note[TDC and `tdcv2` — two names, two things]
...
Covers TDC **%%TDC_VERSION%%**, last updated **%%TDC_UPDATED%%** — the date of the newest
...
[Editor support](getting-started/editor-support.mdx).
...
- **[A cast that exists before the rows do](pools/overview.mdx).** A `<pool>` builds a
...
- **[A column that follows another one](generators/formula.mdx).** A weight that follows
  ...
  and a [distribution parameter](guides/statistical-distributions.mdx#a-parameter-can-follow-another-column)
  can be an expression too. Independent columns are what a model cannot learn anything
  from; these move together.

- **[The shape of a real sample](generators/file.mdx#readquantile--a-measured-sample-as-a-distribution).**
...
- **[One value, without a config](getting-started/quick-api.mdx).** `tdc.person.lastName()`
...
- **[Locale and country packs](data-packs/overview.mdx).** Generate data for people,
  places, medical records, and documents in %%TDC_PACK_LANGUAGES%% languages. Country packs
  also support national ID formats for %%TDC_PACK_COUNTRIES%% countries, with the
...
- **Loose values are all you will ever need.** TDC answers those too —
  ...
  [the one-value API](getting-started/quick-api.mdx) — but a dedicated faker carries a
...
| **[Java](bindings/java.mdx)**             | Maven Central | `io.github.nickliapin:tdcv2` | %%TDC_JAVA_VERSION%%   |

Every published package carries a starter set of data packs, so it works with
nothing else installed; the other %%TDC_PACK_LANGUAGES%% languages and
%%TDC_PACK_COUNTRIES%% country packs are
[a download away](data-packs/installing-packs.mdx).
