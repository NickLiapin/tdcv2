...
- **[One value, without a config](core-concepts/quick-api.mdx).** `tdc.person.lastName()`
...
- **Loose values are all you will ever need.** TDC answers those too —
  ...
  [the one-value API](core-concepts/quick-api.mdx) — but a dedicated faker carries a
