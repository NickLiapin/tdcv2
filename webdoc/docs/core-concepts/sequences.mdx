...
[generator](../generators/overview.mdx) fills every row, and as each output row is
rendered you pull a value out by name, either with
[interpolation](output-formatting.mdx) `${{Name}}` or inside an `if` condition.
...
values are the same either way — see [Large outputs](../guides/large-outputs.mdx).
...
<Figure
  ...
  alt=""
  caption="Six rows of a real run. Each sequence fills its own column, and a row is what you get by reading across them."
/>

<Legend items={[["A", "one sequence — one column, filled independently"], ["B", "a second sequence, which knows nothing about the first"], ["C", "a third"], ["D", "one row: the same position taken from every column and assembled into a line of output"]]} />
...
Every sequence needs at least one [`<gen>`](../generators/overview.mdx) child — that's
...
[`seed`](determinism.mdx) fixes exactly which ones you get. For strict list order instead,
add `order="sequential"`, described in [Masks & case](../guides/masks-and-case.mdx).
...
[`text`](../generators/text.mdx) generator a `percent` list and the values come out in
exact proportions:
...
[Determinism & proportions](determinism.mdx) for how it's enforced.
...
:::tip[Why not `<gen type="text" value="import-2026"/>`]
...
- All the fields share the **same
  [`parent`](../guides/hierarchical-dependencies.mdx) filter**. If a parent is set,
...
`<gen>` draws from the [PRNG](determinism.mdx) independently. That has one consequence
worth remembering:
...
guarantee, see [Determinism & proportions](determinism.mdx).
...
- **[`<mix>`](../constructs/mix.mdx)** — a distribution by exact percent, where each
  branch can be a whole compound of literals and generators, not just a single value.
- **[`<switch>`](../constructs/switch.mdx)** — a deterministic key → value lookup table.
...
- **[Generators](../generators/overview.mdx)** — the `<gen>` that produces a
  sequence's values.
- **[Determinism & proportions](determinism.mdx)** — how `seed`, `count`, and
  `percent` fit together.
- **[Hierarchical dependencies](../guides/hierarchical-dependencies.mdx)** — the full
  `parent` model.
