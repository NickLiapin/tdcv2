...
[`order="sequential"`](../core-concepts/output-formatting.mdx) so the values come out in
...
<Terminal title="./run age.tdc (6 rows)">
{`1. age 15
2. age 17
3. age 18
4. age 25
...
Now hang `if="Age >= 18"` on the [`<line>`](../reference/tags.mdx) itself. When the
...
counter still reads `3, 4, 5, …`: [`_count`](../reference/builtins.mdx) is the record's
place in the whole set, decided before rendering, so suppressing a line doesn't
renumber the rest.

:::warning[Write `<`, `>`, and `&` literally]
...
| :------------------------------------------------------ | :--------------------------------------------------------------------------------------- |
| [`<line>`](../reference/tags.mdx)                       | The entire line is suppressed — including the between-row separator.                     |
| [`<data>`](../reference/tags.mdx)                       | Just that text chunk is suppressed; the other `<data>` on the same line still print.     |
| [`<gen>`](../generators/overview.mdx) in a `<sequence>` | Makes a **conditional sequence** (below). Generators aren't allowed in the output block. |
...
<Terminal title="./run gendered-name.tdc (6 rows)">
{`Female: Emma
Male:   Daniel
Female: Sophia
Female: Olivia
...
[Coherent & relational data](../guides/coherent-data.mdx) and
...
- **The `X.Value` shorthand** — if `X` is a sequence and `X.Value` is not itself a
  ...
  [`parent="X.Value"`](../core-concepts/sequences.mdx).
...
operator. [Comparison and truth](../reference/comparison.mdx) is the full account.
...
<Terminal title="./run age-bands.tdc (6 rows)">
{`age 15: under18
age 17: under18
age 18: adult
age 25: adult
...
<Terminal title="./run roles.tdc (6 rows)">
{`1. guest: [regular]
2. user: [regular]
3. admin: [admin]
4. user: [regular]
...
[Comparison and truth](../reference/comparison.mdx) covers the corners: what counts as a
...
<Terminal title="./run logical.tdc (6 rows)">
{`1. guest/15: flagged
2. user/17: flagged
...
<Terminal title="./run arithmetic.tdc (6 rows)">
{`1/6 age 15:
2/6 age 17:
3/6 age 18:
...
<Terminal title="./run dotted.tdc (6 rows)">
{`1. guest: [!Role.admin]
2. user: [!Role.admin]
3. admin: [Role.admin]
4. user: [!Role.admin]
...
[built-in sequences](../reference/builtins.mdx) `_first` / `_last`, which are stored
as the literal strings `"true"` / `"false"`, behave intuitively in `if`.

:::warning[A column holding `0` is TRUE]
...
The [comparison rules](../reference/comparison.mdx) explain why `"0"` is not the number 0
...
<Terminal title="./run json-list.tdc (4 rows)">
{`--- HEAD ---
{"id": 1},
{"id": 2},
{"id": 3},
...
half of a set. See [Built-in sequences](../reference/builtins.mdx) for the full list.
...
<Terminal title="./run combined.tdc (6 rows)">
...
:::note[Compiled once, cheap to reuse]
...
- [Built-in sequences](../reference/builtins.mdx) — `_count`, `_first`, `_last`,
  `_total`.
- [`if` in the attribute reference](../reference/attributes.mdx).
- [Coherent & relational data](../guides/coherent-data.mdx) and
  [Hierarchical dependencies](../guides/hierarchical-dependencies.mdx) — where conditional
  sequences shine.
