...
A branch can also compute from the row it is on: a [`formula`](../generators/formula.mdx#in-a-branch)
or a [date offset](../generators/date.mdx#in-a-branch) reads that row's columns and nothing else.
The constructs that read the whole run — [`running`](../generators/running.mdx),
[`stat`](../generators/stat.mdx), a formula with `prev()` — and a
[pool reference](../pools/overview.mdx) have to be sequences of their own (`TDC295`, `TDC268`).
...
:::danger[The denominator is small, so a small share can round away to nothing]
...
with [`TDC251`](../reference/errors.mdx) before the run. The worked example is
[A share smaller than one record](./mix.mdx#a-share-smaller-than-one-record).
:::
...
The same `<case>` sits inside [`<mix>`](./mix.mdx), so a nested `<switch>` works there too:
the mix decides whether a record is an invoice, the switch decides which tax it names.

:::note[A share inside a nested branch does not stream]
