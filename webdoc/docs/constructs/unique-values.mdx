---
title: Uniqueness (uniq, distinct)
sidebar_position: 7
---
...
<Figure
  ...
  alt=""
  caption="How often each combination of two fields came out. Across: the first field; down: the second."
/>

<Legend items={[["A", "with distinct, over 60 rows: the diagonal is empty, because a row can never repeat a value across its fields"], ["B", "with uniq, over 6 rows: no cell is ever above 1, because a combination can never repeat across rows — the empty cells are combinations this run simply didn't reach"]]} />

## `<distinct>` — different within a row

Two [`<gen>`](../generators/overview.mdx) fields that draw from the **same** list
...
**1. Inside a [`<sequence>`](../core-concepts/sequences.mdx)** — it wraps the
`<gen name="…">` fields. Read it as "A ≠ B", here over the real symptom list from the
[data pack](../data-packs/overview.mdx):
...
<Terminal title="./run case.tdc (6 rows)">
...
**2. Inside [`<env>`](../core-concepts/configuration.mdx)** — it wraps whole
...
- **At the `<env>` level the group takes single-value sequences only** — a plain
  `<gen>`, a [`<mix>`](mix.mdx) or a [`<switch>`](switch.mdx). A compound (multi-field)
  sequence there is rejected with error `TDC129`.

## `uniq` — the combination never repeats

`uniq="true"` on a **compound** [`<sequence>`](../core-concepts/sequences.mdx) means
...
of each there are. So a [`percent`](../generators/text.mdx) list stays exact —
...
:::note[Keep a comfortable margin]
The maximum number of unique combinations is bounded by the product of the number of
**distinct** values in each field. When a field draws randomly ([`text`](../generators/text.mdx)
...
Only single-value sequences (a plain [`<gen>`](../generators/overview.mdx), a
...
[Which engine runs your config](../guides/large-outputs.mdx#which-engine-runs-your-config)
has the full routing, including the four non-`uniq` shapes that land in memory too.

:::danger[Exact `uniq` on a huge output is SLOW — `uniq` + `percent` most of all]
...
too — exact, but bounded by RAM. See **[Large outputs](../guides/large-outputs.mdx)**.

## See also

- **[Sequences](../core-concepts/sequences.mdx)** — compound sequences and fields,
  the structures `uniq` and `<distinct>` operate on.
- **[Determinism & proportions](../core-concepts/determinism.mdx)** — why `uniq`
