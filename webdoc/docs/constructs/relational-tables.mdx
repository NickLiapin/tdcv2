...
Two pieces do it. [`order="sequential"`](../reference/attributes.mdx) makes the `repeat`
list walk the values in the order they are written instead of drawing them, and
[`parent`](../guides/hierarchical-dependencies.mdx) picks which path this record takes:
