...
:::note[The shorter spelling, and when it is the right one]
One sequence can hold the whole path — `<gen type="text"
value="created,paid,shipped,delivered" repeat="4" order="sequential"/>` — with
[`each`](../reference/attributes.mdx) to unroll it. A fixed `repeat` on a walked list
...
<Terminal title="./run steps.tdc (3 orders)">
{`1000,created
1000,paid
1000,shipped
1000,delivered
...
walked list can say that. See [`order=` / `cycle=`](../generators/overview.mdx) for the
