...
:::warning[Two generators do not go through this layer]
...
- **An empty `from` is refused.** `replace:,+` and `replace` on their own have nothing to
  look for, so [TDC275](../reference/errors.mdx) stops the run rather than letting the
...
deliberately ignored rather than [refused](../reference/errors.mdx).
