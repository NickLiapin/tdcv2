---
title: Output formats (CSV, JSON, SQL…)
sidebar_position: 5
---
...
[output block](../core-concepts/output-formatting.mdx) assembles **text** —
[`<before>`](../core-concepts/output-formatting.mdx) / [`<after>`](../core-concepts/output-formatting.mdx)
wrap the whole run, [`<block>`](../core-concepts/output-formatting.mdx) is one record, and
[`<data>`](../core-concepts/output-formatting.mdx) is the raw text — so the same engine
...
A header belongs in [`<before>`](../core-concepts/output-formatting.mdx): it prints exactly
...
<Terminal title="./run out.tdc (a comma inside a value)">
...
**Tool.** The [`csv`](masks-and-case.mdx) filter wraps a field in quotes and doubles any
...
<Terminal title="./run out.tdc -o out.csv (semicolon-delimited)">
{`id;name;category
1;Pen;Office
2;Mug;Kitchen`}
</Terminal>

(The [`delimiter`](files-and-csv.mdx) attribute is a separate thing — it tells the
[`file`](../generators/file.mdx) generator how to **read** an input CSV. On the output side
...
The first one uses a condition. A second [`<data if="!_last">`](../core-concepts/output-formatting.mdx)
...
The second one uses a dedicated fixture. [`<delimiter_block>`](../core-concepts/output-formatting.mdx)
...
Several values in one field come from [`repeat`](../reference/attributes.mdx), and
[`separator`](../reference/attributes.mdx) is what goes between them. JSON wants `", "` —
...
then turn that into JSON with the [`replace`](masks-and-case.mdx) filter, where quotes
**are** allowed:
...
is enough on its own. [`repeat`](../reference/attributes.mdx) draws each value
independently, so a tag can show up twice in one row; if you don't want that, pin a fixed
set with [`<mix>`](../reference/tags.mdx) instead.

### `null` is a word, not emptiness

**Problem.** For an optional field, [`missing`](../reference/attributes.mdx) is the obvious
...
**Tool.** Write the literal word `null` as one branch of a [`<mix>`](../reference/tags.mdx),
...
[file](files-and-csv.mdx). A product called `Coffee "Arabica" 250g` lands in the string
...
[`replace`](masks-and-case.mdx) filter does it — this is the one place you write the
backslash yourself:
...
[`<delimiter_line>`](../core-concepts/output-formatting.mdx) does. The per-line fixtures see the
...
:::note[The wrapper holds no values]
...
[Large outputs & streaming](large-outputs.mdx) for how TDC writes these to disk without
...
**Tool.** The [`sql`](masks-and-case.mdx) filter doubles the apostrophe, which is how SQL
...
The `CREATE TABLE` runs once, so it belongs in [`<before>`](../core-concepts/output-formatting.mdx),
...
valid — generate one line per list element with [`each`](../reference/attributes.mdx). See
[Coherent & relational data](coherent-data.mdx) for the full pattern.
...
- **A fixed-width report** — pad the fields to a set width with the
  [`mask`](masks-and-case.mdx) and slice filters, then line the columns up.
...
<Terminal title="./run out.tdc (Markdown table)">
{`| id | name  | city    |
|----|-------|---------|
| 1  | James | Chicago |
| 2  | Mary  | Austin  |`}
...
- **[Masks & case](masks-and-case.mdx)** — the `csv` / `sql` / `replace` escaping filters
  in full.
- **[Reading files & CSV](files-and-csv.mdx)** — pull your own values from a file with the
  [`file`](../generators/file.mdx) generator.
- **[Coherent & relational data](coherent-data.mdx)** — related tables, foreign keys, and
  the `each` / `weight` / `row` attributes.
- **[Large outputs & streaming](large-outputs.mdx)** — millions of rows and NDJSON on disk.
