...
| `uniq="true"` on a [counter](../generators/counters.mdx)                 | exact on-disk | bounded            |
| `uniq="true"` on a compound sequence (named `<gen>` fields)              | exact on-disk | bounded            |
| An env-level [`<uniq>`](../constructs/unique-values.mdx) group           | exact on-disk | bounded            |
...
:::caution[The check has a ceiling of its own]
