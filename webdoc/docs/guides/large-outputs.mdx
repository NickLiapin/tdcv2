...
| `uniq="true"` on a [counter](../generators/counters.mdx)                 | exact on-disk | grows with `count` |
| `uniq="true"` on a compound sequence (named `<gen>` fields)              | exact on-disk | bounded at ~1 GB   |
| An env-level [`<uniq>`](../constructs/unique-values.mdx) group           | exact on-disk | grows with `count` |
