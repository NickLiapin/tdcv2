...
| `uniq="true"` on a [counter](../generators/counters.mdx)                 | in-memory     | grows with `count` |
| `uniq="true"` on a compound sequence (named `<gen>` fields)              | exact on-disk | bounded at ~1 GB   |
| An env-level [`<uniq>`](../constructs/unique-values.mdx) group           | in-memory     | grows with `count` |
