...
- **The exact on-disk engine** — turns itself on for **any** uniqueness:
  [`uniq="true"`](../constructs/unique-values.mdx) on a sequence, or a `<uniq>` group.
...
[`uniq="true"`](../constructs/unique-values.mdx) over a finite text list and env-level
...
[`text`](../generators/text.mdx) fields/sequences, uniform (no `percent` on the columns),
