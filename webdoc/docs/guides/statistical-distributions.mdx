...
A plain [`<gen type="number" value="150..200"/>`](../generators/number.mdx) draws
...
<Terminal title="./run life.tdc (8 rows)">
...
may be an [expression](../reference/expressions.mdx) over the row's other columns
...
goes on, and [`_count`](../reference/builtins.mdx) is the row number:
...
:::note[Why this is allowed, when a per-row `repeat=` is not]
...
the property the [streaming engines](large-outputs.mdx) are built on. A per-row
[`repeat=`](../constructs/multiple-values.mdx) count would change the number of draws,
...
<Terminal title="./run uniform.tdc (300 rows)">
{`150 | ############################# 27
154 | ####################### 21
158 | ############################### 29
163 | ######################## 22
...
<Terminal title="./run normal.tdc (300 rows)">
{`157 | ### 5
161 | ############# 19
165 | ###################### 33
169 | ####################################### 57
...
<Figure
  ...
  alt="A flat histogram beside a bell-shaped one over the same range"
  caption="The same 150–200 range, generated 20,000 times each way and counted."
/>

<Legend items={[["faint", "a plain range: every height about as likely as any other"], ["made", "the same range with a normal distribution: a hill around the center"]]} />
...
<Figure
  ...
  alt="Histograms of the nine supported distributions"
  caption="Nine runs of 8000 values each, counted. This is what the names actually look like."
/>

<Legend items={[["made", "20,000 generated values, counted into buckets"]]} />
...
<Terminal title="./run lognormal.tdc (300 rows)">
{` 15633 | ######################################## 114
 41886 | ####################################### 110
 68140 | ################## 51
 94393 | ###### 17
...
<Terminal title="./run exponential.tdc (300 rows)">
{` 0 | ############################ 70
 1 | ######################################## 98
 2 | ################### 47
 3 | ################ 38
...
<Terminal title="./run pareto.tdc (300 rows)">
{` 1002 | ######################################## 259
 4234 | #### 23
 7466 | # 8
10698 | # 5
...
<Terminal title="./run weibull.tdc (300 rows)">
{`  30 | ########################## 57
 358 | ######################################## 88
 687 | ##################### 47
1015 | ######################## 52
...
<Terminal title="./run poisson.tdc (300 rows)">
{` 0 | #### 5
 1 | ############ 17
 2 | #################################### 50
 3 | ####################################### 53
...
<Terminal title="./run zipf.tdc (300 rows)">
{` 1 | ######################################## 174
11 | ########## 42
20 | ###### 27
30 | ### 12
...
<Terminal title="./run gamma.tdc (300 rows)">
{`  71 | ############################ 53
 803 | ######################################## 77
1534 | ############################# 56
2266 | ############################# 55
...
<Terminal title="./run beta.tdc (300 rows)">
{`0 | ######################## 35
0 | ######################################## 59
0 | ################################# 49
0 | ############################### 46
...
<Terminal title="./run age-raw.tdc (300 rows)">
{`-16 | ### 6
 -4 | ############### 27
  7 | ############################## 52
 19 | ################################## 60
...
<Terminal title="./run age-clipped.tdc (300 rows)">
{` 0 | ################################# 46
10 | ################################# 46
20 | ####################################### 53
30 | ######################################## 55
...
<Figure
  ...
  alt="A bell, and the same bell clipped to bounds"
  caption="The same generator, unbounded and bounded. Values that fall outside are pinned to the nearest bound, so the edges gain a spike."
/>

<Legend items={[["made", "left: no bounds; right: the same run bounded to 160 and 180"]]} />
...
[Large outputs](../guides/large-outputs.mdx)). A billion lognormal "salaries" is no
problem.
...
- **[Number](../generators/number.mdx)** — the range and digit-string modes this
  attribute replaces.
- **[Large outputs](../guides/large-outputs.mdx)** — why distributions stay cheap at a
  billion rows.
- **[Coherent data](../guides/coherent-data.mdx)** — keeping generated fields consistent
  within a row.
