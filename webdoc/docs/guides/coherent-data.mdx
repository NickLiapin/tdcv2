---
title: Coherent & relational data
sidebar_position: 2
---
...
Two [sequences](../core-concepts/sequences.mdx): a brand and a model. The model
declares [`parent="Brand"`](../core-concepts/sequences.mdx) (so it sees the chosen
brand) and interpolates it into the [`template`](../generators/template.mdx) address
with `${{Brand}}`:
...
- **Declare the parent before the child** — TDC materializes
  [sequences](../core-concepts/sequences.mdx) top to bottom, so `${{Brand}}` reads a
...
- **`parent="Brand"`** links the child to the parent and fixes the order. For a plain
  ...
  is covered in [Hierarchical dependencies](hierarchical-dependencies.mdx).
...
- **Engine.** A config like this always runs on the in-memory engine — it's the only one
  ...
  config](large-outputs.mdx#which-engine-runs-your-config) lists this and the five other
...
with [`row`](../generators/file.mdx) instead: several [`file`](../generators/file.mdx)
...
Full details are on the [File generator](../generators/file.mdx) page.
...
That is [`formula`](../generators/formula.mdx):
...
[A parameter can follow another column](statistical-distributions.mdx#a-parameter-can-follow-another-column).

## See also

- **[Hierarchical dependencies](hierarchical-dependencies.mdx)** — filtering a child on a specific parent value.
- **[Sequences](../core-concepts/sequences.mdx)** — declaring fields and the `parent` link.
- **[Template](../generators/template.mdx)** and **[File](../generators/file.mdx)** generators.
