...
[sequences](../core-concepts/sequences.mdx) together in a **parent → child**
...
<Figure
  ...
  alt=""
  caption="40 real rows. The parent takes A or B; each parent value has its own child alphabet, and no row ever mixes them."
/>

<Legend items={[["drawn", "rows where the parent chose A — the child below is always 1, 2, or 3"], ["alt", "rows where the parent chose B — the child below is always 7 or 8"]]} />

## The problem: independent fields make impossible pairs

To see why [`parent`](../reference/attributes.mdx) matters, look at what happens
**without** it. Two [`text`](../generators/text.mdx) sequences, country and city,
...
The `Country` column is unchanged (same [`seed`](../core-concepts/determinism.mdx)),
...
1. Declare the **parent** with a distribution
   ([`text`](../generators/text.mdx) with [`percent`](../generators/text.mdx),
   [`template`](../generators/template.mdx), or any type).
...
3. The child materializes **only on rows where `Parent == Value`**. On every other row
   ...
   [interpolation](../core-concepts/output-formatting.mdx).
4. Any percentages inside the child are measured against the number of **active**
   rows — the filtered subset.

:::note[The value compared is the one the parent DREW]

`parent="Parent.Value"` is answered before the
[formatting layer](../guides/masks-and-case.mdx) runs, so it compares against the value
...
- `Gender` at `count="8"` yields 5 `Male` + 3 `Female` (60/40 of eight, rounded by the
  largest-remainder method — see [`percent`](../core-concepts/determinism.mdx)). At
...
[`percent`](../generators/text.mdx) on the child and it applies to the filtered rows:
...
:::caution[`missing=` on the parent does not stop the child]
...
that blanks some of its rows with [`missing=`](../guides/anomalies.mdx) still counts
...
[`if` expressions](../reference/attributes.mdx) evaluate against the current row's
...
[`text.paragraph`](../generators/template.mdx) and does not have to make conversational
sense for the tree to be a valid tree.

## See also

- **[Sequences](../core-concepts/sequences.mdx)** and
  [`parent`](../reference/attributes.mdx) — the mechanics behind the filter.
- **[Determinism & proportions](../core-concepts/determinism.mdx)** — how
  [`seed`](../core-concepts/determinism.mdx) and
  [`percent`](../core-concepts/determinism.mdx) stay exact.
- **[Coherent & relational data](coherent-data.mdx)** — parent → child by name lookup.
