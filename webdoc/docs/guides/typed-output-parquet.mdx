...
just text that a reader has to guess at. Name your [`<data>`](../core-concepts/output-formatting.mdx)
...
- **No NULL.** Nothing between two commas — is that "empty text" or "no value at all"?
  [`missing`](../reference/attributes.mdx) emits an empty string, and the distinction
  is lost.
...
<Figure
  ...
  alt=""
  caption="How a typed file is laid out on disk. Schematic."
/>

<Legend items={[["A", "a row group: a slice of rows, assembled, written, and released"], ["B", "the first column of that slice, stored on its own"], ["C", "the second column"], ["D", "the third — a reader that needs one column touches only its chunks"]]} />

## Turning it on

Two things: mark which [`<data>`](../core-concepts/output-formatting.mdx) tags are
...
<Terminal title="./run data.tdc -o data.parquet   (schema + first rows)">
{`id          INT64       REQUIRED
...
`OPTIONAL`), not an empty string — [`missing`](../reference/attributes.mdx) finally
coming through the way it should. And `is_outlier` is a real `BOOLEAN`: a marker column
from [`anomaly_flag`](../reference/attributes.mdx), which gives you a labeled dataset
...
:::note[`decimal` never rounds silently]
`decimal(18,2)` with the value `123.456` is an **error**, not a lost cent.
:::

:::note[Narrow floats lose precision on purpose]
...
<Terminal title="./run inferred.tdc -o inferred.parquet   (inferred schema)">
{`id     INT64                REQUIRED
...
The rules are simple: a [`number`](../generators/number.mdx) without `decimals` → integer,
with `decimals` → float; an [`increment`](../generators/counters.mdx) counter → integer;
`common.id.uuid` → UUID; an [`anomaly_flag`](../reference/attributes.mdx) marker column →
boolean, and so is a [`<mix flag=>`](../constructs/mix.mdx) column. And handily,
[`missing`](../reference/attributes.mdx) **makes the column nullable
...
| :-------- | :---------- |
| [`timeseries`](../generators/timeseries.mdx), [`pattern`](../generators/pattern.mdx) | integer, or float with `decimals` |
| [`running`](../generators/running.mdx) | the type of the column its `of=` names |
| [`stat`](../generators/stat.mdx) | `count` → integer; `mean`, `median`, `stddev` → float; `sum`, `min`, `max` → the source's type |
| [`formula`](../generators/formula.mdx) | integer or float **when `decimals=` is given**, text otherwise |
| [`file` with `read="quantile"`](../generators/file.mdx) | float, or integer with `decimals="0"` |
| [`increment` / `decrement`](../generators/counters.mdx) | integer, or float when `value=` or `step=` is fractional |
| [`<mix>`](../constructs/mix.mdx) | the shared type when **every** branch is a single `<gen>` that derives to it; `string` on any disagreement, or when a branch holds literal text. A `<mix flag=>` column is boolean |
...
A [quantile read](../generators/file.mdx) is the one row on that table where the file
...
- **A [`date`](../generators/date.mdx) without `format="YYYY-MM-DD"`.** By default a date
  prints as `05/25/1996`, which isn't ISO — calling that a `date` would be dishonest.
- **A [`mask`](masks-and-case.mdx) or [`case`](masks-and-case.mdx) on the generator.**
...
streaming behavior that keeps [large outputs](large-outputs.mdx) flat in memory.
...
One condition: it needs a real file ([`-o`](../reference/cli.mdx)) — the `.parquet`
...
landed.
Set the thread count with [`--jobs`](../reference/cli.mdx) if you like; the bytes are the
same either way.

## Lists

A column can hold a **list of values** rather than one: `type="[]int64"`, or just
[`repeat`](../reference/attributes.mdx) on the generator, in which case the type is
...
<Terminal title="ls -lh   (no compression vs. now)">
...
the full library API in each language, see [Language bindings](../bindings/python.mdx).
...
<Terminal title="./run metrics.tdc -o metrics.parquet">
...
- **[Output formats (CSV, JSON, SQL…)](output-formats.mdx)** — the text side of the same
  output block, and where each format's syntax trips you up.
- **[Output & formatting](../core-concepts/output-formatting.mdx)** — `<block>`, `<line>`,
  and `<data>` in full.
- **[Large outputs & streaming](large-outputs.mdx)** — row groups, `--jobs`, and flat
  memory at any size.
- **[CLI](../reference/cli.mdx)** — `-o`, `--jobs`, `--engine`.
- **[Masks & case](masks-and-case.mdx)** — `mask` / `case`, which switch type inference off.
- **[Language bindings](../bindings/python.mdx)** — read and write from all five:
