...
boolean. And handily, [`missing`](../reference/attributes.mdx) **makes the column nullable
...
| [`file` with `read="quantile"`](../generators/file.mdx) | **text** — declare `type="float"` when you want a numeric column |
...
A [quantile read](../generators/file.mdx) is the one row on that table where the value
...
One condition: it needs a real file ([`-o`](../reference/cli.mdx)). Parquet written to
...
- **`MAP` and nested structures** inside a column — lists already exist (see
  [`repeat`](../reference/attributes.mdx)), but `MAP` and lists-of-lists aren't supported.
