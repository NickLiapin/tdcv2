...
- **Weights on members.** A pool has no per-member weight. Use a
  [`<mix>`](../constructs/mix.mdx) inside the pool — that is the same thing said in the
...
- [Overview](overview.mdx) — what a pool is, and the size ceiling
- [Narrowing with `filter`](filter.mdx) — the expression language a link is written in
- [Hierarchical dependencies](../guides/hierarchical-dependencies.mdx) — `parent`, the
