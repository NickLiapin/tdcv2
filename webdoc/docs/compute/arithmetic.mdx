...
Integer math inside [`<compute>`](overview.mdx). Every operation is its own tag; nest
them to build an expression tree. These are the arithmetic operators of the TDC
language — they live in a [`<sequence>`](../core-concepts/sequences.mdx) right next to
[`<gen>`](../generators/overview.mdx), both in your own config and inside the built-in
identifier packs.
...
Inside [`<reduce>`](lists.mdx), `<add>` accumulates a result:
[`<current/>`](lists.mdx) is the next digit (a one-character string, so it coerces on
its own) and [`<acc/>`](lists.mdx) is the running total. No `<to_number>` is needed
...
<Terminal title="./run digitsum.tdc — ${{Pin}} -> sum ${{DigitSum}}">
{`4821 -> sum 15
3067 -> sum 16
9145 -> sum 19
5530 -> sum 13
...
a check digit (Luhn, ISBN, IBAN). Inside [`<reduce>`](lists.mdx), each digit —
[`<current/>`](lists.mdx), a one-character string that coerces on its own — is
multiplied by a weight pulled from a list by position with [`<at>`](lists.mdx) and
[`<current_index/>`](lists.mdx):
...
<Terminal title="./run weighted.tdc — ${{Base}} -> weighted ${{Weighted}}">
...
<Terminal title="./run mod.tdc — ${{N}}: last=${{Last}}, parity=${{Parity}}">
...
example is in the [compute overview](overview.mdx).
...
<Figure
  ...
  alt=""
  caption="One border, two crossings: one you write, one that happens on its own."
/>

<Legend items={[["A", "the string side: a value from a <field>, digits and all"], ["B", "the crossing you have to write — <to_number>"], ["C", "the crossing that happens on its own: a number placed in <concat> becomes its digits"], ["D", "the number side, where arithmetic lives"]]} />
...
([`<field name="…"/>`](overview.mdx)) and `${{…}}` values are **strings**. Arithmetic
...
numbers" from being confused with "glue as strings" ([`<concat>`](strings.mdx)).
...
<Terminal title="./run bad.tdc — error before generation">
...
([`<reduce>`](lists.mdx), [`<each>`](lists.mdx)), the element
[`<current/>`](lists.mdx) is a **one-character** string and can be added or multiplied
...
field value, a `${{…}}`, or a chunk from [`<slice>`](strings.mdx).
...
[`<current/>`](lists.mdx) inside a walk, [`<slice>`](strings.mdx), or a literal
`<str v="A"/>`.
...
Walk a string character by character with [`<each>`](lists.mdx), encode each in
`base36`, and glue the results with [`<join>`](lists.mdx):
...
- **Tree errors are caught before the run** (codes `TDC180`–`TDC187`): an unknown tag,
  a wrong child count, an unbound [`<var>`](overview.mdx), and the like.

## See also

- **[Lists & iteration](lists.mdx)** — sum or weight a whole list with `<reduce>`.
- **[Conditionals](conditionals.mdx)** — branch on `<greater_than>` / `<mod>` in a check digit.
- **[Compute overview](overview.mdx)** — the full Luhn / check-digit example.
- **[Compute functions reference](../reference/compute.mdx)** — the alphabetical catalog.
