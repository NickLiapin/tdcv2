...
80 is mid, otherwise high". [`<compute>`](overview.mdx) expresses every one of these
...
<Figure
  ...
  alt=""
  caption="Asked about 7, the first test holds; the branches below it are never reached."
/>

<Legend items={[["A", "the value the branches are asked about"], ["B", "the tests, tried in the order they are written"], ["C", "the value of the first branch that held — the faint ones are never reached"]]} />
...
[`<each>`](lists.mdx) walks the digits, [`<current/>`](lists.mdx) is the digit being
...
<Figure
  ...
  frame
  zoom
  caption="The same fork on the Studio canvas — the numbers on the edges are the order the branches are tried in, and the tail is branch three."
/>
...
[`<to_number>`](arithmetic.mdx) with [`<let>`](overview.mdx) and compared as an integer.
...
[`<reduce>`](lists.mdx) adds the digits, and the single `<when>` inside `<choose>` picks
the answer.
...
[`<mod>`](arithmetic.mdx) remainder against zero and yields yes/no; `3005` is odd
...
characters of a mixed alphanumeric code. Here [`<slice>`](strings.mdx) takes the first
...
`<valid>` is a **reject-and-retry** check used in generator [packs](../data-packs/writing-your-own.mdx).
...
- Comparisons are on **integers** — a numeric string is converted with
  [`<to_number>`](arithmetic.mdx) first (as in the grade and parity examples). There are
  no floats.
...
- **[Compute overview](overview.mdx)** — where conditionals fit in a full checksum.
- **[Arithmetic](arithmetic.mdx)** — the `<mod>`, `<subtract>`, and `<to_number>` used
  in the tests above.
- **[Lists & iteration](lists.mdx)** — `<each>` and `<reduce>`, the loops a per-digit
  `<choose>` runs inside.
- **[Compute functions reference](../reference/compute.mdx)** — the full tag catalog.
