...
- **A string iterates one character at a time.** A
  [`<field name="Base"/>`](../core-concepts/sequences.mdx) holding `"5120"` walks as
...
<Figure
  ...
  alt=""
  caption="One <do>, run once per element: four elements in, four out."
/>

<Legend items={[["A", "the list <over> walks — one element per column"], ["B", "the <do> expression, the same one at every step, drawn dashed because it is one tag and not four"], ["C", "the list <each> gives back, in the same order"]]} />
...
[`<add>`](arithmetic.mdx) accepts it without [`<to_number>`](arithmetic.mdx). [`<mod>`](arithmetic.mdx) with `10` keeps the
...
character with [`<encode>`](arithmetic.mdx), or build the list of terms a checksum will
later sum.
...
<Figure
  ...
  frame
  zoom
  caption="The same fold on the Studio canvas — the three slots hang off the fold itself, while the accumulator and the current element arrive as context of the walk."
/>
...
<Figure
  ...
  alt=""
  caption="Summing the digits of 4816. The jar is handed from step to step; the digit above joins it."
/>

<Legend items={[["A", "the characters <over> walks, one per step, numbered from 0 underneath"], ["B", "what <init> puts in the jar before the first step, drawn dashed because it is a starting value and not a step"], ["C", "the jar after each step — this is what <acc/> reads on the next one"], ["D", "the jar after the last step, which is what <reduce> gives back"]]} />
...
sum into a check digit with a double [`<mod>`](arithmetic.mdx) (`% 11`, then `% 10`) and
[`<concat>`](strings.mdx) it onto the base:
...
[`<choose>`](conditionals.mdx) to keep whichever is greater, the digit or the accumulator:
...
[`<to_number>`](arithmetic.mdx) to make the list numeric:
...
grouping of a single string, [`<group>`](strings.mdx) is simpler; `<join>` works on list
elements.
...
[`repeat=`](../generators/overview.mdx) arrives as its elements glued into a single string,
...
- The pieces are **strings**, like every other piece of text in this layer. Wrap one in
  [`<to_number>`](./arithmetic.mdx) to do arithmetic with it.
...
<Terminal title="./run order.tdc (3 rows)">
{`5|1|1 @ 22|14|77 -> 201
4|3|3 @ 58|91|62 -> 691
2|5|2 @ 42|13|72 -> 293`}
</Terminal>
...
<Terminal title="./run tags.tdc (1 row)">
...
- The index is coerced to an `int`: a one-character digit string (a field holding `"3"`)
  works as is; wrap a multi-digit string in [`<to_number>`](arithmetic.mdx) first.
...
- **Out of range:** if the `default` attribute is set, it's returned; without `default`,
  ...
  an **integer**, including over a list of [`<str>`](strings.mdx). `default="Unknown"` passes
  ...
  [`<choose>`](conditionals.mdx#choose--pick-the-first-matching-branch) that tests the index instead.
...
digit count — `<length>` supplies the denominator. [`<divide>`](arithmetic.mdx) is integer
...
- **[Arithmetic](arithmetic.mdx)** — the integer operations you fold and index with.
- **[Strings & formatting](strings.mdx)** — `<concat>`, `<group>`, and the rest.
- **[Conditionals](conditionals.mdx)** — `<choose>` and the predicates used above.
- **[Compute overview](overview.mdx)** — `<reduce>` inside a full checksum.
- **[Compute functions reference](../reference/compute.mdx)** — the full alphabetical catalog.
