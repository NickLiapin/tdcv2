...
page is a value for [`type="template"`](../generators/template.mdx); the
[`value`](./attributes.mdx) attribute is a **dotted path** that picks which identifier you
get.

This page is the full lookup. For a guided tour of the same generator — person data,
dates, localization — see the [`template` generator](../generators/template.mdx). For the
...
Every value below comes from a fixed [`seed`](../core-concepts/determinism.mdx), so it's
...
[`<gen>`](./tags.mdx). Any parameter you leave out is drawn at random; the ones you set
...
A parameter is just a local [`<sequence name="…">`](../core-concepts/sequences.mdx)
inside the [data pack](../data-packs/overview.mdx) behind the path, and the caller
...
**declaratively** with the [`<compute>`](./compute.mdx) tag, right next to the data. If a
country changes its rules, you edit the pack's text file, not the engine. The bodies of
these identifiers are drawn with the [`regex`](../generators/regex.mdx) and
[`number`](../generators/number.mdx) generators, and then `<compute>` appends the check
digit. See [Data Packs](../data-packs/overview.mdx) for how a pack is structured, and
[Writing your own pack](../data-packs/writing-your-own.mdx) for how to add one.

## See also

- **[`template` generator](../generators/template.mdx)** — the guided tour, plus the person, date, and localization families.
- **[Generators reference](./generators.mdx)** — every `type` for `<gen>`.
- **[`<compute>` reference](./compute.mdx)** — how the check digits are defined.
- **[Data Packs](../data-packs/overview.mdx)** — where identifier data lives, and how to add your own.
