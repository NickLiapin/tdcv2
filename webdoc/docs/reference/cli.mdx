...
:::note[Where `tdcv2` comes from, before the first release]
...
[Installation](../getting-started/installation.mdx) has the tab for each. Java is the one
...
[Installing packs](../data-packs/installing-packs.mdx) — and `tdcv2 format`
([below](#tdcv2-format)).
