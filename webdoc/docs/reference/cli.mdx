...
[Installation](../getting-started/installation.mdx) has the tab for each. An alias makes
...
[Installing packs](../data-packs/installing-packs.mdx) — plus `tdcv2 check`
...
| Option                  | What it does                                        |
| :---------------------- | :-------------------------------------------------- |
| `-o, --output <path>`   | Write to a file. Without it, output goes to stdout. A path ending in `.parquet` selects the [Parquet](../guides/typed-output-parquet.mdx) writer — the only way to get it |
...
- **`--mode memory`** — the small in-RAM engine, an escape hatch for small datasets and
  ...
  ([determinism](../core-concepts/determinism.mdx)); it simply holds every column in RAM
...
A [Parquet](../guides/typed-output-parquet.mdx) run reports too, once per row group of
...
- **[Installing packs](../data-packs/installing-packs.mdx)** — `tdcv2 init`, `tdcv2 pack`.
- **[Large outputs](../guides/large-outputs.mdx)** — `--jobs`, `--mode`, `--engine` in depth.
