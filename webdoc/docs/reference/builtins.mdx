...
[interpolation](../core-concepts/output-formatting.mdx) (`${{_count}}`) and in `if`
...
A [`<line each="List">`](../reference/attributes.mdx) repeats once per element of a list,
and two more built-ins are available **only there** — see
[Relational tables](../constructs/relational-tables.mdx) for the full pattern:
...
[TDC193](errors.mdx) refuses the config rather than printing `${{_item}}` literally.
...
- **[Output & formatting](../core-concepts/output-formatting.mdx)** — interpolation and `if`.
