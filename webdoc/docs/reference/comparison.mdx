...
a data pack produces the text `Miller`, and every [built-in](builtins.mdx) is text as well.
...
1. If both sides are **whole numbers**, compare them as whole numbers. Exactly — a 19-digit
   ...
   [Whole numbers](expressions.mdx#whole-numbers).
...
:::caution[`0` is true, and that is the one to remember]
...
`_first`, `_last` and every [`anomaly_flag`](../guides/anomalies.mdx) column hold the literal
...
nowhere else — see [TDC259](errors.mdx).
