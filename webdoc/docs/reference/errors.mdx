...
| `TDC294` | `<gen type="formula">` without `expr=`, or with an `expr=` that does not parse, or a `decimals=` outside 0..10 | A formula IS its expression, so there is nothing to compute without one. The expression is the same little language as `if=` — see [Expressions](expressions.mdx). A name inside it that is not a column declared above raises `TDC240` instead, the same code `running` and `stat` use for the same rule |
