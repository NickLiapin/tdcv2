...
`case=` / `mask=` for [output formatting](../guides/masks-and-case.mdx) and
`order=` / `cycle=` for [value order](overview.mdx).
...
:::caution[The comma is the separator, and there is no escape]
...
- **A bare comma makes an EMPTY option.** `value="a,,b"` is three options, one of which
  ...
  purpose use [`missing=`](../guides/missing-data.mdx), which says so.
:::
...
see [Unique values](../constructs/unique-values.mdx).
...
[Determinism](../core-concepts/determinism.mdx).

For strict list order instead of random picks, add
`order="sequential"` — see [Generators overview](overview.mdx).

:::note[`value` reads differently per generator]
`value` is every generator's main input, but its **meaning depends on `type`**.
In `text` it's a list of whole words; in [`number`](number.mdx) it's a numeric
range like `1..100`; in [`date`](date.mdx) it's a date range or a mode word
like `today`; in [`regex`](regex.mdx) it's a pattern. Same attribute, different
...
[A share smaller than one record](../constructs/mix.mdx#a-share-smaller-than-one-record)
...
When the sequence has a [`parent`](../core-concepts/sequences.mdx), the percentages
...
- **[Number](number.mdx)** — integers, ranges, and fixed-width digit strings.
- **[File](file.mdx)** — the same "pick from a list" idea, but the list lives in a file or CSV column.
