...
A cell that [`parent=`](../core-concepts/sequences.mdx) or [`missing=`](../guides/missing-data.mdx)
...
[running](running.mdx) and [stat](stat.mdx) follow when they skip an emptied cell,
...
[`<case>`](../constructs/switch.mdx#case--a-branch-with-a-generator) of a `<switch>` or a
...
out of a branch, and `check` says so with [`TDC295`](../reference/errors.mdx): a formula that
...
`anomaly_factor=` are refused with [`TDC015`](../reference/errors.mdx) rather than
...
- **a name that is not a column declared above** — [`TDC240`](../reference/errors.mdx),
  ...
  arithmetic.
- **a formula where it cannot mean anything** — [`TDC295`](../reference/errors.mdx): one that
...
- **It streams.** Reading only its own row is exactly what the
  [streaming engine](../guides/large-outputs.mdx) is built on, so a formula runs there like
  ...
  This is what separates a formula from [running](running.mdx) and [stat](stat.mdx): those
