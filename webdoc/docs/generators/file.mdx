...
| :----------------------------------------- | :------- | :------------------------------------------------------------------ |
| [`src`](../reference/attributes.mdx)       | yes      | Where the file is — relative, `@data`, or absolute path             |
...
appear in the file (see [`order=` / `cycle=`](overview.mdx)).

:::warning[Past the end of the file, it starts again]
...
config](../guides/large-outputs.mdx#which-engine-runs-your-config). Linked groups
themselves are covered in **[Coherent & relational
data](../guides/coherent-data.mdx)**.
...
[`decimals`](../reference/attributes.mdx) overrides it when you want something else.
...
seeded permutation [`uniq`](../constructs/unique-values.mdx) uses — and the run
...
and asking for two at once is [`TDC297`](../reference/errors.mdx).
