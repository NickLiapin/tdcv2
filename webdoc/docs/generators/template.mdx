...
[`value`](../reference/attributes.mdx) attribute is a **dotted path** that selects
...
With [`text`](text.mdx) you'd **type** names by hand — a short list that repeats and
...
name is pulled to match it through [`parent`](../core-concepts/sequences.mdx). The
final line is assembled in a [`<data>`](../core-concepts/output-formatting.mdx) block:
...
[catalogue](../data-packs/catalogue.mdx) for the list. Any other locale gets
[TDC217](../reference/errors.mdx), which names the locales that do ship
the path rather than letting the run guess.

:::note[Why `lastName` mixes two pools]
...
<Terminal title="./run patient.tdc (local=en)">
...
the key in [`parent`](../core-concepts/sequences.mdx), so a locale switch changes the
key you match on:
...
<Terminal title="./run gender.tdc (localization: en vs ru)">
{`local="en"     local="ru"
Male           мужчина
Male           мужчина
Female         женщина
...
The path doesn't change — only [`local`](../core-concepts/configuration.mdx) on
...
<Terminal title="./run names.tdc (localization: en vs ru)">
{`local="en"           local="ru"
...
:::note[Every locale that has a pack]
...
<Terminal title="./run csv.tdc — row 2 now has three fields, not two">
{`1,Mongolia
...
[`csv` filter](../core-concepts/output-formatting.mdx) quotes the value, and quoting is
...
<Terminal title="./run csv.tdc — the same three rows, and row 2 is one field again">
{`1,"Mongolia"
2,"Korea, Republic of (South Korea)"
3,"India"`}
</Terminal>
...
[Data packs](../data-packs/overview.mdx) covers what a given country carries.
...
[`date` generator](date.mdx#formatting-the-output).
...
:::note[The same birthday, two defaults]
`person.b_day` and [`<gen type="date" value="birth">`](date.mdx#a-birthday-with-valuebirth)
...
refused with [TDC073](../reference/errors.mdx) rather than silently misread.
...
[Reference](../reference/generators.mdx).
...
[`<gen>`](../reference/tags.mdx). Any parameter you leave out is drawn at random; the
...
accepts is defined by the [data pack](../data-packs/overview.mdx) behind it: each local
...
**declaratively** with the [`<compute>`](../reference/compute.mdx) tag, right next to the
data. If a country changes its rules, you edit the pack's text file, not the engine. See
[Data Packs](../data-packs/overview.mdx) for how a pack is structured.
...
- **[Date](date.mdx)** — the format tokens these date templates use.
- **[`<compute>`](../reference/compute.mdx)** — how the checksums are defined.
- **[Reference: generators](../reference/generators.mdx)** — the full identifier catalog.
- **[Data Packs](../data-packs/overview.mdx)** — where template data comes from, and adding your own.
