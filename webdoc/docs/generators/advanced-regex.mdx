...
**Use it when** you need everything [`regex`](regex.mdx) does, but the shape of the
...
Plain [`regex`](regex.mdx) picks an alternation `(RU|US|DE)` **randomly**, so the
...
[`type="regex"`](regex.mdx) generator stays stable and TDC-neutral, while
...
digits" would take several sequences or a [`<mix>`](../reference/tags.mdx#distributions-and-choice)
...
<Terminal title="./run countries.tdc (100 rows, by prefix)">
{`RU   70
US   20
DE   10`}
</Terminal>

:::note[The cost of exactness is memory]
...
memory, any output size), [`<mix percent>`](../reference/tags.mdx#distributions-and-choice)
and [`<gen type="text" percent="…">`](text.mdx#exact-proportions-with-percent) give
...
<Terminal title="./run weights.tdc (1000 rows, by prefix)">
{`weights          RU     US     DE
...
The difference is easiest to see side by side. Plain [`regex`](regex.mdx) picks each
...
<Terminal title="./run coded.tdc (first 8 of 100 rows)">
{`RU-441627
RU-476822
RU-948319
US-450875
...
Every finite construct from [`regex`](regex.mdx) works here too — literals, escapes,
...
[`regex_max_length`](../reference/attributes.mdx) cap. A simple weighted split over
plain Latin codes:
...
[named alphabets](symbol.mdx#named-alphabets-with-alphabet), you can hold exact
...
<Terminal title="./run nested.tdc (100 rows)">
{`AX   40
AY   10
B    50`}
</Terminal>
...
[sequence-hierarchy](../core-concepts/sequences.mdx) philosophy exactly.
...
<Terminal title="./run two.tdc (100 rows)">
{`M-99   44
F-99   31
M-00   16
F-00    9`}
...
is filtered by [`parent`](../core-concepts/sequences.mdx#dependent-sequences-parent), the
...
<Terminal title="./run parent.tdc (100 rows)">
{`F      50    (MaleCode empty)
M-A    20
M-B    30`}
</Terminal>
...
plain [`regex`](regex.mdx) — the first three digits are echoed at the end:
...
<Terminal title="./run branch-capture.tdc (40 rows)">
{`AB-AB   10
CD-CD   30`}
</Terminal>
...
- inside a [`<sequence>`](../core-concepts/sequences.mdx) — from `count`, or from a
  `parent` subset
- inside a `<case>` of a [`<mix>`](../reference/tags.mdx#distributions-and-choice) —
...
[`<line>`](../core-concepts/output-formatting.mdx) only formats — so weighted choice
...
<Terminal title="./run code.tdc (first rows of 100)">
{`code=A-8870
code=B-2495
code=B-1961
code=A-8865
...
<Terminal title="./run segment.tdc (1000 rows, by prefix)">
{`REG    800
VIP    150
TEST    50`}
</Terminal>
...
<Terminal title="./run doc.tdc (first rows of 100)">
{`924-B-924
419-B-419
788-A-788
692-B-692`}
...
<Terminal title="./run codes.tdc (100 rows, by length)">
{`length  4    85    (AB42)
...
Use [`<mix>`](../reference/tags.mdx#distributions-and-choice) when the branches have
...
`\k<name>` repeats the group by that name, exactly as in [`regex`](regex.mdx), and it
...
rebuilding the column as a [`<switch>`](../reference/tags.mdx).
...
- **Read anything but a named group** — not another column, not `${{…}}`. For
  cross-column logic use [`<switch>`](../reference/tags.mdx) or a
  [`parent`](../core-concepts/sequences.mdx#dependent-sequences-parent) filter;
...
- **[Regex](regex.mdx)** — the finite constructs this page inherits.
- **[Symbol](symbol.mdx#named-alphabets-with-alphabet)** — the named alphabets
  (`\a{name}`).
- [`regex_max_length`](../reference/attributes.mdx) in the attribute reference.
- [`<mix>`](../reference/tags.mdx#distributions-and-choice) — exact percentages
