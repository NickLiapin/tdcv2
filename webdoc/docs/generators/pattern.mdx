...
Every [`<gen>`](../reference/generators.mdx) with `type="pattern"` takes exactly
...
<Figure
  ...
  panel
  alt="line-input.png — a single wavy stroke on transparency"
  caption="line-input.png — the whole file. The white backing is only here so the stroke is visible; the file itself is transparent."
/>
...
<Figure
  ...
  alt="Generated values land exactly on the drawn curve"
  caption="Every row lands exactly on the stroke — a single line carries no randomness at all."
/>

<Legend
  items={[["drawn", "the line drawn in the file"], ["made", "generated values (40 rows)"]]}
/>
...
<Figure
  ...
  panel
  alt="curve-input.svg — one cubic Bezier inside a flipping transform"
  caption="curve-input.svg — one path, one transform."
/>

<Figure
  src="/img/pattern/svg.svg"
  alt="Generated values follow the Bezier exactly, with the transform applied"
  caption="The curve is followed as drawn: the Bezier is walked, the transform applied, the y axis flipped back."
/>

<Legend
  items={[["drawn", "the Bezier from the file, with its transform applied"], ["made", "generated values (40 rows)"]]}
/>
...
<Figure
  ...
  alt="Two readings per column: meeting on one pixel gives an exact value, standing apart gives a band"
  caption="The same two readings, on a single line and on a band. Nothing else is inspected."
/>

<Legend
  items={[["A", "the readings meet on one pixel — the value is exact"], ["B", "the readings stand apart — the value is random between them"], ["mark", "the readings themselves: top-down and bottom-up"]]}
/>
...
- The readings land **apart** → that column is a **band**. A row falling there gets a
  random value between the two edges, fixed by the [`seed`](../core-concepts/determinism.mdx).
...
<Figure
  ...
  panel
  alt="tunnel-input.png — two strokes forming a band"
  caption="tunnel-input.png — two strokes, nothing else."
/>

<Figure
  src="/img/pattern/tunnel.svg"
  alt="300 generated values scattered inside the drawn band"
  caption="Every value lands inside the band. Where the band is wide the spread is wide; where it narrows, so does the data."
/>

<Legend
  items={[["band", "the band drawn in the file"], ["made", "generated values (300 rows)"], ["A", "one column: the value is taken between its edges"]]}
/>
...
<Figure
  ...
  panel
  alt="car-input.png — a filled car silhouette"
  caption="car-input.png — a silhouette, painted solid."
/>

<Figure
  src="/img/pattern/car.svg"
  alt="Generated values fill the car silhouette"
  caption="The values fill the body between its top and bottom edges — and where the drawing is a single line (the ground before and after the car), they collapse to one exact value."
/>

<Legend
  items={[["band", "the drawn car"], ["made", "generated values (300 rows)"], ["A", "a wheel lowers the bottom edge of the body"], ["B", "the ground before and after the car is a single line, so the values are exact"]]}
/>
...
<Figure
  ...
  panel
  alt="split-input.png — one line that forks into two branches halfway"
  caption="split-input.png — a single stroke that becomes two."
/>

<Figure
  src="/img/pattern/split.svg"
  alt="Exact values before the fork, random values inside the band after it"
  caption="Before the fork every row gets the same exact value; after it, a random one inside the widening band."
/>

<Legend
  items={[["drawn", "the lines from the file"], ["made", "generated values (60 rows)"], ["A", "the line forks here"], ["B", "before the fork — the same exact value every time"], ["C", "after it — random inside the widening band"]]}
/>

The same run on two different seeds shows it plainly — the first half is identical,
the second half is not:

<Terminal title="./run split.tdc (11 rows, two seeds)">
...
Both are reported by [`check`](../reference/cli.mdx) as well as by the run: `points`,
...
<Terminal title="./run yrange.tdc (the same triangle, 11 rows)">
...
<Figure
  ...
  alt="The same triangle under three different y_range settings"
  caption="One drawing, three scales. The shape is identical; only the numbers on the axis change."
/>

<Legend
  items={[["drawn", "the same drawn triangle"], ["made", "generated values (25 rows)"]]}
/>
...
`fit` is **refused beside `points` / `upper` / `lower`** ([`TDC300`](../reference/errors.mdx)).
...
<Terminal title='./run fit.tdc (the same corridor, typed and drawn, seed="fit", 8 rows)'>
...
:::note[Higher on the picture means larger in value]
...
<Terminal title="./run decimals.tdc (11 rows)">
...
<Terminal title="./run saw.tdc (5 teeth into 5 rows)">
{`0   50   100   50   0`}
</Terminal>
...
<Figure
  ...
  alt="A zigzag read at 300 rows and at 6 rows"
  caption="The same zigzag. With rows to spare, every tooth comes through; with seven rows, each one reads the place its own line crosses the drawing, and the teeth in between are simply not among the seven readings asked for."
/>

<Legend
  items={[["drawn", "the drawn zigzag"], ["made", "generated values"], ["A", "300 rows — the teeth come through"], ["B", "7 rows — seven crossings, and the rest fall between them"]]}
/>
...
<Terminal title="./run kink.tdc (11 rows, three modes)">
...
<Figure
  ...
  alt="linear follows the polyline, smooth rounds the corners, step holds each value"
  caption="linear rides the polyline exactly; smooth rounds every corner without ever leaving the drawn range; step holds each point until the next."
/>

<Legend
  items={[["dash", "what was drawn: a polyline through five points"], ["made", "generated values (41 rows)"]]}
/>
...
<Terminal title="./run ramp.tdc (11 rows)">
...
<Figure
  ...
  alt="The drawn line with a ±6 band around it and generated values scattered inside"
  caption="One drawn line, a tunnel you never drew. The shape is untouched; each row just sits somewhere within ±6 of it."
/>

<Legend
  items={[["dash", "the drawn line — the center"], ["band", "the ±6 corridor you never drew"], ["made", "generated values (60 rows)"]]}
/>
...
<Figure
  ...
  panel
  alt="threshold-input.png — a black stroke and a gray stroke on white"
  caption="threshold-input.png — two strokes on an opaque canvas."
/>

<Figure
  src="/img/pattern/threshold.svg"
  alt="At 0.5 only the black stroke is read; at 0.8 both are, and a corridor appears"
  caption="At the default cutoff the gray stroke is background and the reading is a single exact line. Raise the cutoff and gray counts as ink too — now there are two edges, so the values fall randomly between them."
/>

<Legend
  items={[["drawn", "the black stroke"], ["faint", "the light gray stroke"], ["made", "generated values (60 rows)"]]}
/>
...
<Figure
  ...
  panel
  alt="hump-input.png — a filled bell-shaped hump"
  caption="hump-input.png — a hump painted with a brush, saved on transparency."
/>
...
<Figure
  ...
  alt="The drawn hump above, the histogram of generated values below, aligned on the same value axis"
  caption="Top: what was drawn. Bottom: how often each value actually came up. The horizontal axis is the same in both — and it is the value, not the row number."
/>

<Legend
  items={[["A", "what is drawn in the file"], ["B", "histogram of 6000 generated values"], ["C", "the height of the drawing here = how often that value comes up"]]}
/>
...
<Figure
  ...
  alt="Two humps of different heights, and the matching histogram"
  caption="Two humps, the right one twice as tall. The histogram below reproduces both, in proportion."
/>

<Legend
  items={[["A", "what is drawn: two humps of different height"], ["B", "histogram of 6000 generated values"], ["C", "the left hump"], ["D", "the right one, twice as tall — and twice as many values"]]}
/>
...
- **Deterministic:** where the drawing is a single line, a row's value is the
  ...
  — the scatter is fixed by the [`seed`](../core-concepts/determinism.mdx). Same seed
...
- **Any size, either engine:** each row is computed from its own number, so memory
  ...
  [streaming engines](../guides/large-outputs.mdx).
...
<Terminal title="./run spike.tdc (20 rows)">
...
- **[Time series](timeseries.mdx)** — when the shape is trend + season + noise.
- **[Number](number.mdx)** — for ranges and simple random integers.
- **[Determinism & proportions](../core-concepts/determinism.mdx)** — how `seed` fixes the scatter.
- **[Large outputs & streaming](../guides/large-outputs.mdx)** — millions of rows from one small drawing.
