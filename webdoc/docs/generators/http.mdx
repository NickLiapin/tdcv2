...
| `secret="k7Fm2p…"`                  | the value itself — works, and warns ([`TDC284`](../reference/errors.mdx)) |
...
- **From the library, only the async path.** A network call cannot come out of a
  ...
  Use [`toStringAsync()` / `writeFileAsync()`](../bindings/typescript.mdx#terminal-methods)
...
- [Error codes](../reference/errors.mdx) — `TDC065`–`TDC069`, `TDC284`, and the run-time failures
