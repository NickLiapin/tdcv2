...
| `local`               | `ar`, `de`, `el`, `en`, `es`, `fr`, `it`, `pl`, `pt`, `ru`, `zh-cn`. Nine of them also answer to a three-letter alias — `ara`, `deu`, `ell`, `eng`, `spa`, `fra`, `ita`, `pol`, `por` — and `ru` and `zh-cn` have none. Inherited from [`<env>`](../reference/tags.mdx) if omitted |
