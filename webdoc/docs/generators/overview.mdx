...
Add a fixed [`repeat`](../constructs/multiple-values.mdx) and the row takes **N values
...
<Terminal title="./run steps.tdc (3 rows)">
...
record's [whole lifecycle](../constructs/relational-tables.mdx) in one sequence. Over a
...
[length quota](../constructs/multiple-values.mdx) has no such number. **A walked `date`**
