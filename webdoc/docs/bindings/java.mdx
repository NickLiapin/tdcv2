...
The method names mirror the [TypeScript API](typescript.mdx) — see
[the same names everywhere](same-names.mdx) for the table, which the test suites check
...
at a time](../getting-started/quick-api.mdx) is the whole surface.
