...
[the same names everywhere](same-names.mdx). Python's own `to_list`, `rows`, `uses_http`,
...
implementations. [One value at a time](../getting-started/quick-api.mdx) is the whole
surface.
