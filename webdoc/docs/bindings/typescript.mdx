...
| `mode`          | `"memory"` or `"disk"` — the same choice as [`--mode`](../reference/cli.mdx)         |
...
[`<gen type="http">`](../generators/http.mdx) **must** use it. A network call cannot be
...
:::note[Same values, one row at a time]
...
time](../getting-started/quick-api.mdx) is the whole surface.
