...
impl Validator {
    ...
    fn check_uniq_memory(&mut self, open: &Element, named: Option<&str>) {
        ...
        self.warn(
            "TDC299",
            format!(
                "uniq on \"{}\" costs about {} at {} rows — memory that follows the row count",
                named.unwrap_or("?"),
                size,
                Self::grouped(self.env_count)
            ),
            "Keeping a promise about the finished column costs memory that follows count, on \
             every engine — the cost belongs to the promise, not to one of them. About 250 bytes \
             a value, measured; a compound uniq measures higher still. A single drawn column pays \
             twice: drawing without replacement cannot be done a row at a time, so that shape \
             also runs in memory whatever mode= asks for. It works — it is worth being deliberate \
             about at this size.",
            open.pos,
        );
    }
