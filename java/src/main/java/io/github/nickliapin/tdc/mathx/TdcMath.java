...
public final class TdcMath {
  ...
  /**
   * The largest k for which 10^k and 10^-k are both exactly doubles. Past 10^22 a power of ten is
   * no longer representable, so beyond here "exact" has nothing to mean.
   */
  private static final double EXACT_POWER_OF_TEN = 22;

  /**
   * {@code log10(x)}.
   *
   * <p>A power of ten is the argument someone passes to log10, and log(x)/ln10 gets it wrong: it
   * returned 2.9999999999999996 for 1000. There is no exponent to separate here the way log2
   * separates a power of two — a double is binary — so the whole answer is checked instead: if
   * raising ten back to it returns the argument unchanged, it was exact.
   */
  public static double log10(double x) {
    double r = log(x) / 2.302585092994046;
    double whole = r >= 0 ? Math.floor(r + 0.5) : Math.ceil(r - 0.5);
