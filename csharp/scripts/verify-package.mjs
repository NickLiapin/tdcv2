...
try {
  ...
  const declared = /<Version>([^<]+)<\/Version>/.exec(
    readFileSync(join(projectDir, 'Tdcv2', 'Tdcv2.csproj'), 'utf8'),
  )?.[1];
  if (!declared) {
    throw new Error('could not read <Version> from csharp/Tdcv2/Tdcv2.csproj');
  }
  const nupkg = `Tdcv2.${declared}.nupkg`;
